...
#include "lib/lib_math_rotation.h"
#include "lib/lib_math_geom.h"
...
#  define LIB_ASSERT_UNIT_QUAT(q)               assert(fabs(len_squared_v4(q)-1.0)<QUAT_EPSILON)
#  define LIB_ASSERT_UNIT_V3(v)                 assert(fabs(len_squared_v3(v)-1.0)<QUAT_EPSILON)
#  define LIB_ASSERT_UNIT_V3_DB(v)              assert(fabs(len_squared_v3_db(v)-1.0)<QUAT_EPSILON)
#  define LIB_ASSERT_UNIT_V2(v)                 assert(fabs(len_squared_v2(v)-1.0)<QUAT_EPSILON)            
#  define LIB_ASSERT_UNIT_V2_DB(v)              assert(fabs(len_squared_v2_db(v)-1.0)<QUAT_EPSILON)            
#endif

#ifndef LIB_ASSERT_UNIT_QUAT
#  define LIB_ASSERT_UNIT_QUAT(q)               
#endif
#ifndef LIB_ASSERT_UNIT_M4
#  define LIB_ASSERT_UNIT_M4(m)               
#endif
#ifndef LIB_ASSERT_UNIT_M3
#  define LIB_ASSERT_UNIT_M3(m)               
#endif  
#ifndef LIB_ASSERT_UNIT_M2
#  define LIB_ASSERT_UNIT_M2(m)               
#endif
...
void mid_v3_v3_array ( float r [ 3 ] , const float ( *vec_arr ) [ 3 ] , const unsigned int vec_arr_num ) {
        const float factor = 1.0f / ( float ) vec_arr_num;
        zero_v3 ( r );

        for ( unsigned int i = 0; i < vec_arr_num; i++ ) {
...
void mid_v3_angle_weighted ( float r [ 3 ] ) {
        ...
        /* double check they are normalized */
        assert ( len_squared_v3 ( r ) <= 1.0f + FLT_EPSILON );
