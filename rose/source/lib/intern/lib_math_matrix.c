...
float mat3_to_size_max_axis ( const float M [ 3 ][ 3 ] ) {
        return sqrtf ( ROSE_MAX ( len_squared_v3 ( M [ 0 ] ) , len_squared_v3 ( M [ 1 ] ) , len_squared_v3 ( M [ 2 ] ) ) );
}
...
void svd_m4 ( float U [ 4 ][ 4 ] , float s [ 4 ] , float V [ 4 ][ 4 ] , float A_ [ 4 ][ 4 ] ) {
        ...
        int nct = ROSE_MIN ( m - 1 , n );
        int nrt = ROSE_MIN ( 0 , ROSE_MIN ( n - 2 , m ) );

        copy_m4_m4 ( A , A_ );
        zero_m4 ( U );
        zero_v4 ( s );

        for ( k = 0; k < ROSE_MAX ( nct , nrt ); k++ ) {
                if ( k < nct ) {
