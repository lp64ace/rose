...
static void update_global_peak() {
	...
	for (Local *local : global.locals) {
		assert(!local->destructed);
		/** Updating this makes sure that the peak is not updated too often, which would degrade performance. */
...
void memory_usage_block_free(const size_t size) {
	if (use_local_counters) {
		/** Decrease local memory counts. See comment in #memory_usage_block_alloc for details regarding thread
		 * synchronization. */
		Local &local = get_local_data();
