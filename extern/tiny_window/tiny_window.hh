...
#ifndef TINYWINDOW_HH
#define TINYWINDOW_HH
...
struct monitorSetting_t {
	...
	monitorSetting_t(vec2_t<unsigned int> inResolution = vec2_t<unsigned int>().Zero(), unsigned int inBitsPerPixel = 0, unsigned int inDisplayFrequency = 0) : resolution(inResolution), bitsPerPixel(inBitsPerPixel), displayFrequency(inDisplayFrequency) {
...
public:
	formatSetting_t(int redBits = 8, int greenBits = 8, int blueBits = 8, int alphaBits = 8, int depthBits = 32, int stencilBits = 8, int accumRedBits = 8, int accumGreenBits = 8, int accumBlueBits = 8, int accumAlphaBits = 8, int auxBuffers = 0, int numSamples = 0, bool stereo = false, bool doubleBuffer = true) {
...
struct windowSetting_t {
	...
	char versionMajor; /**< Major OpenGL version*/
	char versionMinor; /**< Minor OpenGL version*/
private:
...
public:
...
#if defined(TW_WINDOWS)
		SetWindowPos(windowHandle, HWND_TOP, position.x, position.y, newResolution.x, newResolution.y, SWP_SHOWWINDOW | SWP_NOMOVE | SWP_NOZORDER | SWP_NOACTIVATE);
#elif defined(TW_LINUX)
		XResizeWindow(currentDisplay, windowHandle, newResolution.x, newResolution.y);
#endif
	...
	std::error_code SetPosition(TinyWindow::vec2_t<int> newPosition) {
...
#if defined(TW_WINDOWS)
		SetWindowPos(windowHandle, HWND_TOP, newPosition.x, newPosition.y, settings.resolution.x, settings.resolution.y, SWP_SHOWWINDOW | SWP_NOSIZE | SWP_NOZORDER | SWP_NOACTIVATE);
#elif defined(TW_LINUX)
		...
		XConfigureWindow(currentDisplay, windowHandle, CWX | CWY, &windowChanges);
#endif
		return TinyWindow::error_t::success;
	}
...
#elif defined(TW_LINUX)
		XWarpPointer(currentDisplay, windowHandle, windowHandle, position.x, position.y, settings.resolution.width, settings.resolution.height, newMousePosition.x, newMousePosition.y);
#endif
		return TinyWindow::error_t::success;
	}
...
#	endif
	...
	std::error_code Maximize(bool newState) {
		if (newState) {
			...
			XSendEvent(currentDisplay, windowHandle, 0, SubstructureNotifyMask, &currentEvent);
#endif
		}
	...
	std::error_code SetFullScreen(bool newState /* need to add definition for which screen*/) {
		settings.currentState = (newState) ? state_t::fullscreen : state_t::normal;

#if defined(TW_WINDOWS)

		SetWindowLongPtr(windowHandle, GWL_STYLE, static_cast<LONG_PTR>(WS_SYSMENU | WS_POPUP | WS_CLIPCHILDREN | WS_CLIPSIBLINGS | WS_VISIBLE));
	...
	std::error_code SetStyle(style_t windowStyle) {
		...
		switch (windowStyle) {
			case style_t::normal: {
				EnableDecorators(titleBar | border | closeButton | minimizeButton | maximizeButton | sizeableBorder);
				break;
			}
		...
		switch (windowStyle) {
			case style_t::normal: {
				...
				currentStyle = linuxMove | linuxClose | linuxMaximize | linuxMinimize;
				long Hints[5] = {hint_t::function | hint_t::decorator, currentStyle, linuxDecorators, 0, 0};

				XChangeProperty(currentDisplay, windowHandle, AtomHints, XA_ATOM, 32, PropModeReplace, (unsigned char *)Hints, 5);
	...
	std::error_code EnableDecorators(unsigned int decorators) {
		...
		SetWindowPos(windowHandle, HWND_TOP, position.x, position.y, settings.resolution.width, settings.resolution.height, SWP_FRAMECHANGED);
	...
	std::error_code DisableDecorators(unsigned int decorators) {
		...
		SetWindowLongPtr(windowHandle, GWL_STYLE, static_cast<LONG_PTR>(currentStyle | WS_VISIBLE));

		SetWindowPos(windowHandle, HWND_TOPMOST, position.x, position.y, settings.resolution.width, settings.resolution.height, SWP_FRAMECHANGED);
#elif defined(TW_LINUX)
		if (decorators & closeButton) {
...
#elif defined(TW_LINUX)
		// afaik linux does not need to generate a dummy context
		return error_t::functionNotImplemented;
#endif
	}
	...
	static LRESULT CALLBACK WindowProcedure(HWND windowHandle, unsigned int winMessage, WPARAM wordParam, LPARAM longParam) {
		...
		switch (winMessage) {
			...
			case WM_XBUTTONUP: {
				...
				if (XButton == XBUTTON1) {
					window->mouseButton[(unsigned int)mouseButton_t::XFirst] = buttonState_t::up;
				...
				if (XButton == XBUTTON2) {
					window->mouseButton[(unsigned int)mouseButton_t::XSecond] = buttonState_t::up;
...
#	endif
	...
	std::error_code Windows_CreateDummyWindow() {
		...
		dummyWindowHandle = CreateWindow("dummy", "dummy", WS_OVERLAPPEDWINDOW, 0, 0, 1, 1, nullptr, nullptr, nullptr, nullptr);
	...
	static void Windows_SetWindowIcon(tWindow *window, const char *icon, unsigned int width, unsigned int height) {
		SendMessage(window->windowHandle, (UINT)WM_SETICON, ICON_BIG, (LPARAM)LoadImage(window->instanceHandle, icon, IMAGE_ICON, (int)width, (int)height, LR_LOADFROMFILE));
	}
	...
	std::error_code Linux_InitializeWindow(tWindow *window) {
		...
		*/
		...
		window->windowHandle = XCreateWindow(currentDisplay, XDefaultRootWindow(currentDisplay), 0, 0, window->settings.resolution.width, window->settings.resolution.height, 0, window->visualInfo->depth, InputOutput, window->visualInfo->visual, CWColormap | CWEventMask, &window->setAttributes);
	...
	void Linux_ProcessEvents(XEvent currentEvent) {
		...
		switch (currentEvent.type) {
			...
			case KeyPress: {
				unsigned int functionKeysym = XkbKeycodeToKeysym(currentDisplay, currentEvent.xkey.keycode, 0, currentEvent.xkey.state & ShiftMask ? 1 : 0);
			...
			case KeyRelease: {
				...
				if (XEventsQueued(currentDisplay, QueuedAfterReading)) {
					...
					if (nextEvent.type == KeyPress && nextEvent.xkey.time == currentEvent.xkey.time && nextEvent.xkey.keycode == currentEvent.xkey.keycode) {
						unsigned int functionKeysym = XkbKeycodeToKeysym(currentDisplay, currentEvent.xkey.keycode, 0, currentEvent.xkey.state & ShiftMask ? 1 : 0);
			...
			case MotionNotify: {
				...
				window->mousePosition.y = currentEvent.xmotion.y;
				...
				if (mouseMoveEvent != nullptr) {
					mouseMoveEvent(window, vec2_t<int>(currentEvent.xmotion.x, currentEvent.xmotion.y), vec2_t<int>(currentEvent.xmotion.x_root, currentEvent.xmotion.y_root));
				}
				break;
			}
			...
			case ResizeRequest: {
				...
				if (resizeEvent != nullptr) {
					resizeEvent(window, vec2_t<unsigned int>(currentEvent.xresizerequest.width, currentEvent.xresizerequest.height));
				}
			...
			case ConfigureNotify: {
				glViewport(0, 0, currentEvent.xconfigure.width, currentEvent.xconfigure.height);

				// check if window was resized
				if ((unsigned int)currentEvent.xconfigure.width != window->settings.resolution.width || (unsigned int)currentEvent.xconfigure.height != window->settings.resolution.height) {
					if (resizeEvent != nullptr) {
				...
				if (currentEvent.xconfigure.x != window->position.x || currentEvent.xconfigure.y != window->position.y) {
			...
			case PropertyNotify: {
				...
				XGetWindowProperty(currentDisplay, currentEvent.xproperty.window, window->AtomState, 0, LONG_MAX, false, AnyPropertyType, &type, &format, &numItems, &bytesAfter, &properties);
	...
	GLXFBConfig GetBestFrameBufferConfig(tWindow *window) {
		...
		for (int configIndex = 0; configIndex < frameBufferCount; configIndex++) {
			...
			if (visualInfo) {
				...
				glXGetFBConfigAttrib(currentDisplay, configs[configIndex], GLX_BUFFER_SIZE, &bufferSize);
				glXGetFBConfigAttrib(currentDisplay, configs[configIndex], GLX_DOUBLEBUFFER, &doubleBuffer);
				glXGetFBConfigAttrib(currentDisplay, configs[configIndex], GLX_AUX_BUFFERS, &numAuxBuffers);
				glXGetFBConfigAttrib(currentDisplay, configs[configIndex], GLX_RED_SIZE, &redSize);
				glXGetFBConfigAttrib(currentDisplay, configs[configIndex], GLX_GREEN_SIZE, &greenSize);
				glXGetFBConfigAttrib(currentDisplay, configs[configIndex], GLX_BLUE_SIZE, &blueSize);
				glXGetFBConfigAttrib(currentDisplay, configs[configIndex], GLX_ALPHA_SIZE, &alphaSize);
				glXGetFBConfigAttrib(currentDisplay, configs[configIndex], GLX_DEPTH_SIZE, &depthSize);
				glXGetFBConfigAttrib(currentDisplay, configs[configIndex], GLX_STENCIL_SIZE, &stencilSize);
		...
		GLXFBConfig BestConfig = configs[0];
	...
	std::error_code Linux_InitGL(tWindow *window) {
		...
		glXMakeCurrent(currentDisplay, window->windowHandle, dummyContext);

		// we need to load our extensions first
		InitExtensions();
#	if defined(DEBUG)
		printf("%s \n", glXQueryExtensionsString(window->currentDisplay, XDefaultScreen(window->currentDisplay)));
#	endif
		...
		int blarg = glGetError();
		if (blarg != GL_NO_ERROR) {
		}
		...
		if (window->context) {
			...
			XGetWindowAttributes(currentDisplay, window->windowHandle, &attributes);
