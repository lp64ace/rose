...
static char *create_key(SupportLevel support_level, const char *vendor, const char *renderer, const char *version) {
	...
	switch (support_level) {
		case GPU_SUPPORT_LEVEL_SUPPORTED: {
			support_key = LIB_sprintf_allocN("[%s/SUPPORTED] %s %s", vendor, renderer, version);
		} break;
		case GPU_SUPPORT_LEVEL_LIMITED: {
			support_key = LIB_sprintf_allocN("[%s/LIMITED] %s %s", vendor, renderer, version);
		} break;
		default: {
			support_key = LIB_sprintf_allocN("[%s/UNSUPPORTED] %s %s", vendor, renderer, version);
		} break;
	}
...
static char *create_gpu_name(const char *vendor, const char *renderer, const char *version) {
	char *gpu_name = LIB_sprintf_allocN("[%s] %s %s", vendor, renderer, version);
...
void GPUPlatformGlobal::init(DeviceType device,
