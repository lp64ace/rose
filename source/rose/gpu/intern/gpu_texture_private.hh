...
typedef enum TextureFormatFlag {
...
ENUM_OPERATORS(TextureFormatFlag, GPU_FORMAT_SIGNED)
...
ENUM_OPERATORS(SamplerFormat, GPU_SAMPLER_TYPE_UINT)
...
protected:
	...
	TextureFormatFlag format_flag_;
	/** Texture type. */
	TextureType type_;
	/** Texture usage flags. */
	TextureUsage gpu_image_usage_flags_;
	...
	AttachmentType fb_attachment_[GPU_TEX_MAX_FBO_ATTACHED];
...
public:
	...
	virtual void clear(DataFormat format, const void *data) = 0;
	virtual void swizzle_set(const char swizzle_mask[4]) = 0;
	virtual void mip_range_set(int min, int max) = 0;
	virtual void *read(int mip, DataFormat format) = 0;

	void attach_to(FrameBuffer *fb, AttachmentType type);
	void detach_from(FrameBuffer *fb);

	void update(DataFormat format, const void *data);

	void usage_set(TextureUsage usage_flags);

	virtual void update_sub(int mip, int offset[3], int extent[3], DataFormat format, const void *data) = 0;
	virtual void update_sub(int offset[3], int extent[3], DataFormat format, GPUPixelBuffer *pixbuf) = 0;
	...
	void mip_size_get(int mip, int r_size[3]) const {
		...
		int div = 1 << mip;
		r_size[0] = ROSE_MAX(1, w_ / div);

		if (type_ == GPU_TEXTURE_1D_ARRAY) {
			r_size[1] = h_;
		}
		else if (h_ > 0) {
			r_size[1] = ROSE_MAX(1, h_ / div);
		}
		...
		if (type_ & (GPU_TEXTURE_ARRAY | GPU_TEXTURE_CUBE)) {
			...
			r_size[2] = ROSE_MAX(1, d_ / div);
		}
	}

	int mip_width_get(int mip) const {
		return ROSE_MAX(1, w_ / (1 << mip));
	}
	int mip_height_get(int mip) const {
		return (type_ == GPU_TEXTURE_1D_ARRAY) ? h_ : ROSE_MAX(1, h_ / (1 << mip));
	}
	int mip_depth_get(int mip) const {
		return (type_ & (GPU_TEXTURE_ARRAY | GPU_TEXTURE_CUBE)) ? d_ : ROSE_MAX(1, d_ / (1 << mip));
	}
	...
	TextureFormatFlag format_flag_get() const {
		return format_flag_;
	}
	TextureType type_get() const {
		return type_;
	}
	AttachmentType attachment_type(int slot) const {
		switch (format_) {
			...
			case GPU_SRGB8:
			case GPU_RGB9_E5:
				ROSE_assert_msg(0, "Texture cannot be attached to a framebuffer because of its type");
...
inline size_t to_block_size(TextureFormat data_type) {
	switch (data_type) {
		...
		case GPU_RGBA8_DXT5:
			return 16;
		default:
			ROSE_assert_msg(0, "Texture format is not a compressed format");
			return 0;
	}
}

inline TextureFormatFlag to_format_flag(TextureFormat format) {
	switch (format) {
		/* Formats texture & render-buffer */
		case GPU_RGBA8UI:
...
inline size_t to_bytesize(DataFormat data_format) {
	switch (data_format) {
		case GPU_DATA_UBYTE:
			return 1;
...
inline size_t to_bytesize(TextureFormat tex_format, DataFormat data_format) {
...
constexpr inline bool validate_data_format(TextureFormat tex_format, DataFormat data_format) {
	switch (tex_format) {
		/* Formats texture & render-buffer */
		case GPU_RGBA32UI:
		case GPU_RG32UI:
...
inline DataFormat to_data_format(TextureFormat tex_format) {
	switch (tex_format) {
		/* Formats texture & render-buffer */
		case GPU_RGBA32UI:
		case GPU_RG32UI:
...
static inline TextureFormat to_texture_format(const GPUVertFormat *format) {
	if (format->attr_len == 0) {
		ROSE_assert_msg(0, "Incorrect vertex format for buffer texture");
	...
	ROSE_assert_msg(0, "Unsupported vertex format for buffer texture");
	return GPU_DEPTH_COMPONENT24;
}
