...
void GPU_face_culling(FaceCullTest culling) {
	SET_IMMUTABLE_STATE(culling_test, culling);
}

FaceCullTest GPU_face_culling_get() {
	GPUState &state = Context::get()->state_manager->state;
	return (FaceCullTest)state.culling_test;
}
...
void GPU_provoking_vertex(ProvokingVertex vert) {
...
void GPU_state_set(WriteMask write_mask,
				   Blend blend,
				   FaceCullTest culling_test,
				   DepthTest depth_test,
				   StencilTest stencil_test,
				   StencilOp stencil_op,
				   ProvokingVertex provoking_vert) {
	StateManager *stack = Context::get()->state_manager;
	auto &state = stack->state;
...
Blend GPU_blend_get() {
	GPUState &state = Context::get()->state_manager->state;
	return (Blend)state.blend;
}

WriteMask GPU_write_mask_get() {
	GPUState &state = Context::get()->state_manager->state;
	return (WriteMask)state.write_mask;
}
...
DepthTest GPU_depth_test_get() {
	GPUState &state = Context::get()->state_manager->state;
	return (DepthTest)state.depth_test;
}

StencilTest GPU_stencil_test_get() {
	GPUState &state = Context::get()->state_manager->state;
	return (StencilTest)state.stencil_test;
}
...
void GPU_bgl_start() {
	...
	if (state_manager.use_bgl == false) {
		 ...
		 */
		{
			DepthTest depth_test_real = GPU_depth_test_get();
			DepthTest depth_test_temp = GPU_DEPTH_LESS_EQUAL;
...
StateManager::StateManager() {
	...
	/* Negative is not using point size. */
