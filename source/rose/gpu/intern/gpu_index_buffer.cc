...
void GPU_indexbuf_init_ex(GPUIndexBufBuilder *builder, PrimType primitive, uint index_len, uint vertex_len) {
	...
	builder->index_max = 0;
	builder->prim_type = primitive;

#ifdef __APPLE__
	/**
	 * Only encode restart indices for restart-compatible primitive types.
	 * Resolves out-of-bounds read error on macOS. Using 0-index will ensure degenerative primitives when
	 * skipping primitives is required and will incur no additional performance cost for rendering.
	 */
	if (GPU_type_matches_ex(GPU_DEVICE_ANY, GPU_OS_MAC, GPU_DRIVER_ANY, GPU_BACKEND_METAL)) {
		/**
		 * We will still use restart-indices for point primitives and then patch these during IndexBuf::init,
		 * as we cannot benefit from degenerative primitives to eliminate these.
		 */
		builder->restart_index_value = (is_restart_compatible(prim_type) || prim_type == GPU_PRIM_POINTS) ? RESTART_INDEX : 0;
	}
	else {
	...
	builder->uses_restart_index = false;
	builder->data = (uint32_t *)MEM_callocN(builder->max_index_len * sizeof(uint), "rose::gpu::IndexBuf::data");
}

void GPU_indexbuf_init(GPUIndexBufBuilder *builder, PrimType prim_type, uint prim_len, uint vertex_len) {
	int vertex_per_prim = GPU_indexbuf_primitive_len(prim_type);

	ROSE_assert(vertex_per_prim > 0);
	GPU_indexbuf_init_ex(builder, prim_type, prim_len * (uint)vertex_per_prim, vertex_len);
}

void GPU_indexbuf_join(GPUIndexBufBuilder *builder, const GPUIndexBufBuilder *builder_from) {
	ROSE_assert(builder->data == builder_from->data);
	builder->index_len = ROSE_MAX(builder->index_len, builder_from->index_len);
	builder->index_min = ROSE_MIN(builder->index_min, builder_from->index_min);
	builder->index_max = ROSE_MAX(builder->index_max, builder_from->index_max);
}
...
void GPU_indexbuf_add_generic_vert(GPUIndexBufBuilder *builder, uint v) {
	...
	ROSE_assert(builder->index_len < builder->max_index_len);
	ROSE_assert(v <= builder->max_allowed_index);

	builder->data[builder->index_len++] = v;
	builder->index_min = ROSE_MIN(builder->index_min, v);
	builder->index_max = ROSE_MAX(builder->index_max, v);
}
...
void GPU_indexbuf_add_tri_verts(GPUIndexBufBuilder *builder, uint v1, uint v2, uint v3) {
	ROSE_assert(builder->prim_type == GPU_PRIM_TRIS);
	ROSE_assert(v1 != v2 && v2 != v3 && v3 != v1);
...
void GPU_indexbuf_add_line_adj_verts(GPUIndexBufBuilder *builder, uint v1, uint v2, uint v3, uint v4) {
	ROSE_assert(builder->prim_type == GPU_PRIM_LINES_ADJ);
	ROSE_assert(v2 != v3);
...
void GPU_indexbuf_set_point_vert(GPUIndexBufBuilder *builder, uint elem, uint v1) {
	ROSE_assert(builder->prim_type == GPU_PRIM_POINTS);
	ROSE_assert(elem < builder->max_index_len);

	builder->data[elem++] = v1;
	builder->index_min = ROSE_MIN(builder->index_min, v1);
	builder->index_max = ROSE_MAX(builder->index_max, v1);
	builder->index_len = ROSE_MAX(builder->index_len, elem);
}

void GPU_indexbuf_set_line_verts(GPUIndexBufBuilder *builder, uint elem, uint v1, uint v2) {
	ROSE_assert(builder->prim_type == GPU_PRIM_LINES);
	ROSE_assert(v1 != v2);
	ROSE_assert(v1 <= builder->max_allowed_index);
	ROSE_assert(v2 <= builder->max_allowed_index);
	ROSE_assert((elem + 1) * 2 <= builder->max_index_len);

	uint idx = elem * 2;
	builder->data[idx++] = v1;
	builder->data[idx++] = v2;
	builder->index_min = ROSE_MIN(builder->index_min, v1, v2);
	builder->index_max = ROSE_MAX(builder->index_max, v1, v2);
	builder->index_len = ROSE_MAX(builder->index_len, idx);
}
...
void GPU_indexbuf_set_tri_verts(GPUIndexBufBuilder *builder, uint elem, uint v1, uint v2, uint v3) {
	...
	ROSE_assert(v1 <= builder->max_allowed_index);
	ROSE_assert(v2 <= builder->max_allowed_index);
	ROSE_assert(v3 <= builder->max_allowed_index);
	ROSE_assert((elem + 1) * 3 <= builder->max_index_len);
	...
	builder->index_min = ROSE_MIN(builder->index_min, v1, v2, v3);
	builder->index_max = ROSE_MAX(builder->index_max, v1, v2, v3);
	builder->index_len = ROSE_MAX(builder->index_len, idx);
}
...
void IndexBuf::init_subrange(IndexBuf *elem_src, uint start, uint length) {
	...
	ROSE_assert(elem_src && elem_src->is_subrange_ == false);
	ROSE_assert((length == 0) || (start + length <= elem_src->index_len_));
...
void IndexBuf::squeeze_indices_short(uint min_idx, uint max_idx, PrimType prim_type, bool clamp_indices_in_range) {
	/** data will never be *larger* than builder->data... converting in place to avoid extra allocation */
...
void GPU_indexbuf_discard(GPUIndexBuf *elem) {
	MEM_delete<IndexBuf>(unwrap(elem));
}
