...
void Shader::print_log(Span<const char *> sources, const char *log, const char *stage, const bool error, GPULogParser *parser) {
	...
	// TODO: check if we have color support.
	if (false) {
	...
	while ((line_end = strchr(log_line, '\n'))) {
		...
		if (logref.endswith(" shader failed to compile with the following errors:") || logref.endswith(" No code generated")) {
		...
		if (previous_location.source != log_item.cursor.source || previous_location.row != log_item.cursor.row) {
		...
		if ((log_item.cursor.row != -1) && (log_item.cursor.row != previous_location.row || log_item.cursor.column != previous_location.column)) {
			...
			while ((src_line_end = strchr(src_line, '\n'))) {
				...
				if (src_line_index >= log_item.cursor.row - DEBUG_CONTEXT_LINES) {
					...
					fprintf(stderr, "%.*s", static_cast<int>(src_line_end + 1 - src_line), src_line);
				}
		...
		if (log_item.severity == Severity::Error) {
			fprintf(stderr, "%s%s%s: ", err_col, "Error", info_col);
		}
		else if (log_item.severity == Severity::Warning) {
			fprintf(stderr, "%s%s%s: ", warn_col, "Warning", info_col);
		}
		else if (log_item.severity == Severity::Note) {
			fprintf(stderr, "%s%s%s: ", warn_col, "Note", info_col);
		}
		/* Print the error itself. */
		fprintf(stderr, "%s", info_col);
		fprintf(stderr, "%.*s", static_cast<int>(line_end + 1 - log_line), log_line);
