...
void ShaderInterface::sort_inputs() {
	...
	sort_input_list(MutableSpan<ShaderInput>(inputs_ + attr_len_ + ubo_len_ + uniform_len_, ssbo_len_));
}
...
void ShaderInterface::debug_print() const {
	...
	Span<ShaderInput> ssbos = Span<ShaderInput>(inputs_ + attr_len_ + ubo_len_ + uniform_len_, ssbo_len_);
