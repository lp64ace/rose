...
public:
	...
	Self &push_constant(Type type, StringRefNull name, int array_size = 0) {
		ROSE_assert_msg(name.find("[") == LIB_NPOS, "Array syntax is forbidden for push constants. Use the array_size parameter instead.");
		...
		auto print_resource = [&](const Resource &res) {
			switch (res.bind_type) {
				case Resource::BindType::UNIFORM_BUFFER:
					stream << "UNIFORM_BUFFER(" << res.slot << ", " << res.uniformbuf.name << ")" << std::endl;
					break;
				case Resource::BindType::STORAGE_BUFFER:
					stream << "STORAGE_BUFFER(" << res.slot << ", " << res.storagebuf.name << ")" << std::endl;
					break;
