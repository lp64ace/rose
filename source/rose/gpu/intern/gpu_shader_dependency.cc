...
struct GPUSource {
	...
	static bool is_in_comment(const StringRef &input, int64_t offset) {
		return (input.rfind("/*", offset) > input.rfind("*/", offset)) || (input.rfind("//", offset) > input.rfind("\n", offset));
	}

	template<bool check_whole_word = true, bool reversed = false, typename T> static int64_t find_str(const StringRef &input, const T keyword, int64_t offset = 0) {
		while (true) {
...
#define CHECK(test_value, str, ofs, msg) \
	...
	void material_functions_parse(GPUFunctionDictionnary *g_functions) {
		...
		auto function_parse = [&](const StringRef input, int64_t &cursor, StringRef &out_return_type, StringRef &out_name, StringRef &out_args) -> bool {
		...
		auto arg_parse = [&](const StringRef str, int64_t &cursor, StringRef &out_qualifier, StringRef &out_type, StringRef &out_name) -> bool {
	...
	void char_literals_preprocess() {
		...
		while (true) {
			...
			char hex[8];
			LIB_snprintf(hex, ARRAY_SIZE(hex), "0x%.2Xu", char_value);
			output << hex;
		...
		while (true) {
			...
			auto print_string = [&](std::string str) -> int {
				...
				for (size_t i = 0; i < len_before_pad; i += 4) {
					...
					char uint_hex[12];
					LIB_snprintf(uint_hex, ARRAY_SIZE(uint_hex), "0x%.2X%.2X%.2X%.2Xu", chars[3], chars[2], chars[1], chars[0]);
