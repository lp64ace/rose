...
void FrameBuffer::attachment_set(AttachmentType type, const GPUAttachment &new_attachment) {
	...
	if (new_attachment.tex) {
		if (new_attachment.layer > 0) {
			ROSE_assert(GPU_texture_is_cube(new_attachment.tex) || GPU_texture_is_array(new_attachment.tex));
		}
		if (GPU_texture_has_stencil_format(new_attachment.tex)) {
			ROSE_assert(ELEM(type, GPU_FB_DEPTH_STENCIL_ATTACHMENT));
		}
		else if (GPU_texture_has_depth_format(new_attachment.tex)) {
			ROSE_assert(ELEM(type, GPU_FB_DEPTH_ATTACHMENT));
		}
	}
...
void FrameBuffer::attachment_remove(AttachmentType type) {
...
void FrameBuffer::load_store_config_array(const GPULoadStore *load_store_actions, uint actions_len) {
	...
	AttachmentType type = GPU_FB_COLOR_ATTACHMENT0;
...
void FrameBuffer::recursive_downsample(int max_lvl, void (*callback)(void *user_data, int level), void *user_data) {
	/* Bind to make sure the frame-buffer is up to date. */
	this->bind(true);

	/* FIXME(fclem): This assumes all mips are defined which may not be the case. */
	max_lvl = ROSE_MIN(max_lvl, floor(log2(ROSE_MAX(width_, height_))));
	...
	for (int mip_lvl = 1; mip_lvl <= max_lvl; mip_lvl++) {
		...
		AttachmentType type = GPU_FB_DEPTH_ATTACHMENT;
		for (GPUAttachment &attachment : attachments_) {
			Texture *tex = reinterpret_cast<Texture *>(attachment.tex);
			if (tex != nullptr) {
...
void GPU_framebuffer_free(GPUFrameBuffer *gpu_fb) {
	MEM_delete<FrameBuffer>(unwrap(gpu_fb));
}
...
void GPU_framebuffer_subpass_transition_array(GPUFrameBuffer *gpu_fb,
											  const AttachmentState *attachment_states,
											  uint attachment_len) {
...
void GPU_backbuffer_bind(BackBuffer buffer) {
...
static void gpu_framebuffer_texture_attach_ex(GPUFrameBuffer *gpu_fb, GPUAttachment attachment, int slot) {
	Texture *tex = reinterpret_cast<Texture *>(attachment.tex);
	AttachmentType type = tex->attachment_type(slot);
...
void GPU_framebuffer_clear(
	GPUFrameBuffer *gpu_fb, FrameBufferBits buffers, const float clear_col[4], float clear_depth, uint clear_stencil) {
...
void GPU_framebuffer_read_color(
	GPUFrameBuffer *gpu_fb, int x, int y, int w, int h, int channels, int slot, DataFormat format, void *data) {
	int rect[4] = {x, y, w, h};
	unwrap(gpu_fb)->read(GPU_COLOR_BIT, format, rect, channels, slot, data);
}

void GPU_frontbuffer_read_color(int x, int y, int w, int h, int channels, DataFormat format, void *data) {
	int rect[4] = {x, y, w, h};
...
void GPU_framebuffer_blit(
	GPUFrameBuffer *gpufb_read, int read_slot, GPUFrameBuffer *gpufb_write, int write_slot, FrameBufferBits blit_buffers) {
	...
	if (blit_buffers & GPU_DEPTH_BIT) {
		ROSE_assert(GPU_texture_has_depth_format(read_tex) && GPU_texture_has_depth_format(write_tex));
		ROSE_assert(GPU_texture_format(read_tex) == GPU_texture_format(write_tex));
	}
	if (blit_buffers & GPU_STENCIL_BIT) {
		ROSE_assert(GPU_texture_has_stencil_format(read_tex) && GPU_texture_has_stencil_format(write_tex));
		ROSE_assert(GPU_texture_format(read_tex) == GPU_texture_format(write_tex));
	}
#endif
...
GPUOffScreen *GPU_offscreen_create(
	int width, int height, bool depth, TextureFormat format, TextureUsage usage, char err_out[256]) {
	...
	if (depth) {
		...
		TextureUsage depth_usage = usage | GPU_TEXTURE_USAGE_FORMAT_VIEW;
		ofs->depth = GPU_texture_create_2d("ofs_depth", width, height, 1, GPU_DEPTH24_STENCIL8, depth_usage, nullptr);
	}

	if ((depth && !ofs->depth) || !ofs->color) {
		const char error[] = "GPUTexture: Texture allocation failed.";
		if (err_out) {
			LIB_strncpy(err_out, error, ARRAY_SIZE(err_out));
		}
		else {
...
void GPU_offscreen_read_color_region(GPUOffScreen *ofs, DataFormat format, int x, int y, int w, int h, void *r_data) {
	ROSE_assert(ELEM(format, GPU_DATA_UBYTE, GPU_DATA_FLOAT));
	ROSE_assert(x >= 0 && y >= 0 && w > 0 && h > 0);
	ROSE_assert(x + w <= GPU_texture_width(ofs->color));
	ROSE_assert(y + h <= GPU_texture_height(ofs->color));

	GPUFrameBuffer *ofs_fb = gpu_offscreen_fb_get(ofs);
	GPU_framebuffer_read_color(ofs_fb, x, y, w, h, 4, 0, format, r_data);
}

void GPU_offscreen_read_color(GPUOffScreen *ofs, DataFormat format, void *r_data) {
	ROSE_assert(ELEM(format, GPU_DATA_UBYTE, GPU_DATA_FLOAT));
...
TextureFormat GPU_offscreen_format(const GPUOffScreen *offscreen) {
