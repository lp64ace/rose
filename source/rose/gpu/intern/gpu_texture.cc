...
bool Texture::init_1D(int w, int layers, int mip_len, TextureFormat format) {
	...
	h_ = layers;
	d_ = 0;
	int mip_len_max = 1 + (int)(log2f(w));
	mipmaps_ = ROSE_MIN(mip_len, mip_len_max);
	format_ = format;
...
bool Texture::init_2D(int w, int h, int layers, int mip_len, TextureFormat format) {
	...
	h_ = h;
	d_ = layers;
	int mip_len_max = 1 + (int)(log2f(ROSE_MAX(w, h)));
	mipmaps_ = ROSE_MIN(mip_len, mip_len_max);
	format_ = format;
	format_flag_ = to_format_flag(format);
...
bool Texture::init_3D(int w, int h, int d, int mip_len, TextureFormat format) {
	...
	h_ = h;
	d_ = d;
	int mip_len_max = 1 + (int)(log2f(ROSE_MAX(w, h, d)));
	mipmaps_ = ROSE_MIN(mip_len, mip_len_max);
	format_ = format;
	format_flag_ = to_format_flag(format);
...
bool Texture::init_buffer(GPUVertBuf *vbo, TextureFormat format) {
...
bool Texture::init_view(GPUTexture *src_, TextureFormat format, TextureType type, int mip_start, int mip_len, int layer_start, int layer_len, bool cube_as_array, bool use_stencil) {
	const Texture *src = unwrap(src_);
	w_ = src->w_;
	h_ = src->h_;
	d_ = src->d_;
	layer_start = ROSE_MIN(layer_start, src->layer_count() - 1);
	layer_len = ROSE_MIN(layer_len, (src->layer_count() - layer_start));
	switch (type) {
		case GPU_TEXTURE_1D_ARRAY:
			h_ = layer_len;
			break;
		case GPU_TEXTURE_CUBE_ARRAY:
			ROSE_assert(layer_len % 6 == 0);
		case GPU_TEXTURE_2D_ARRAY:
			d_ = layer_len;
			break;
		default:
			ROSE_assert(layer_len == 1 && layer_start == 0);
			break;
	}
	mip_start = ROSE_MIN(mip_start, src->mipmaps_ - 1);
	mip_len = ROSE_MIN(mip_len, (src->mipmaps_ - mip_start));
	mipmaps_ = mip_len;
	format_ = format;
	format_flag_ = to_format_flag(format);
	type_ = type;
	if (cube_as_array) {
		ROSE_assert(type_ & GPU_TEXTURE_CUBE);
...
void Texture::usage_set(TextureUsage usage_flags) {
...
void Texture::attach_to(FrameBuffer *fb, AttachmentType type) {
	for (int i = 0; i < ARRAY_SIZE(fb_); i++) {
		if (fb_[i] == fb) {
	...
	ROSE_assert_msg(0, "GPU: Error: Texture: Not enough attachment");
}
...
void Texture::detach_from(FrameBuffer *fb) {
	...
	ROSE_assert_msg(0, "GPU: Error: Texture: Framebuffer is not attached");
}

void Texture::update(DataFormat format, const void *data) {
	int mip = 0;
	int extent[3] = {1, 1, 1};
	int offset[3] = {0, 0, 0};
...
size_t GPU_texture_memory_usage_get() {
...
static inline GPUTexture *gpu_texture_create(const char *name, const int w, const int h, const int d, const TextureType type, int mip_len, TextureFormat tex_format, DataFormat data_format, TextureUsage usage, const void *pixels) {
	ROSE_assert(mip_len > 0);
...
GPUTexture *GPU_texture_create_1d(const char *name, int w, int mip_len, TextureFormat format, TextureUsage usage, const float *data) {
	return gpu_texture_create(name, w, 0, 0, GPU_TEXTURE_1D, mip_len, format, GPU_DATA_FLOAT, usage, data);
}

GPUTexture *GPU_texture_create_1d_array(const char *name, int w, int h, int mip_len, TextureFormat format, TextureUsage usage, const float *data) {
	return gpu_texture_create(name, w, h, 0, GPU_TEXTURE_1D_ARRAY, mip_len, format, GPU_DATA_FLOAT, usage, data);
}

GPUTexture *GPU_texture_create_2d(const char *name, int w, int h, int mip_len, TextureFormat format, TextureUsage usage, const float *data) {
	return gpu_texture_create(name, w, h, 0, GPU_TEXTURE_2D, mip_len, format, GPU_DATA_FLOAT, usage, data);
}

GPUTexture *GPU_texture_create_2d_array(const char *name, int w, int h, int d, int mip_len, TextureFormat format, TextureUsage usage, const float *data) {
	return gpu_texture_create(name, w, h, d, GPU_TEXTURE_2D_ARRAY, mip_len, format, GPU_DATA_FLOAT, usage, data);
}

GPUTexture *GPU_texture_create_3d(const char *name, int w, int h, int d, int mip_len, TextureFormat texture_format, TextureUsage usage, const void *data) {
	return gpu_texture_create(name, w, h, d, GPU_TEXTURE_3D, mip_len, texture_format, GPU_DATA_FLOAT, usage, data);
}

GPUTexture *GPU_texture_create_cube(const char *name, int w, int mip_len, TextureFormat format, TextureUsage usage, const float *data) {
	return gpu_texture_create(name, w, w, 0, GPU_TEXTURE_CUBE, mip_len, format, GPU_DATA_FLOAT, usage, data);
}

GPUTexture *GPU_texture_create_cube_array(const char *name, int w, int d, int mip_len, TextureFormat format, TextureUsage usage, const float *data) {
	return gpu_texture_create(name, w, w, d, GPU_TEXTURE_CUBE_ARRAY, mip_len, format, GPU_DATA_FLOAT, usage, data);
}

GPUTexture *GPU_texture_create_compressed_2d(const char *name, int w, int h, int miplen, TextureFormat tex_format, TextureUsage usage, const void *data) {
	Texture *tex = GPUBackend::get()->texture_alloc(name);
	tex->usage_set(usage);
	...
	if (data) {
		...
		for (int mip = 0; mip < miplen; mip++) {
			...
			tex->update_sub(mip, offset, extent, to_data_format(tex_format), (unsigned char *)data + ofs);
...
GPUTexture *GPU_texture_create_error(int dimension, bool is_array) {
	...
	return gpu_texture_create("invalid_tex", w, h, d, type, 1, GPU_RGBA8, GPU_DATA_FLOAT, GPU_TEXTURE_USAGE_GENERAL, pixel);
}

GPUTexture *GPU_texture_create_view(const char *name, GPUTexture *src, TextureFormat format, int mip_start, int mip_len, int layer_start, int layer_len, bool cube_as_array, bool use_stencil) {
	ROSE_assert(mip_len > 0);
	ROSE_assert(layer_len > 0);
	ROSE_assert_msg(use_stencil == false || (GPU_texture_usage(src) & GPU_TEXTURE_USAGE_FORMAT_VIEW),
					"Source texture of TextureView must have GPU_TEXTURE_USAGE_FORMAT_VIEW usage "
					"flag if view texture uses stencil texturing.");
	ROSE_assert_msg((format == GPU_texture_format(src)) || (GPU_texture_usage(src) & GPU_TEXTURE_USAGE_FORMAT_VIEW),
					"Source texture of TextureView must have GPU_TEXTURE_USAGE_FORMAT_VIEW usage "
					"flag if view texture format is different.");
	Texture *view = GPUBackend::get()->texture_alloc(name);
	view->init_view(src, format, unwrap(src)->type_get(), mip_start, mip_len, layer_start, layer_len, cube_as_array, use_stencil);
	return wrap(view);
}

/* ------ Usage ------ */
TextureUsage GPU_texture_usage(const GPUTexture *texture_) {
...
void GPU_texture_update_mipmap(GPUTexture *tex_, int miplvl, DataFormat data_format, const void *pixels) {
...
void GPU_texture_update_sub(GPUTexture *tex, DataFormat data_format, const void *pixels, int offset_x, int offset_y, int offset_z, int width, int height, int depth) {
...
void GPU_texture_update_sub_from_pixel_buffer(GPUTexture *tex, DataFormat data_format, GPUPixelBuffer *pix_buf, int offset_x, int offset_y, int offset_z, int width, int height, int depth) {
	int offset[3] = {offset_x, offset_y, offset_z};
	int extent[3] = {width, height, depth};
	reinterpret_cast<Texture *>(tex)->update_sub(offset, extent, data_format, pix_buf);
}

void *GPU_texture_read(GPUTexture *tex_, DataFormat data_format, int miplvl) {
	Texture *tex = reinterpret_cast<Texture *>(tex_);
	ROSE_assert_msg(GPU_texture_usage(tex_) & GPU_TEXTURE_USAGE_HOST_READ,
...
void GPU_texture_clear(GPUTexture *tex, DataFormat data_format, const void *data) {
	ROSE_assert(data != nullptr); /* Do not accept nullptr as parameter. */
	reinterpret_cast<Texture *>(tex)->clear(data_format, data);
}

void GPU_texture_update(GPUTexture *tex, DataFormat data_format, const void *data) {
...
void GPU_texture_compare_mode(GPUTexture *tex_, bool use_compare) {
	...
	ROSE_assert(!(use_compare) || (tex->format_flag_get() & GPU_FORMAT_DEPTH));

	tex->sampler_state.type = use_compare ? GPU_SAMPLER_STATE_TYPE_CUSTOM : GPU_SAMPLER_STATE_TYPE_PARAMETERS;
...
void GPU_texture_filter_mode(GPUTexture *tex_, bool use_filter) {
	...
	ROSE_assert(!(use_filter) || !(tex->format_flag_get() & (GPU_FORMAT_STENCIL | GPU_FORMAT_INTEGER)));
...
void GPU_texture_mipmap_mode(GPUTexture *tex_, bool use_mipmap, bool use_filter) {
	Texture *tex = reinterpret_cast<Texture *>(tex_);
	/* Stencil and integer format does not support filtering. */
	ROSE_assert(!(use_filter || use_mipmap) || !(tex->format_flag_get() & (GPU_FORMAT_STENCIL | GPU_FORMAT_INTEGER)));
...
void GPU_texture_anisotropic_filter(GPUTexture *tex_, bool use_aniso) {
	Texture *tex = reinterpret_cast<Texture *>(tex_);
	/* Stencil and integer format does not support filtering. */
	ROSE_assert(!(use_aniso) || !(tex->format_flag_get() & (GPU_FORMAT_STENCIL | GPU_FORMAT_INTEGER)));
	tex->sampler_state.set_filtering_flag_from_test(GPU_SAMPLER_FILTERING_ANISOTROPIC, use_aniso);
}

void GPU_texture_extend_mode_x(GPUTexture *tex_, SamplerExtendMode extend_mode) {
	Texture *tex = reinterpret_cast<Texture *>(tex_);
	tex->sampler_state.extend_x = extend_mode;
}

void GPU_texture_extend_mode_y(GPUTexture *tex_, SamplerExtendMode extend_mode) {
	Texture *tex = reinterpret_cast<Texture *>(tex_);
	tex->sampler_state.extend_yz = extend_mode;
}

void GPU_texture_extend_mode(GPUTexture *tex_, SamplerExtendMode extend_mode) {
	Texture *tex = reinterpret_cast<Texture *>(tex_);
	tex->sampler_state.extend_x = extend_mode;
	tex->sampler_state.extend_yz = extend_mode;
}
...
int GPU_texture_dimensions(const GPUTexture *tex_) {
	TextureType type = reinterpret_cast<const Texture *>(tex_)->type_get();
...
TextureFormat GPU_texture_format(const GPUTexture *tex) {
...
bool GPU_texture_has_normalized_format(const GPUTexture *tex) {
	return (reinterpret_cast<const Texture *>(tex)->format_flag_get() & GPU_FORMAT_NORMALIZED_INTEGER) != 0;
}
...
GPUPixelBuffer *GPU_pixel_buffer_create(size_t size) {
	...
	size = ceil_to_multiple_u64(size, 256);
...
size_t GPU_texture_dataformat_size(DataFormat data_format) {
