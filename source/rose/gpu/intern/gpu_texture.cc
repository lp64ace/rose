...
bool Texture::init_2D(int w, int h, int layers, int mip_len, TextureFormat format) {
	...
	h_ = h;
	d_ = layers;
	int mip_len_max = 1 + floorf(log2f(ROSE_MAX(w, h)));
	mipmaps_ = ROSE_MIN(mip_len, mip_len_max);
	format_ = format;
	format_flag_ = to_format_flag(format);
...
bool Texture::init_3D(int w, int h, int d, int mip_len, TextureFormat format) {
	...
	h_ = h;
	d_ = d;
	int mip_len_max = 1 + floorf(log2f(ROSE_MAX(w, h, d)));
	mipmaps_ = ROSE_MIN(mip_len, mip_len_max);
	format_ = format;
	format_flag_ = to_format_flag(format);
...
GPUTexture *GPU_texture_create_1d(
	const char *name, int w, int mip_len, TextureFormat format, TextureUsage usage, const float *data) {
	return gpu_texture_create(name, w, 0, 0, GPU_TEXTURE_1D, mip_len, format, GPU_DATA_FLOAT, usage, data);
}

GPUTexture *GPU_texture_create_1d_array(
	const char *name, int w, int h, int mip_len, TextureFormat format, TextureUsage usage, const float *data) {
...
GPUTexture *GPU_texture_create_2d_array(
	const char *name, int w, int h, int d, int mip_len, TextureFormat format, TextureUsage usage, const float *data) {
	return gpu_texture_create(name, w, h, d, GPU_TEXTURE_2D_ARRAY, mip_len, format, GPU_DATA_FLOAT, usage, data);
}

GPUTexture *GPU_texture_create_3d(
	const char *name, int w, int h, int d, int mip_len, TextureFormat texture_format, TextureUsage usage, const void *data) {
...
GPUTexture *GPU_texture_create_cube_array(
	const char *name, int w, int d, int mip_len, TextureFormat format, TextureUsage usage, const float *data) {
...
GPUTexture *GPU_texture_create_compressed_2d(
	const char *name, int w, int h, int miplen, TextureFormat tex_format, TextureUsage usage, const void *data) {
	Texture *tex = GPUBackend::get()->texture_alloc(name);
	tex->usage_set(usage);
...
GPUTexture *GPU_texture_create_view(const char *name,
	...
	view->init_view(
		src, format, unwrap(src)->type_get(), mip_start, mip_len, layer_start, layer_len, cube_as_array, use_stencil);
	return wrap(view);
}
