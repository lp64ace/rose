 ...
 */
void GPU_select_begin(struct GPUSelectResult *buffer, unsigned int buffer_len, const struct rcti *input, enum SelectMode mode, int oldhits);
/**
 ...
 *
 * Note that comparing depth as unsigned int is fine.
 */
const struct GPUSelectResult *GPU_select_buffer_near(const struct GPUSelectResult *buffer, int hits);
unsigned int GPU_select_buffer_remove_by_id(struct GPUSelectResult *buffer, int hits, unsigned int select_id);
/**
 * Part of the solution copied from `rect_subregion_stride_calc`.
 */
void GPU_select_buffer_stride_realign(const struct rcti *src, const struct rcti *dst, unsigned int *r_buf);
