...
ENUM_OPERATORS(SamplerFiltering, GPU_SAMPLER_FILTERING_ANISOTROPIC)

/** The number of every possible filtering configuration. */
static const int GPU_SAMPLER_FILTERING_TYPES_COUNT = 1 << 3;

typedef enum SamplerExtendMode {
	/**
...
typedef enum SamplerCustomType {
	/**
...
/**
 * The `SamplerStateType` specifies how the GPUSamplerState structure should be interpreted
...
typedef struct GPUSamplerState {
	 ...
	 */
	void enable_filtering_flag(SamplerFiltering filtering_flags) {
	 ...
	 */
	void disable_filtering_flag(SamplerFiltering filtering_flags) {
		this->filtering = this->filtering & ~filtering_flags;
	}
	 ...
	 * filtering flags.
	 */
	void set_filtering_flag_from_test(SamplerFiltering filtering_flags, bool test) {
		if (test) {
	...
	std::string to_string() const {
		...
		ROSE_assert(this->type == GPU_SAMPLER_STATE_TYPE_PARAMETERS);
...
typedef enum TextureUsage {
	...
	/* Create a texture whose usage cannot be defined prematurely. This is unoptimized and should not be used. */
	GPU_TEXTURE_USAGE_GENERAL = (0xFF & (~(GPU_TEXTURE_USAGE_MEMORYLESS | GPU_TEXTURE_USAGE_ATOMIC))),
} TextureUsage;

ENUM_OPERATORS(TextureUsage, GPU_TEXTURE_USAGE_GENERAL);
...
/**
 * Texture data is expected to be float. If the \a format is not compatible with float data or the data is not in float format,
 * use GPU_texture_update to upload the data with the right data format.
 */
GPUTexture *GPU_texture_create_1d(const char *name, int w, int mip, TextureFormat, TextureUsage, const float *);
GPUTexture *GPU_texture_create_2d(const char *name, int w, int h, int mip, TextureFormat, TextureUsage, const float *);
GPUTexture *GPU_texture_create_3d(const char *name, int w, int h, int d, int mip, TextureFormat, TextureUsage, const float *);

GPUTexture *GPU_texture_create_1d_array(const char *name, int w, int layers, int mip, TextureFormat, TextureUsage, const float *);
GPUTexture *GPU_texture_create_2d_array(const char *name, int w, int h, int layers, int mip, TextureFormat, TextureUsage, const float *);
GPUTexture *GPU_texture_create_3d_array(const char *name, int w, int h, int d, int layers, int mip, TextureFormat, TextureUsage, const float *);

GPUTexture *GPU_texture_create_cube(const char *name, int w, int mip, TextureFormat, TextureUsage, const float *);

GPUTexture *GPU_texture_create_cube_array(const char *name, int w, int layers, int mip, TextureFormat, TextureUsage, const float *);

/* clang-format on */

/**
 * Create a buffer texture that allow access to a buffer through a sampler of type `FLOAT/INT/UINT_BUFFER`.
 */
GPUTexture *GPU_texture_create_from_vertbuf(const char *name, struct GPUVertBuf *vertex_buffer);

/**
 * Create an error texture that will bind a pink texture at draw time.
 */
...
/**
 * Add a reference to this texture for usage, this will increment the reference counter.
 * This avoids the texture being freed between the time it is referenced by the drawing logic and the time it is actually
 * derefernced.
 */
void GPU_texture_ref(GPUTexture *texture);

/**
 * This internally decrements the reference counter.
 * If the reference counter is 1 when calling this function the texture will be freed.
 */
 ...
 */
void GPU_texture_update(GPUTexture *texture, DataFormat format, const void *data);

/**
 * Update the content of a region of a texture's base mip-map level (mip 0).
 * \a format is the format of the \a data . It needs to be compatible with the internal texture storage.
 ...
 * \a format is the format of the \a pixels . It needs to be compatible with the internal
 * texture storage.
 * The \a data should be the size of the entire \a mip_level.
 */
void GPU_texture_update_mipmap(GPUTexture *texture, int mip_level, DataFormat format, const void *pixels);

/**
 * Fills the whole texture with the same data for all pixels.
 * \warning Only works for 2D and 3D textures.
 * \warning Only clears the MIP 0 of the texture.
 * \param format: data format of the pixel data.
 * \note The format is float for UNORM textures.
 * \param data: 1 pixel worth of data to fill the texture with.
 */
void GPU_texture_clear(GPUTexture *texture, DataFormat format, const void *data);
 ...
 */
void *GPU_texture_read(GPUTexture *texture, DataFormat format, int mip_level);
 ...
 */
void GPU_texture_extend_mode_x(GPUTexture *texture, SamplerExtendMode extend_mode);

/**
 * Set \a tex texture sampling method for coordinates outside of the [0..1] uv range along the y
 * axis. See GPUSamplerExtendMode for the available and meaning of different extend modes.
 */
void GPU_texture_extend_mode_y(GPUTexture *texture, SamplerExtendMode extend_mode);
 ...
 */
void GPU_texture_extend_mode(GPUTexture *texture, SamplerExtendMode extend_mode);
 ...
 */
TextureFormat GPU_texture_format(const GPUTexture *texture);

/**
 * Return the usage flags of \a tex.
 */
TextureUsage GPU_texture_usage(const GPUTexture *texture);
 ...
 */
size_t GPU_texture_component_len(TextureFormat format);
 ...
 */
const char *GPU_texture_format_name(TextureFormat format);
 ...
 */
size_t GPU_texture_memory_usage_get(void);
...
/** Opaque type hiding rose::gpu::PixelBuffer. */
