...
#	define EXPECT_OP(OP, val1, val2) \
		out_test[g_test_id++] = test_output(as_raw_data(val1), as_raw_data(val2), bool(OP), int(__LINE__), to_type(val1))
#else
...
#	define EXPECT_OP(OP, val1, val2) \
		...
		if (int(gl_FragCoord.y) == g_test_id - 1) { \
			TestOutput to = test_output(as_raw_data(val1), as_raw_data(val2), bool(OP), int(__LINE__), to_type(val1)); \
			switch (int(gl_FragCoord.x)) { \
				case 0: \
					out_test = uvec4(to.expect.data[0], to.expect.data[1], to.expect.data[2], to.expect.data[3]); \
					break; \
				case 1: \
					out_test = uvec4(to.expect.data[4], to.expect.data[5], to.expect.data[6], to.expect.data[7]); \
					break; \
				case 2: \
					out_test = uvec4(to.expect.data[8], to.expect.data[9], to.expect.data[10], to.expect.data[11]); \
					break; \
				case 3: \
					out_test = uvec4(to.expect.data[12], to.expect.data[13], to.expect.data[14], to.expect.data[15]); \
					break; \
				case 4: \
					out_test = uvec4(to.result.data[0], to.result.data[1], to.result.data[2], to.result.data[3]); \
					break; \
				case 5: \
					out_test = uvec4(to.result.data[4], to.result.data[5], to.result.data[6], to.result.data[7]); \
					break; \
				case 6: \
					out_test = uvec4(to.result.data[8], to.result.data[9], to.result.data[10], to.result.data[11]); \
					break; \
				case 7: \
					out_test = uvec4(to.result.data[12], to.result.data[13], to.result.data[14], to.result.data[15]); \
					break; \
				case 8: \
...
#define EXPECT_NEAR(result, expect, threshold) EXPECT_OP(is_equal(result, expect, threshold), result, expect)
