...
#pragma ROSE_REQUIRE(gpu_shader_math_vector_lib.glsl)
#pragma ROSE_REQUIRE(gpu_shader_math_rotation_lib.glsl)
...
mat4x4 mat4x4_diagonal(float v) {
	return mat4x4(vec4(v, 0.0, 0.0, 0.0), vec4(0.0, v, 0.0, 0.0), vec4(0.0, 0.0, v, 0.0), vec4(0.0, 0.0, 0.0, v));
}
...
void to_rot_scale(mat3x3 mat, out EulerXYZ r_rotation, out vec3 r_scale, const bool allow_negative_scale);
void to_rot_scale(mat3x3 mat, out Quaternion r_rotation, out vec3 r_scale, const bool allow_negative_scale);
void to_rot_scale(mat3x3 mat, out AxisAngle r_rotation, out vec3 r_scale, const bool allow_negative_scale);
void to_loc_rot_scale(
	mat4x4 mat, out vec3 r_location, out EulerXYZ r_rotation, out vec3 r_scale, const bool allow_negative_scale);
void to_loc_rot_scale(
	mat4x4 mat, out vec3 r_location, out Quaternion r_rotation, out vec3 r_scale, const bool allow_negative_scale);
void to_loc_rot_scale(
	mat4x4 mat, out vec3 r_location, out AxisAngle r_rotation, out vec3 r_scale, const bool allow_negative_scale);
 ...
 */
mat4x4 projection_orthographic(float left, float right, float bottom, float top, float near_clip, float far_clip);
 ...
 */
mat4x4 projection_perspective(float left, float right, float bottom, float top, float near_clip, float far_clip);
 ...
 * The resulting matrix can be used with #project_point.
 */
mat4x4 projection_perspective_fov(
	float angle_left, float angle_right, float angle_bottom, float angle_top, float near_clip, float far_clip);
