...
#include "intern/gpu_vertex_format_private.h"
...
unsigned char *GLImmediate::begin() {
	...
	else if (bytes_needed < DEFAULT_INTERNAL_BUFFER_SIZE && buffer_size() > DEFAULT_INTERNAL_BUFFER_SIZE) {
...
#ifndef NDEBUG
	{
		...
		ROSE_assert(buffer_offset() + bytes_needed <= bufsize);
	}
#endif
...
void GLImmediate::end() {
	ROSE_assert(prim_type != GPU_PRIM_NONE); /* make sure we're between a Begin/End pair */
	...
	if (vertex_len > 0) {
		...
		GLVertArray::update_bindings(vao_id_, v_first, &vertex_format, reinterpret_cast<Shader *>(shader)->interface);
