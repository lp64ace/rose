...
void GLStateManager::set_state(const GPUState &state) {
	...
	if (changed.blend != 0) {
		set_blend((Blend)state.blend);
	}
	if (changed.write_mask != 0) {
		set_write_mask((WriteMask)state.write_mask);
	}
	if (changed.depth_test != 0) {
		set_depth_test((DepthTest)state.depth_test);
	}
	if (changed.stencil_test != 0 || changed.stencil_op != 0) {
		set_stencil_test((StencilTest)state.stencil_test, (StencilOp)state.stencil_op);
		set_stencil_mask((StencilTest)state.stencil_test, mutable_state);
	}
	if (changed.clip_distances != 0) {
		set_clip_distances(state.clip_distances, current_.clip_distances);
	}
	if (changed.culling_test != 0) {
		set_backface_culling((FaceCullTest)state.culling_test);
	}
	...
	if (changed.provoking_vert != 0) {
		set_provoking_vert((ProvokingVertex)state.provoking_vert);
	}
...
void GLStateManager::set_mutable_state(const GPUStateMutable &state) {
	...
	if (float_as_int(changed.point_size) != 0) {
	...
	if (float_as_int(changed.line_width) != 0) {
		/* TODO: remove, should use wide line shader. */
		glLineWidth(clampf(state.line_width, line_width_range_[0], line_width_range_[1]));
	}

	if (float_as_int(changed.depth_range[0]) != 0 || float_as_int(changed.depth_range[1]) != 0) {
		/* TODO: remove, should modify the projection matrix instead. */
		glDepthRange(UNPACK2(state.depth_range));
	}

	if (changed.stencil_compare_mask != 0 || changed.stencil_reference != 0 || changed.stencil_write_mask != 0) {
		set_stencil_mask((StencilTest)current_.stencil_test, state);
	}
...
void GLStateManager::set_depth_test(const DepthTest value) {
	GLenum func;
	switch (value) {
		case GPU_DEPTH_LESS:
			func = GL_LESS;
...
void GLStateManager::set_stencil_test(const StencilTest test, const StencilOp operation) {
	switch (operation) {
...
void GLStateManager::set_stencil_mask(const StencilTest test, const GPUStateMutable state) {
	GLenum func;
	switch (test) {
...
void GLStateManager::set_backface_culling(const FaceCullTest test) {
...
void GLStateManager::set_provoking_vert(const ProvokingVertex vert) {
	GLenum value = (vert == GPU_VERTEX_FIRST) ? GL_FIRST_VERTEX_CONVENTION : GL_LAST_VERTEX_CONVENTION;
...
void GLStateManager::set_blend(const Blend value) {
	/**
...
void GLStateManager::texture_bind(Texture *tex_, GPUSamplerState sampler_state, int unit) {
	ROSE_assert(unit < GPU_get_info_i(GPU_INFO_MAX_TEXTURES));
	GLTexture *tex = static_cast<GLTexture *>(tex_);
	/* Eliminate redundant binds. */
	if ((textures_[unit] == tex->tex_id_) && (samplers_[unit] == GLTexture::get_sampler(sampler_state))) {
		return;
	}
...
void GLStateManager::image_bind(Texture *tex_, int unit) {
	...
	ROSE_assert(GPU_get_info_i(GPU_INFO_SHADER_IMAGE_LOAD_STORE_SUPPORT) && unit < 8);
	GLTexture *tex = static_cast<GLTexture *>(tex_);
...
void GLStateManager::issue_barrier(Barrier barrier_bits) {
