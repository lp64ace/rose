...
GLShaderInterface::GLShaderInterface(GLuint program) {
	...
	ROSE_assert_msg(ubo_len <= 16, "enabled_ubo_mask_ is uint16_t");
	...
	LIB_bitmap *uniforms_from_blocks = ROSE_BITMAP_NEW(active_uniform_len, __func__);
	...
	for (int i = 0; i < ubo_len; i++) {
		...
		for (int u = 0; u < ubo_uni_len; u++) {
			ROSE_BITMAP_ENABLE(uniforms_from_blocks, ubo_uni_ids[u]);
		}
	}
	...
	for (int i = 0, sampler = 0, image = 0; i < active_uniform_len; i++) {
		if (ROSE_BITMAP_TEST(uniforms_from_blocks, i)) {
			continue;
		}
		char *name = name_buffer_ + name_buffer_offset;
		GLsizei remaining_buffer = name_buffer_len - name_buffer_offset;
	...
	for (int i = 0; i < ssbo_len; i++) {
		...
		GLsizei remaining_buffer = name_buffer_len - name_buffer_offset;
		GLsizei name_len = 0;
		glGetProgramResourceName(program, GL_SHADER_STORAGE_BLOCK, i, remaining_buffer, &name_len, name);
...
GLShaderInterface::GLShaderInterface(GLuint program, const shader::ShaderCreateInfo &info) {
	...
	ROSE_assert_msg(ubo_len_ <= 16, "enabled_ubo_mask_ is uint16_t");

	int input_tot_len = attr_len_ + ubo_len_ + uniform_len_ + ssbo_len_;
	inputs_ = (ShaderInput *)MEM_callocN(sizeof(ShaderInput) * input_tot_len, __func__);
	ShaderInput *input = inputs_;

	name_buffer_ = (char *)MEM_mallocN(info.interface_names_size_ + workaround_names_size, "name_buffer");
	uint32_t name_buffer_offset = 0;
