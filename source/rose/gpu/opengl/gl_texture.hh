...
public:
	...
	void update_sub(int mip, int offset[3], int extent[3], DataFormat type, const void *data) override;
	void update_sub(int offset[3], int extent[3], DataFormat format, GPUPixelBuffer *pixbuf) override;
	...
	void clear(DataFormat format, const void *data) override;
	void swizzle_set(const char swizzle_mask[4]) override;
	void mip_range_set(int min, int max) override;
	void *read(int mip, DataFormat type) override;
...
inline GLenum to_gl_internal_format(TextureFormat format) {
	...
	ROSE_assert_msg(0, "Texture format incorrect or unsupported");
	return 0;
}

inline GLenum to_gl_target(TextureType type) {
	switch (type) {
		case GPU_TEXTURE_1D:
...
inline GLenum to_gl_proxy(TextureType type) {
	switch (type) {
		case GPU_TEXTURE_1D:
...
inline GLenum to_gl(DataFormat format) {
	switch (format) {
		case GPU_DATA_FLOAT:
			return GL_FLOAT;
		case GPU_DATA_INT:
