...
void GLIndexBuf::bind_as_ssbo(uint binding) {
	if (ibo_id_ == 0 || data_ != nullptr) {
		/**
		 * Calling `glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, ibo_id_)` changes the index buffer of the currently bound VAO.
		 *
		 * In the OpenGL backend, the VAO state persists even after `GLVertArray::update_bindings` is called.
		 *
