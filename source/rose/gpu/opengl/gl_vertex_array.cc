...
static uint16_t vbo_bind(const ShaderInterface *interface, const GPUVertFormat *format, uint v_first, uint v_len, const bool use_instancing) {
	...
	for (uint a_idx = 0; a_idx < attr_len; a_idx++) {
		...
		const GLenum type = to_gl(static_cast<VertCompType>(a->comp_type));
		...
		for (uint n_idx = 0; n_idx < a->name_len; n_idx++) {
			...
			if (ELEM(a->comp_len, 16, 12, 8)) {
				ROSE_assert(a->fetch_mode == GPU_FETCH_FLOAT);
				ROSE_assert(a->comp_type == GPU_COMP_F32);
				for (int i = 0; i < a->comp_len / 4; i++) {
					glEnableVertexAttribArray(input->location + i);
					glVertexAttribDivisor(input->location + i, divisor);
					glVertexAttribPointer(input->location + i, 4, type, GL_FALSE, stride, (const GLubyte *)pointer + i * 16);
				}
			}
			else {
...
void GLVertArray::update_bindings(const GLuint vao,
	...
	for (int v = GPU_BATCH_VBO_MAX_LEN - 1; v > -1; v--) {
		...
		if (vbo) {
			vbo->bind();
			attr_mask &= ~vbo_bind(interface, &vbo->format, 0, vbo->vertex_length, false);
		}
	}
	...
	for (int v = GPU_BATCH_INST_VBO_MAX_LEN - 1; v > -1; v--) {
		...
		if (vbo) {
			vbo->bind();
			attr_mask &= ~vbo_bind(interface, &vbo->format, base_instance, vbo->vertex_length, true);
		}
	}
	...
	if (batch->resource_id_buf) {
		...
		if (input) {
			...
			glVertexAttribIPointer(input->location, component_len, to_gl(GPU_COMP_I32), 0, (GLvoid *)nullptr);
