...
void GLVertBuf::acquire_data() {
	...
	MEM_SAFE_FREE(data);
	data = (unsigned char *)MEM_mallocN(sizeof(unsigned char) * this->size_alloc_get(), __func__);
}
...
void GLVertBuf::resize_data() {
	...
	data = (unsigned char *)MEM_reallocN(data, sizeof(unsigned char) * this->size_alloc_get());
}
...
void GLVertBuf::duplicate_data(VertBuf *dst_) {
	...
	if (data != nullptr) {
		dst->data = (unsigned char *)MEM_dupallocN(src->data);
	}
}
...
void GLVertBuf::bind_as_texture(uint binding) {
	...
	if (buffer_texture_ == nullptr) {
		buffer_texture_ = GPU_texture_create_from_vertbuf("VertBuf as Texture", wrap(this));
	}
	GPU_texture_bind(buffer_texture_, binding);
}

void GLVertBuf::read(void *data) const {
	ROSE_assert(is_active());
...
void GLVertBuf::wrap_handle(uint64_t handle) {
	ROSE_assert(vbo_id_ == 0);
	ROSE_assert(glIsBuffer(static_cast<unsigned int>(handle)));
	is_wrapper_ = true;
	vbo_id_ = static_cast<unsigned int>(handle);
