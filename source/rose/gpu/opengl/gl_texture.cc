...
#include "gl_debug.hh"
#include "gl_state.hh"
#include "intern/gpu_vertex_buffer_private.hh" /* TODO: should be `gl_vertex_buffer.hh`. */
...
bool GLTexture::init_internal() {
	if ((format_ == GPU_DEPTH24_STENCIL8) && GPU_get_info_i(GPU_INFO_DEPTH_BLITTING_WORKAROUND)) {
	...
	auto mip_size = [&](int h, int w = 1, int d = 1) -> size_t { return divide_ceil_u(w, 4) * divide_ceil_u(h, 4) * divide_ceil_u(d, 4) * to_block_size(format_); };
	switch (dimensions) {
		default:
		case 1:
		...
		case 2:
			if (GLContext::texture_storage_support) {
				...
				for (int i = 0, w = w_, h = h_; i < mipmaps_; i++) {
					...
					w = ROSE_MAX(1, (w / 2));
					h = is_layered ? h_ : ROSE_MAX(1, (h / 2));
				}
			}
			break;
		case 3:
			if (GLContext::texture_storage_support) {
				...
				for (int i = 0, w = w_, h = h_, d = d_; i < mipmaps_; i++) {
					if (is_compressed) {
						glCompressedTexImage3D(target_, i, internal_format, w, h, d, 0, mip_size(w, h, d), nullptr);
					}
					else {
					...
					w = ROSE_MAX(1, (w / 2));
					h = ROSE_MAX(1, (h / 2));
					d = is_layered ? d_ : ROSE_MAX(1, (d / 2));
				}
			}
			break;
	}
...
bool GLTexture::init_internal(GPUTexture *src, int mip_offset, int layer_offset, bool use_stencil) {
	ROSE_assert(GLContext::texture_storage_support);

	const GLTexture *gl_src = static_cast<const GLTexture *>(unwrap(src));
	GLenum internal_format = to_gl_internal_format(format_);
	target_ = to_gl_target(type_);

	glTextureView(tex_id_, target_, gl_src->tex_id_, internal_format, mip_offset, mipmaps_, layer_offset, this->layer_count());
	...
	if (format_flag_ & GPU_FORMAT_COMPRESSED) {
		...
		switch (this->dimensions_count()) {
			...
			case 1:
				...
				break;
			case 2:
				glCompressedTextureSubImage2D(tex_id_, mip, UNPACK2(offset), UNPACK2(extent), format, size, data);
				break;
			case 3:
				glCompressedTextureSubImage3D(tex_id_, mip, UNPACK3(offset), UNPACK3(extent), format, size, data);
				break;
		}
	}
	else {
...
void GLTexture::update_sub(int mip, int offset[3], int extent[3], DataFormat type, const void *data) {
	ROSE_assert(validate_data_format(format_, type));
	...
	if (type_ == GPU_TEXTURE_CUBE) {
		...
		switch (dimensions) {
			...
			case 1:
				...
				break;
			case 2:
				glCompressedTexSubImage2D(target_, mip, UNPACK2(offset), UNPACK2(extent), gl_format, size, data);
				break;
			case 3:
				glCompressedTexSubImage3D(target_, mip, UNPACK3(offset), UNPACK3(extent), gl_format, size, data);
				break;
		}
	}
	else {
...
void GLTexture::update_sub(int offset[3], int extent[3], DataFormat format, GPUPixelBuffer *pixbuf) {
	/* Update texture from pixel buffer. */
	ROSE_assert(validate_data_format(format_, format));
	ROSE_assert(pixbuf != nullptr);
...
void GLTexture::clear(DataFormat data_format, const void *data) {
	ROSE_assert(validate_data_format(format_, data_format));
...
void GLTexture::copy_to(Texture *dst_) {
	...
	ROSE_assert((dst->w_ == src->w_) && (dst->h_ == src->h_) && (dst->d_ == src->d_));
	ROSE_assert(dst->format_ == src->format_);
	ROSE_assert(dst->type_ == src->type_);

	int mip = 0;
	/* NOTE: mip_size_get() won't override any dimension that is equal to 0. */
	int extent[3] = {1, 1, 1};
	this->mip_size_get(mip, extent);
	glCopyImageSubData(src->tex_id_, target_, mip, 0, 0, 0, dst->tex_id_, target_, mip, 0, 0, 0, UNPACK3(extent));

	has_pixels_ = true;
}

void *GLTexture::read(int mip, DataFormat type) {
	ROSE_assert(!(format_flag_ & GPU_FORMAT_COMPRESSED));
	ROSE_assert(mip <= mipmaps_ || mip == 0);
	ROSE_assert(validate_data_format(format_, type));
...
void GLTexture::stencil_texture_mode_set(bool use_stencil) {
	ROSE_assert(GLContext::stencil_texturing_support);
...
void GLTexture::mip_range_set(int min, int max) {
	ROSE_assert(min <= max && min >= 0 && max <= mipmaps_);
	mip_min_ = min;
	mip_max_ = max;
	if (GLContext::direct_state_access_support) {
...
/** A function that maps SamplerExtendMode values to their OpenGL enum counterparts. */
static inline GLenum to_gl(SamplerExtendMode extend_mode) {
	switch (extend_mode) {
...
void GLTexture::samplers_init() {
	...
	for (int extend_yz_i = 0; extend_yz_i < GPU_SAMPLER_EXTEND_MODES_COUNT; extend_yz_i++) {
		const SamplerExtendMode extend_yz = static_cast<SamplerExtendMode>(extend_yz_i);
		const GLenum extend_t = to_gl(extend_yz);

		for (int extend_x_i = 0; extend_x_i < GPU_SAMPLER_EXTEND_MODES_COUNT; extend_x_i++) {
			const SamplerExtendMode extend_x = static_cast<SamplerExtendMode>(extend_x_i);
...
void GLTexture::samplers_update() {
	...
	const float anisotropic_filter = max_anisotropy;
...
GLuint GLTexture::get_sampler(const GPUSamplerState &sampler_state) {
	...
	ROSE_assert(sampler_state.type != GPU_SAMPLER_STATE_TYPE_INTERNAL);

	if (sampler_state.type == GPU_SAMPLER_STATE_TYPE_CUSTOM) {
		return custom_samplers_state_cache_[sampler_state.custom_type];
	}

	return samplers_state_cache_[sampler_state.extend_yz][sampler_state.extend_x][sampler_state.filtering];
}
...
bool GLTexture::proxy_check(int mip) {
	...
	int max_3d_size = GPU_get_info_i(GPU_INFO_MAX_TEXTURE_3D_SIZE);
	int max_cube_size = GLContext::max_cubemap_size;
	int size[3] = {1, 1, 1};
	this->mip_size_get(mip, size);

	if (type_ & GPU_TEXTURE_ARRAY) {
		if (this->layer_count() > GPU_get_info_i(GPU_INFO_MAX_TEXTURE_LAYERS)) {
			return false;
		}
	}
