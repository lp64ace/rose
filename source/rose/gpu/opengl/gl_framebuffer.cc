...
GLFrameBuffer::GLFrameBuffer(const char *name, GLContext *ctx, GLenum target, GLuint fbo, int w, int h) : FrameBuffer(name) {
	context_ = ctx;
...
GLFrameBuffer::~GLFrameBuffer() {
	...
	if (context_->active_fb == this && context_->back_left != this) {
		 ...
		 */
		ROSE_assert(context_ == Context::get());
...
#undef FORMAT_STATUS
	...
	if (err_out) {
		LIB_strnformat(err_out, 256, "[FrameBuffer] %s status %s\n", this->name_, err);
	}
	else {
		fprintf(stderr, "[FrameBuffer] %s status %s\n", this->name_, err);
	}
...
void GLFrameBuffer::update_attachments() {
	 ...
	 */
	AttachmentType first_attachment = GPU_FB_MAX_ATTACHMENT;
	/* NOTE: Inverse iteration to get the first color texture. */
	for (AttachmentType type = GPU_FB_MAX_ATTACHMENT - 1; type >= 0; --type) {
		...
		if (attach.layer > -1 && GPU_texture_is_cube(attach.tex) && !GPU_texture_is_array(attach.tex)) {
			/**
			 * Could be avoided if ARB_direct_state_access is required.
			 * In this case #glFramebufferTextureLayer would bind the correct face.
			 */
		...
		/** We found one depth buffer type. Stop here, otherwise we would override it by setting GPU_FB_DEPTH_ATTACHMENT */
	...
	if (GLContext::unused_fb_slot_workaround) {
		...
		for (int i = ARRAY_SIZE(gl_attachments_) - 1; i >= 0; --i) {
			AttachmentType type = GPU_FB_COLOR_ATTACHMENT0 + i;
			GPUAttachment &attach = attachments_[type];
			if (attach.tex != nullptr) {
...
void GLFrameBuffer::subpass_transition(const AttachmentState depth_attachment_state,
	 ...
	 */
	ROSE_assert(depth_attachment_state != GPU_ATTACHEMENT_READ);
	...
	if (GLContext::texture_barrier_support) {
		if (any_read) {
			glTextureBarrier();
		}
		...
		/**
		 * We have to use `glDrawBuffers` instead of `glColorMaski` because the later is overwritten by the `GLStateManager`. */
		 ...
		 */
		/* TODO(@fclem): Could avoid the frame-buffer reconfiguration by creating multiple frame-buffers internally.  */
		for (int i : color_attachment_states.index_range()) {
			AttachmentType type = GPU_FB_COLOR_ATTACHMENT0 + i;
...
void GLFrameBuffer::attachment_set_loadstore_op(AttachmentType type, GPULoadStore ls) {
	ROSE_assert(context_->active_fb == this);

	/* TODO(fclem): Add support for other ops. */
	if (ls.load_action == LoadOp::GPU_LOADACTION_CLEAR) {
		if (tmp_detached_[type].tex != nullptr) {
			/**
...
void GLFrameBuffer::apply_state() {
	...
	if (multi_viewport_ == false) {
		...
		for (int i = 0; i < GPU_MAX_VIEWPORTS; i++) {
			for (int j = 0; j < 4; j++) {
				viewports_f[i][j] = static_cast<int>(viewport_[i][j]);
			}
		}
...
void GLFrameBuffer::bind(bool enabled_srgb) {
	...
	if (context_ != GLContext::get()) {
		ROSE_assert_msg(0, "Trying to use the same frame-buffer in multiple context");
		return;
	}
...
void GLFrameBuffer::clear(FrameBufferBits buffers, const float clear_col[4], float clear_depth, uint clear_stencil) {
	ROSE_assert(GLContext::get() == context_);
	ROSE_assert(context_->active_fb == this);

	/* Save and restore the state. */
	WriteMask write_mask = GPU_write_mask_get();
	uint stencil_mask = GPU_stencil_mask_get();
	StencilTest stencil_test = GPU_stencil_test_get();
...
void GLFrameBuffer::clear_attachment(AttachmentType type, DataFormat data_format, const void *clear_value) {
	ROSE_assert(GLContext::get() == context_);
	ROSE_assert(context_->active_fb == this);
	...
	if (type == GPU_FB_DEPTH_STENCIL_ATTACHMENT) {
		ROSE_assert(data_format == GPU_DATA_UINT_24_8);
		...
		if (data_format == GPU_DATA_FLOAT) {
			...
			ROSE_assert_msg(0, "Unhandled data format");
		}
	}
	else {
...
void GLFrameBuffer::clear_multi(const float (*clear_cols)[4]) {
	/**
	 * WATCH: This can easily access clear_cols out of bounds it clear_cols is not big enough for all attachments.
...
void GLFrameBuffer::blit_to(FrameBufferBits planes, int src_slot, FrameBuffer *dst_, int dst_slot, int x, int y) {
	...
	if (planes & GPU_COLOR_BIT) {
		ROSE_assert(src->immutable_ == false || src_slot == 0);
		ROSE_assert(dst->immutable_ == false || dst_slot == 0);
		ROSE_assert(src->gl_attachments_[src_slot] != GL_NONE);
		ROSE_assert(dst->gl_attachments_[dst_slot] != GL_NONE);
