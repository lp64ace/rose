...
#include "intern/gpu_batch_private.hh"
...
void GLVaoCache::remove(const GLShaderInterface *interface) {
	...
	const GLShaderInterface **interfaces = (is_dynamic_vao_count) ? dynamic_vaos.interfaces : static_vaos.interfaces;
...
void GLBatch::draw(int v_first, int v_count, int i_first, int i_count) {
	...
	ROSE_assert(v_count > 0 && i_count > 0);
...
void GLBatch::multi_draw_indirect(GPUStorageBuf *indirect_buf, int count, intptr_t offset, intptr_t stride) {
	this->bind(0);
