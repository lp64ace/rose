...
void GPU_vertbuf_data_len_set(GPUVertBuf *buffer, unsigned int vertex_length);
...
ROSE_INLINE void *GPU_vertbuf_raw_step(GPUVertBufRaw *attr) {
	unsigned char *data = attr->data;
	attr->data += attr->stride;
	return (void *)data;
}

ROSE_INLINE unsigned int GPU_vertbuf_raw_used(GPUVertBufRaw *attr) {
...
void *GPU_vertbuf_steal_data(GPUVertBuf *buffer);
