...
typedef enum VertCompType { GPU_COMP_I8 = 0, GPU_COMP_U8, GPU_COMP_I16, GPU_COMP_U16, GPU_COMP_I32, GPU_COMP_U32, GPU_COMP_F32, GPU_COMP_I10, GPU_COMP_MAX } VertCompType;
...
unsigned int GPU_vertformat_add(GPUVertFormat *, const char *name, VertCompType, unsigned int comp_len, VertFetchMode);
...
ROSE_INLINE const char *GPU_vertformat_attr_name_get(const GPUVertFormat *format, const GPUVertAttr *attr, unsigned int n_idx) {
...
void GPU_vertformat_attr_rename(GPUVertFormat *format, unsigned int attr, const char *new_name);
...
ROSE_INLINE void GPU_normal_convert_v3(GPUNormal *normal, const float data[3], const bool hq) {
	if (hq) {
		normal_float_to_short_v3(normal->high, data);
	}
	else {
