...
ROSE_INLINE bool translate_number_from_integer_literal(RCCToken *t) {
	...
	if (suffix) {
		if (RCC_type_is_unsigned(t->type)) {
			unsigned long long *u = (unsigned long long *)t->payload;

			*u = (unsigned long long)strtoull(s, &p, base);
			return true;
		}
		else {
			signed long long *u = (signed long long *)t->payload;

			*u = (signed long long)strtoll(s, &p, base);
			return true;
		}
	}
	return false;
...
ROSE_INLINE RCCToken *token_new(RCContext *C, int kind, const RCCFile *f, const RCCSLoc *loc, size_t length) {
	 ...
	 * this way we can do comparisons with the full extend of the payload in `RCC_token_match`.
	 */
	RCCToken *t = RCC_context_calloc(C, sizeof(RCCToken));
	...
	if (loc) {
		...
		RCC_sloc_copy(&t->location, loc);
	}
	return t;
}

RCCToken *RCC_token_new_identifier(RCContext *C, const RCCFile *f, const RCCSLoc *loc, size_t length) {
	return token_new(C, TOK_IDENTIFIER, f, loc, length);
}
RCCToken *RCC_token_new_keyword(RCContext *C, const RCCFile *f, const RCCSLoc *loc, size_t length) {
	return token_new(C, TOK_KEYWORD, f, loc, length);
}
RCCToken *RCC_token_new_punctuator(RCContext *C, const RCCFile *f, const RCCSLoc *loc, size_t length) {
	return token_new(C, TOK_PUNCTUATOR, f, loc, length);
}

RCCToken *RCC_token_new_number(RCContext *C, const RCCFile *f, const RCCSLoc *loc, size_t length) {
...
RCCToken *RCC_token_new_string(RCContext *C, const RCCFile *f, const RCCSLoc *loc, size_t length) {
...
struct RCCToken *RCC_token_new_eof(RCContext *C) {
...
bool RCC_token_match(const RCCToken *a, const RCCToken *b) {
	if (a == b) {
...
bool RCC_token_is_identifier(const RCCToken *tok) {
	return tok->kind == TOK_IDENTIFIER;
}
bool RCC_token_is_keyword(const RCCToken *tok) {
	return tok->kind == TOK_KEYWORD;
}
bool RCC_token_is_punctuator(const RCCToken *tok) {
	return tok->kind == TOK_PUNCTUATOR;
}
bool RCC_token_is_number(const RCCToken *tok) {
	return tok->kind == TOK_NUMBER;
}
bool RCC_token_is_string(const RCCToken *tok) {
	return tok->kind == TOK_STRING;
}

const char *RCC_token_as_string(const RCCToken *tok) {
...
bool RCC_token_is_unsigned(const RCCToken *tok) {
	ROSE_assert(RCC_token_is_number(tok) && tok->type && tok->type->is_basic);
...
int RCC_token_as_int(const RCCToken *tok) {
	ROSE_assert(RCC_token_is_integer(tok) && RCC_token_is_signed(tok) && tok->type->kind == TP_INT);

	long long payload = *(const long long *)tok->payload;
	long long mask = (1LL << (sizeof(int) * 8 + 1)) - 1;

	return (int)(payload & mask);
}
long RCC_token_as_long(const RCCToken *tok) {
	ROSE_assert(RCC_token_is_integer(tok) && RCC_token_is_signed(tok) && tok->type->kind == TP_LONG);

	long long payload = *(const long long *)tok->payload;
	long long mask = (1LL << (sizeof(long) * 8 + 1)) - 1;

	return (int)(payload & mask);
}
long long RCC_token_as_llong(const RCCToken *tok) {
	ROSE_assert(RCC_token_is_integer(tok) && RCC_token_is_signed(tok) && tok->type->kind == TP_LLONG);

	return *(const long long *)tok->payload;
}

unsigned int RCC_token_as_uint(const RCCToken *tok) {
	ROSE_assert(RCC_token_is_integer(tok) && RCC_token_is_unsigned(tok) && tok->type->kind == TP_INT);

	unsigned long long payload = *(const unsigned long long *)tok->payload;
	unsigned long long mask = (1ULL << (sizeof(int) * 8 + 1)) - 1;

	return (unsigned int)(payload & mask);
}
unsigned long RCC_token_as_ulong(const RCCToken *tok) {
	ROSE_assert(RCC_token_is_integer(tok) && RCC_token_is_unsigned(tok) && tok->type->kind == TP_LONG);

	unsigned long long payload = *(const unsigned long long *)tok->payload;
	unsigned long long mask = (1ULL << (sizeof(long) * 8 + 1)) - 1;

	return (unsigned long)(payload & mask);
}
unsigned long long RCC_token_as_ullong(const RCCToken *tok) {
	ROSE_assert(RCC_token_is_integer(tok) && RCC_token_is_unsigned(tok) && tok->type->kind == TP_LLONG);

	return *(const unsigned long long *)tok->payload;
}

long double RCC_token_as_scalar(const RCCToken *tok) {
	ROSE_assert(RCC_token_is_floating(tok));

	return *(const long double *)tok->payload;
}

float RCC_token_as_float(const RCCToken *tok) {
	ROSE_assert(RCC_token_is_floating(tok) && tok->type->kind == TP_FLOAT);

	return (float)RCC_token_as_scalar(tok);
}
double RCC_token_as_double(const RCCToken *tok) {
	ROSE_assert(RCC_token_is_floating(tok) && tok->type->kind == TP_DOUBLE);

	return (double)RCC_token_as_scalar(tok);
}
long double RCC_token_as_ldouble(const RCCToken *tok) {
	ROSE_assert(RCC_token_is_floating(tok) && tok->type->kind == TP_LDOUBLE);

	return (long double)RCC_token_as_scalar(tok);
}
