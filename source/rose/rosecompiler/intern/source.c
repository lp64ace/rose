...
RCCFileCache *RCC_fcache_read(const char *path) {
	...
	if (file) {
		...
		if (content) {
			...
			cache = RCC_fcache_new(path, content, actual);
		}
...
void RCC_source_error(const RCCFile *file, const RCCSLoc *location, const char *fmt, ...) {
	const char *input = file ? RCC_file_content(file) : NULL;
	...
	int indent = fprintf(stderr, "%s:%u(%u): ", RCC_file_fname(file), location->line, location->column);
	fprintf(stderr, "%.*s\n", (int)(end - begin), begin);
...
void RCC_sloc_copy(RCCSLoc *dst, const RCCSLoc *src) {
	if (src) {
