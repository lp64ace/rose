...
ROSE_INLINE bool same_qualified_type(const RCCType *a, const RCCType *b) {
	...
	if (!RCC_type_same(q1->base, q2->base)) {
		return false;
	}
	const RCCTypeQualification *qualification1 = RCC_type_qualification(a);
	const RCCTypeQualification *qualification2 = RCC_type_qualification(b);
...
ROSE_INLINE bool compatible_qualified_type(const RCCType *a, const RCCType *b) {
	if (a->kind == TP_QUALIFIED && b->kind != TP_QUALIFIED) {
		return RCC_type_is_zero_qualified(a) && RCC_type_compatible(RCC_type_unqualified(a), b);
	}
	if (!(a->kind == TP_QUALIFIED && b->kind == TP_QUALIFIED)) {
		return false;
	}
	const RCCTypeQualified *q1 = &a->tp_qualified, *q2 = &b->tp_qualified;
	if (!RCC_type_compatible(q1->base, q2->base)) {
		return false;
	}
	const RCCTypeQualification *qualification1 = RCC_type_qualification(a);
	const RCCTypeQualification *qualification2 = RCC_type_qualification(b);
...
ROSE_INLINE const struct RCCType *composite_qualified_type(struct RCContext *C, const struct RCCType *a, const struct RCCType *b) {
	...
	a = RCC_type_unqualified(a);
	b = RCC_type_unqualified(b);
	struct RCCType *composite = RCC_type_new_qualified(C, RCC_type_composite(C, a, b));
...
ROSE_INLINE RCCType *qualified_new(struct RCContext *C, const struct RCCType *type) {
	RCCType *q = RCC_context_calloc(C, sizeof(RCCType));
...
RCCType *RCC_type_new_qualified(RCContext *C, const RCCType *type) {
	RCCType *q = qualified_new(C, type);

	return q;
}

RCCType *RCC_type_new_qualified_ex(RCContext *C, const RCCType *type, const RCCTypeQualification *src) {
	RCCType *q = qualified_new(C, type);
