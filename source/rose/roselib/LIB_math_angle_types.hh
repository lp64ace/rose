#ifndef LIB_MATH_ANGLE_TYPES_HH
#define LIB_MATH_ANGLE_TYPES_HH
...
public:
	...
	/** Return the angle wrapped inside [-pi...pi] interval. */
	...
	ROSE_STRUCT_EQUALITY_OPERATORS_1(AngleRadianBase, value_)
...
/**
 * A `rose::math::AngleCartesianBase<T>` stores the angle as cosine + sine tuple.
 * - Storage : `2 * sizeof(T)`
 * - Range : [-pi..pi]
...
public:
	...
	AngleCartesianBase(const T &x, const T &y) : cos_(x), sin_(y) {
		ROSE_assert(math::abs(x * x + y * y - T(1)) < T(1e-4));
	}

	/**
	 * Create an angle from a radian value.
	 */
	explicit AngleCartesianBase(const T &radian) : AngleCartesianBase(math::cos(radian), math::sin(radian)) {};
	explicit AngleCartesianBase(const AngleRadianBase<T> &angle) : AngleCartesianBase(math::cos(angle.radian()), math::sin(angle.radian())) {};
	...
	friend AngleCartesianBase operator*(const AngleCartesianBase &a, const T &b) {
		...
		if (b == T(3)) {
			return {T(4) * (a.cos_ * a.cos_ * a.cos_) - T(3) * a.cos_, T(3) * a.sin_ - T(4) * (a.sin_ * a.sin_ * a.sin_)};
		}
		ROSE_assert_msg(0,
	...
	friend AngleCartesianBase operator/(const AngleCartesianBase &a, const T &divisor) {
		if (divisor == T(2)) {
			...
			AngleCartesianBase result = {math::sqrt((T(1) + a.cos_) / T(2)), math::sqrt((T(1) - a.cos_) / T(2))};
	...
	ROSE_STRUCT_EQUALITY_OPERATORS_2(AngleCartesianBase, cos_, sin_)
...
/**
 * A `rose::math::AngleFraction<T>` stores a radian angle as quotient.
 ...
 *
 * NOTE: Prefer converting to `rose::math::AngleCartesianBase<T>` if both `cos()` and `sin()`
...
private:
	 ...
	 * fractions.
	 */
	AngleFraction(int64_t numerator, int64_t denominator = 1) : numerator_(numerator), denominator_(denominator) {};
...
public:
	...
	AngleFraction wrapped() const {
		...
		return {mod_periodic(numerator_ + denominator_, denominator_ * 2) - denominator_, denominator_};
	}
	...
	friend AngleFraction operator+(const AngleFraction &a, const AngleFraction &b) {
		...
		return {(a.numerator_ * b.denominator_) + (b.numerator_ * a.denominator_), a.denominator_ * b.denominator_};
	}
	...
	operator AngleCartesianBase<T>() const {
		...
		ROSE_assert(abs(a.numerator_) <= a.denominator_);
		ROSE_assert(a.denominator_ > 0);
		...
		if (rest == 0) {
			...
			ROSE_assert(a.numerator_ >= 0 && a.numerator_ <= a.denominator_ / 4);
