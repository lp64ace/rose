...
/**
 * A specialization of `rose::DefaultHash<T>` provides a hash function for values of type T.
 ...
 *
 * Hash table implementations like rose::Set support heterogeneous key lookups. That means that
 ...
 *   either global or `rose` namespace.
 *
 *     template<> struct rose::DefaultHash<TheType> {
...
template<typename T1, typename T2, typename T3, typename T4> uint64_t get_default_hash(const T1 &v1, const T2 &v2, const T3 &v3, const T4 &v4) {
	const uint64_t h1 = get_default_hash(v1);
	const uint64_t h2 = get_default_hash(v2);
	const uint64_t h3 = get_default_hash(v3);
