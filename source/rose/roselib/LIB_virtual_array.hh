#ifndef LIB_VIRTUAL_ARRAY_HH
#define LIB_VIRTUAL_ARRAY_HH

#include <optional>

#include "LIB_any.hh"
#include "LIB_array.hh"
#include "LIB_devirtualize_parameters.hh"
...
struct CommonVArrayInfo {
	...
	CommonVArrayInfo(const Type _type, const bool _may_have_ownership, const void *_data) : type(_type), may_have_ownership(_may_have_ownership), data(_data) {
	}
};
...
public:
	VArrayImpl(const size_t size) : size_(size) {
		ROSE_assert(size_ >= 0);
	}
	...
	virtual void materialize(const IndexMask &mask, T *dst) const {
		mask.foreach_index([&](const size_t i) { dst[i] = this->get(i); });
	}
	...
	virtual void materialize_to_uninitialized(const IndexMask &mask, T *dst) const {
		mask.foreach_index([&](const size_t i) { new (dst + i) T(this->get(i)); });
	}
	...
	virtual void materialize_compressed(const IndexMask &mask, T *dst) const {
		mask.foreach_index([&](const size_t i, const size_t pos) { dst[pos] = this->get(i); });
	}
	...
	virtual void materialize_compressed_to_uninitialized(const IndexMask &mask, T *dst) const {
		mask.foreach_index([&](const size_t i, const size_t pos) { new (dst + pos) T(this->get(i)); });
	}
...
public:
	...
	virtual void set_all(Span<T> src) {
		...
		if (info.type == CommonVArrayInfo::Type::Span) {
			initialized_copy_n(src.data(), this->size_, const_cast<T *>(static_cast<const T *>(info.data)));
		}
		else {
...
public:
	VArrayImpl_For_Span(const MutableSpan<T> data) : VMutableArrayImpl<T>(data.size()), data_(data.data()) {
	}

protected:
	VArrayImpl_For_Span(const size_t size) : VMutableArrayImpl<T>(size) {
	}

	T get(const size_t index) const final {
		return data_[index];
	}

	void set(const size_t index, T value) final {
		data_[index] = value;
	}
	...
	void materialize(const IndexMask &mask, T *dst) const override {
		mask.foreach_index_optimized<size_t>([&](const size_t i) { dst[i] = data_[i]; });
	}

	void materialize_to_uninitialized(const IndexMask &mask, T *dst) const override {
		mask.foreach_index_optimized<size_t>([&](const size_t i) { new (dst + i) T(data_[i]); });
	}

	void materialize_compressed(const IndexMask &mask, T *dst) const override {
		mask.foreach_index_optimized<size_t>([&](const size_t i, const size_t pos) { dst[pos] = data_[i]; });
	}

	void materialize_compressed_to_uninitialized(const IndexMask &mask, T *dst) const override {
		mask.foreach_index_optimized<size_t>([&](const size_t i, const size_t pos) { new (dst + pos) T(data_[i]); });
	}
};
...
template<typename T> inline constexpr bool is_trivial_extended_v<VArrayImpl_For_Span_final<T>> = true;
 ...
 * elements.
 */
template<typename Container, typename T = typename Container::value_type> class VArrayImpl_For_ArrayContainer : public VArrayImpl_For_Span<T> {
private:
	Container container_;

public:
	VArrayImpl_For_ArrayContainer(Container container) : VArrayImpl_For_Span<T>(size_t(container.size())), container_(std::move(container)) {
...
public:
	VArrayImpl_For_Single(T value, const size_t size) : VArrayImpl<T>(size), value_(std::move(value)) {
	}
...
protected:
	...
	void materialize(const IndexMask &mask, T *dst) const override {
		mask.foreach_index([&](const size_t i) { dst[i] = value_; });
	}

	void materialize_to_uninitialized(const IndexMask &mask, T *dst) const override {
		mask.foreach_index([&](const size_t i) { new (dst + i) T(value_); });
	}
...
template<typename T> inline constexpr bool is_trivial_extended_v<VArrayImpl_For_Single<T>> = is_trivial_extended_v<T>;
...
public:
	VArrayImpl_For_Func(const size_t size, GetFunc get_func) : VArrayImpl<T>(size), get_func_(std::move(get_func)) {
	}
...
private:
	...
	void materialize(const IndexMask &mask, T *dst) const override {
		mask.foreach_index([&](const size_t i) { dst[i] = get_func_(i); });
	}

	void materialize_to_uninitialized(const IndexMask &mask, T *dst) const override {
		mask.foreach_index([&](const size_t i) { new (dst + i) T(get_func_(i)); });
	}

	void materialize_compressed(const IndexMask &mask, T *dst) const override {
		mask.foreach_index([&](const size_t i, const size_t pos) { dst[pos] = get_func_(i); });
	}

	void materialize_compressed_to_uninitialized(const IndexMask &mask, T *dst) const override {
		mask.foreach_index([&](const size_t i, const size_t pos) { new (dst + pos) T(get_func_(i)); });
	}
};
 ...
 */
template<typename StructT, typename ElemT, ElemT (*GetFunc)(const StructT &), void (*SetFunc)(StructT &, ElemT) = nullptr> class VArrayImpl_For_DerivedSpan final : public VMutableArrayImpl<ElemT> {
private:
	StructT *data_;

public:
	VArrayImpl_For_DerivedSpan(const MutableSpan<StructT> data) : VMutableArrayImpl<ElemT>(data.size()), data_(data.data()) {
	}

	template<typename OtherStructT, typename OtherElemT, OtherElemT (*OtherGetFunc)(const OtherStructT &), void (*OtherSetFunc)(OtherStructT &, OtherElemT)> friend class VArrayImpl_For_DerivedSpan;

private:
	ElemT get(const size_t index) const override {
		return GetFunc(data_[index]);
	}

	void set(const size_t index, ElemT value) override {
		SetFunc(data_[index], std::move(value));
	}

	void materialize(const IndexMask &mask, ElemT *dst) const override {
		mask.foreach_index_optimized<size_t>([&](const size_t i) { dst[i] = GetFunc(data_[i]); });
	}

	void materialize_to_uninitialized(const IndexMask &mask, ElemT *dst) const override {
		mask.foreach_index_optimized<size_t>([&](const size_t i) { new (dst + i) ElemT(GetFunc(data_[i])); });
	}

	void materialize_compressed(const IndexMask &mask, ElemT *dst) const override {
		mask.foreach_index_optimized<size_t>([&](const size_t i, const size_t pos) { dst[pos] = GetFunc(data_[i]); });
	}

	void materialize_compressed_to_uninitialized(const IndexMask &mask, ElemT *dst) const override {
		mask.foreach_index_optimized<size_t>([&](const size_t i, const size_t pos) { new (dst + pos) ElemT(GetFunc(data_[i])); });
	}
};

template<typename StructT, typename ElemT, ElemT (*GetFunc)(const StructT &), void (*SetFunc)(StructT &, ElemT)> inline constexpr bool is_trivial_extended_v<VArrayImpl_For_DerivedSpan<StructT, ElemT, GetFunc, SetFunc>> = true;
...
template<typename T> struct VArrayAnyExtraInfo {
	...
	template<typename StorageT> static constexpr VArrayAnyExtraInfo get() {
		...
		static_assert(std::is_base_of_v<VArrayImpl<T>, StorageT> || is_same_any_v<StorageT, const VArrayImpl<T> *, std::shared_ptr<const VArrayImpl<T>>>);
		...
		if constexpr (std::is_base_of_v<VArrayImpl<T>, StorageT>) {
			return {[](const void *buffer) { return static_cast<const VArrayImpl<T> *>((const StorageT *)buffer); }};
		}
...
protected:
	 ...
	 */
	using Storage = Any<rose::detail::VArrayAnyExtraInfo<T>, 24, 8>;
...
protected:
	...
	template<typename ImplT, typename... Args> void emplace(Args &&...args) {
		...
		if constexpr (std::is_copy_constructible_v<ImplT> && Storage::template is_inline_v<ImplT>) {
			 ...
			 * storage. */
			std::shared_ptr<const VArrayImpl<T>> ptr = std::make_shared<ImplT>(std::forward<Args>(args)...);
			impl_ = &*ptr;
...
public:
	...
	void materialize(const IndexMask &mask, MutableSpan<T> r_span) const {
		ROSE_assert(mask.min_array_size() <= this->size());
...
public:
	...
	VArray(varray_tag::single /*tag*/, T value, const size_t size) {
	 ...
	 */
	static VArray ForSingle(T value, const size_t size) {
	 ...
	 * accessed.
	 */
	template<typename GetFunc> static VArray ForFunc(const size_t size, GetFunc get_func) {
	 ...
	 */
	template<typename StructT, T (*GetFunc)(const StructT &)> static VArray ForDerivedSpan(Span<StructT> values) {
...
public:
	...
	VMutableArray(std::shared_ptr<const VMutableArrayImpl<T>> impl) : VArrayCommon<T>(std::move(impl)) {
	}
	 ...
	 * ownership of the span.
	 */
	template<typename StructT, T (*GetFunc)(const StructT &), void (*SetFunc)(StructT &, T)> static VMutableArray ForDerivedSpan(MutableSpan<StructT> values) {
	...
	void set_all(Span<T> src) {
		ROSE_assert(src.size() == this->size());
...
public:
	...
	VArraySpan(VArraySpan &&other) : varray_(std::move(other.varray_)), owned_data_(std::move(other.owned_data_)) {
		if (!varray_) {
			return;
		}
		this->size_ = varray_.size();
...
public:
	...
	MutableVArraySpan(VMutableArray<T> varray, const bool copy_values_to_span = true) : MutableSpan<T>(), varray_(std::move(varray)) {
		if (!varray_) {
			return;
		}
	...
	MutableVArraySpan(MutableVArraySpan &&other) : varray_(std::move(other.varray_)), owned_data_(std::move(other.owned_data_)), show_not_saved_warning_(other.show_not_saved_warning_) {
		if (!varray_) {
			return;
		}
...
public:
	SingleAsSpan(T value, size_t size) : value_(std::move(value)), size_(size) {
		ROSE_assert(size_ >= 0);
	}
...
template<typename T, bool UseSingle, bool UseSpan> struct VArrayDevirtualizer {
	...
	template<typename Fn> bool devirtualize(const Fn &fn) const {
		...
		const size_t size = this->varray.size();
 ...
 */
template<typename T, typename Func> inline void devirtualize_varray(const VArray<T> &varray, const Func &func, bool enable = true) {
	if (enable) {
		if (call_with_devirtualized_parameters(std::make_tuple(VArrayDevirtualizer<T, true, true>{varray}), func)) {
			return;
		}
	}
 ...
 * cases.
 */
template<typename T1, typename T2, typename Func> inline void devirtualize_varray2(const VArray<T1> &varray1, const VArray<T2> &varray2, const Func &func, bool enable = true) {
	if (enable) {
		if (call_with_devirtualized_parameters(std::make_tuple(VArrayDevirtualizer<T1, true, true>{varray1}, VArrayDevirtualizer<T2, true, true>{varray2}), func)) {
			return;
		}
	}
