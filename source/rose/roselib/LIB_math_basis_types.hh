#ifndef LIB_MATH_BASIS_TYPES_HH
#define LIB_MATH_BASIS_TYPES_HH

/**
 * Orthonormal rotation and orientation.
 *
 * A practical reminder:
 * - Forward is typically the positive Y direction in Rose.
 * - Up is typically the positive Z direction in Rose.
 * - Right is typically the positive X direction in Rose.
 * - Rose uses right handedness.
...
public:
	...
	constexpr static Axis from_char(char axis_char) {
		...
		ROSE_assert(int(Value::X) <= axis.as_int() && axis.as_int() <= int(Value::Z));
		return axis;
	}
	...
	constexpr static Axis from_int(const int axis_int) {
		...
		ROSE_assert(Axis::X <= axis && axis <= Axis::Z);
		return axis;
	}
...
public:
	...
	constexpr static AxisSigned from_int(int axis_int) {
		...
		ROSE_assert(AxisSigned::X_POS <= axis && axis <= AxisSigned::Z_NEG);
		return axis;
	}
...
[[nodiscard]] inline AxisSigned cross(const AxisSigned a, const AxisSigned b) {
	ROSE_assert_msg(abs(a) != abs(b), "Axes must not be colinear.");
	switch (a) {
...
template<typename T> T to_vector(const Axis axis) {
	ROSE_assert(axis.as_int() < T::type_length);
	T vec{};
	vec[axis.as_int()] = 1;
	return vec;
}
...
template<typename T> T to_vector(const AxisSigned axis) {
	ROSE_assert(abs(axis) <= AxisSigned::from_int(T::type_length - 1));
	T vec{};
...
/**
 * An `rose::math::CartesianBasis` represents an orientation that is aligned with the basis
...
struct CartesianBasis {
	...
	CartesianBasis(const AxisSigned x, const AxisSigned y, const AxisSigned z) : axes(x, y, z) {
		ROSE_assert(abs(x) != abs(y));
		ROSE_assert(abs(y) != abs(z));
		ROSE_assert(abs(z) != abs(x));
	}
 ...
 * The third axis is chosen by right hand rule to follow rose coordinate system.
 * \a forward is Y axis in rose coordinate system.
 * \a up is Z axis in rose coordinate system.
 * \note \a forward and \a up must be different axes.
 */
[[nodiscard]] ROSE_INLINE CartesianBasis from_orthonormal_axes(const AxisSigned forward, const AxisSigned up) {
	ROSE_assert(math::abs(forward) != math::abs(up));
 ...
 */
[[nodiscard]] ROSE_INLINE CartesianBasis rotation_between(const CartesianBasis &a, const CartesianBasis &b) {
 ...
 */
[[nodiscard]] ROSE_INLINE CartesianBasis rotation_between(const AxisSigned a_forward, const AxisSigned b_forward) {
	...
	return rotation_between(from_orthonormal_axes(a_forward, a_up), from_orthonormal_axes(b_forward, b_up));
}

template<typename T> [[nodiscard]] ROSE_INLINE VecBase<T, 3> transform_point(const CartesianBasis &basis, const VecBase<T, 3> &v) {
	VecBase<T, 3> result;
 ...
 */
[[nodiscard]] ROSE_INLINE CartesianBasis invert(const CartesianBasis &basis) {
