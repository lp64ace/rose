#ifndef LIB_VECTOR_SET_HH
#define LIB_VECTOR_SET_HH

/** \file
 *
 * A `rose::VectorSet<Key>` is an ordered container for elements of type `Key`. It has the same
 * interface as `rose::Set` with the following extensions:
 ...
 *
 * rose::VectorSet is implemented using open addressing in a slot array with a power-of-two
 * size. Other than in rose::Set, a slot does not contain the key though. Instead it only
 ...
 * - The hash function can be customized. See roselib_hash.hh for details.
 * - The probing strategy can be customized. See roselib_probing_strategies.hh for details.
 * - The slot type can be customized. See roselib_vector_set_slots.hh for details.
...
#include "LIB_vector_set_slots.hh"
...
template<
	...
	typename Key,
	/**
	 * The strategy used to deal with collisions. They are defined in roselib_probing_strategies.hh.
	 */
	typename ProbingStrategy = DefaultProbingStrategy,
	/**
	 * The hash function used to hash the keys. There is a default for many types. See roselib_hash.hh
	 ...
	 * Also see roselib_vector_set_slots.hh.
	 */
...
private:
	 ...
	 */
	size_t removed_slots_;
	size_t occupied_and_removed_slots_;

	/**
	 * The maximum number of slots that can be used (either occupied or removed) until the set has
	 * to grow. This is the total number of slots times the max load factor.
	 */
	size_t usable_slots_;
...
public:
	 ...
	 */
	VectorSet(Allocator allocator = {}) noexcept : removed_slots_(0), occupied_and_removed_slots_(0), usable_slots_(0), slot_mask_(0), slots_(1, allocator), keys_(nullptr) {
	}
	...
	/**
	 * Construct a vector set that contains the given keys. Duplicates will be removed
	 * automatically.
	 */
	VectorSet(const std::initializer_list<Key> &keys, Allocator allocator = {}) : VectorSet(Span(keys), allocator) {
	}
	...
	VectorSet(VectorSet &&other) noexcept : removed_slots_(other.removed_slots_), occupied_and_removed_slots_(other.occupied_and_removed_slots_), usable_slots_(other.usable_slots_), slot_mask_(other.slot_mask_), slots_(std::move(other.slots_)), keys_(other.keys_) {
	 ...
	 */
	const Key &operator[](const size_t index) const {
		ROSE_assert(index >= 0);
	...
	/**
	 * Add a key to the vector set. If the key exists in the set already, nothing is done. The
	 * return value is true if the key was newly added.
	 *
	...
	/**
	 * Deletes the key from the set. Returns true when the key existed in the set and is now
	 * removed. This might change the order of elements in the vector.
	 *
	...
	template<typename ForwardKey> size_t index_of_as(const ForwardKey &key) const {
	 ...
	 */
	size_t index_of_try(const Key &key) const {
		return this->index_of_try_as(key);
	}
	template<typename ForwardKey> size_t index_of_try_as(const ForwardKey &key) const {
		return this->index_of_try__impl(key, hash_(key));
	}

	/**
	 * Return the index of the key in the vector. If the key is not in the set, add it and return
	 * its index.
	 */
	size_t index_of_or_add(const Key &key) {
		return this->index_of_or_add_as(key);
	}
	size_t index_of_or_add(Key &&key) {
		return this->index_of_or_add_as(std::move(key));
	}
	template<typename ForwardKey> size_t index_of_or_add_as(ForwardKey &&key) {
	...
	template<typename ForwardKey> const Key &lookup_key_as(const ForwardKey &key) const {
		...
		ROSE_assert(key_ptr != nullptr);
		return *key_ptr;
	}

	/**
	 * Returns a pointer to the key that is stored in the vector set that compares equal to the
	 * given key. If the key is not in the set, null is returned.
	 */
	...
	size_t size_in_bytes() const {
		return size_t(sizeof(Slot) * slots_.size() + sizeof(Key) * usable_slots_);
	}

	/**
	 * Potentially resize the vector set such that it can hold n elements without doing another
	 * grow.
	 */
...
private:
	ROSE_NOINLINE void realloc_and_reinsert(const size_t min_usable_slots) {
		size_t total_slots, usable_slots;
	...
	template<typename ForwardKey> size_t index_of__impl(const ForwardKey &key, const uint64_t hash) const {
		ROSE_assert(this->contains_as(key));
	...
	template<typename ForwardKey> size_t index_of_try__impl(const ForwardKey &key, const uint64_t hash) const {
		VECTOR_SET_SLOT_PROBING_BEGIN(hash, slot) {
			if (slot.contains(key, is_equal_, hash, keys_)) {
				return slot.index();
			}
	...
	template<typename ForwardKey> size_t index_of_or_add__impl(ForwardKey &&key, const uint64_t hash) {
		this->ensure_can_add();
	...
	void remove_key_internal(Slot &slot) {
		size_t index_to_remove = slot.index();
	...
	void update_slot_index(const Key &key, const size_t old_index, const size_t new_index) {
	...
	template<typename ForwardKey> size_t count_collisions__impl(const ForwardKey &key, const uint64_t hash) const {
		size_t collisions = 0;
	...
	Key *allocate_keys_array(const size_t size) {
		return static_cast<Key *>(slots_.allocator().allocate(sizeof(Key) * size_t(size), alignof(Key), AT));
	}
...
/**
 * Same as a normal VectorSet, but does not use Rose's guarded allocator. This is useful when
 * allocating memory with static storage duration.
 */
template<typename Key, typename ProbingStrategy = DefaultProbingStrategy, typename Hash = DefaultHash<Key>, typename IsEqual = DefaultEquality<Key>, typename Slot = typename DefaultVectorSetSlot<Key>::type> using RawVectorSet = VectorSet<Key, ProbingStrategy, Hash, IsEqual, Slot, RawAllocator>;
 ...
 */
template<typename T, typename GetIDFn> using CustomIDVectorSet = VectorSet<T, DefaultProbingStrategy, CustomIDHash<T, GetIDFn>, CustomIDEqual<T, GetIDFn>>;

}  // namespace rose

#endif	// LIB_VECTOR_SET_HH
