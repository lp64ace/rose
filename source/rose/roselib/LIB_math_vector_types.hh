...
/**
 * Variadic tempaltes are used to unroll loops manually. This helps GCC avoid
 * branching during math operations and makes the code generation more explicit
 * and predictable. Unrolling should always be worth it because the vector size
...
template<typename T, int Size> struct VecBase : public vec_struct_base<T, Size> {
	...
	template<typename U, ROSE_ENABLE_IF((std::is_constructible_v<U, T>))> explicit VecBase(U value) : VecBase(T(value)) {
	}
...
#undef ROSE_ENABLE_IF_VEC
	...
	template<typename U, ROSE_ENABLE_IF((std::is_constructible_v<U, T>))> explicit VecBase(const U *ptr) {
		unroll<Size>([&](auto i) { (*this)[i] = ptr[i]; });
	}
...
#define ROSE_INT_OP(_T) template<typename U = _T, ROSE_ENABLE_IF((std::is_integral_v<U>))>
	...
	VecBase &operator<<=(const VecBase &b) {
		VecBase result;
		unroll<Size>([&](auto i) { result[i] <<= b[i]; });
		return result;
	}
...
using uchar3 = rose::VecBase<int8_t, 3>;
