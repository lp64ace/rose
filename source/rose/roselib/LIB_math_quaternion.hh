#ifndef LIB_MATH_QUATERNION_HH
#define LIB_MATH_QUATERNION_HH
 ...
 */
template<typename T> [[nodiscard]] ROSE_INLINE T dot(const QuaternionBase<T> &a, const QuaternionBase<T> &b);
 ...
 */
template<typename T> [[nodiscard]] ROSE_INLINE QuaternionBase<T> conjugate(const QuaternionBase<T> &a);

/**
 * Negate the quaternion if real component (w) is negative.
 */
template<typename T> [[nodiscard]] ROSE_INLINE QuaternionBase<T> canonicalize(const QuaternionBase<T> &q);
 ...
 */
template<typename T> [[nodiscard]] ROSE_INLINE QuaternionBase<T> invert(const QuaternionBase<T> &q);
 ...
 */
template<typename T> [[nodiscard]] ROSE_INLINE QuaternionBase<T> invert_normalized(const QuaternionBase<T> &q);

/**
 * Return a unit quaternion representing the same rotation as \a q or
 * the identity quaternion if \a q is ill-formed.
 */
template<typename T> [[nodiscard]] ROSE_INLINE QuaternionBase<T> normalize(const QuaternionBase<T> &q);
template<typename T> [[nodiscard]] ROSE_INLINE QuaternionBase<T> normalize_and_get_length(const QuaternionBase<T> &q, T &out_length);

/**
 * Use spherical interpolation between two quaternions.
 * Always interpolate along the shortest angle.
 */
template<typename T> [[nodiscard]] ROSE_INLINE QuaternionBase<T> interpolate(const QuaternionBase<T> &a, const QuaternionBase<T> &b, T t);
 ...
 */
template<typename T> [[nodiscard]] ROSE_INLINE VecBase<T, 3> transform_point(const QuaternionBase<T> &q, const VecBase<T, 3> &v);
 ...
 */
template<typename T> [[nodiscard]] ROSE_INLINE bool is_zero(const QuaternionBase<T> &q) {
 ...
 */
template<typename T> [[nodiscard]] ROSE_INLINE bool is_equal(const QuaternionBase<T> &a, const QuaternionBase<T> &b, const T epsilon = T(0)) {
	return math::abs(a.w - b.w) <= epsilon && math::abs(a.y - b.y) <= epsilon && math::abs(a.x - b.x) <= epsilon && math::abs(a.z - b.z) <= epsilon;
}

template<typename T> [[nodiscard]] ROSE_INLINE bool is_unit_scale(const QuaternionBase<T> &q) {
	...
	return (!(math::abs(test_unit - T(1)) >= AssertUnitEpsilon<QuaternionBase<T>>::value) || !(math::abs(test_unit) >= AssertUnitEpsilon<QuaternionBase<T>>::value));
}
...
template<typename T> QuaternionBase<T> QuaternionBase<T>::swing(const Axis axis) const {
	...
	ROSE_assert(math::abs(VecBase<T, 4>(swing)[axis.as_int() + 1]) < 0.0002f /*ROSE_ASSERT_UNIT_EPSILON*/);
	return swing;
}
...
template<typename T> QuaternionBase<T> QuaternionBase<T>::twist(const Axis axis) const {
	...
	AngleCartesianBase<T> half_angle = AngleCartesianBase<T>::from_point(input_vec[0], input_vec.yzw()[axis.as_int()]);
...
template<typename T> QuaternionBase<T> QuaternionBase<T>::wrapped_around(const QuaternionBase<T> &reference) const {
	ROSE_assert(is_unit_scale(*this));
	...
	return (distance_squared(VecBase<T, 4>(-result), VecBase<T, 4>(reference)) < distance_squared(VecBase<T, 4>(result), VecBase<T, 4>(reference))) ? -result : result;
}
 ...
 */
template<typename T> [[nodiscard]] ROSE_INLINE VecBase<T, 2> interpolate_dot_slerp(const T t, const T cosom) {
	const T eps = T(1e-4);

	ROSE_assert(IN_RANGE_INCL(cosom, T(-1.0001), T(1.0001)));

	VecBase<T, 2> w;
	T abs_cosom = math::abs(cosom);
	/* Within [-1..1] range, avoid aligned axis. */
	if (abs_cosom < (T(1) - eps)) {
...
template<typename T> [[nodiscard]] ROSE_INLINE QuaternionBase<T> interpolate(const QuaternionBase<T> &a, const QuaternionBase<T> &b, T t) {
	...
	ROSE_assert(is_unit_scale(a));
	ROSE_assert(is_unit_scale(b));
...
template<typename T> DualQuaternionBase<T>::DualQuaternionBase(const QuaternionBase<T> &non_dual, const QuaternionBase<T> &dual) : quat(non_dual), trans(dual), scale_weight(0), quat_weight(1) {
	ROSE_assert(is_unit_scale(non_dual));
}

template<typename T> DualQuaternionBase<T>::DualQuaternionBase(const QuaternionBase<T> &non_dual, const QuaternionBase<T> &dual, const MatBase<T, 4, 4> &scale_mat) : quat(non_dual), trans(dual), scale(scale_mat), scale_weight(1), quat_weight(1) {
	ROSE_assert(is_unit_scale(non_dual));
}

/* -------------- Operators -------------- */

template<typename T> DualQuaternionBase<T> &DualQuaternionBase<T>::operator+=(const DualQuaternionBase<T> &b) {
 ...
 */
template<typename T> [[nodiscard]] DualQuaternionBase<T> normalize(const DualQuaternionBase<T> &dual_quat) {
 ...
 */
template<typename T> [[nodiscard]] VecBase<T, 3> transform_point(const DualQuaternionBase<T> &dq, const VecBase<T, 3> &point, MatBase<T, 3, 3> *r_crazy_space_mat = nullptr) {
	ROSE_assert(is_normalized(dq));
	ROSE_assert(is_unit_scale(dq.quat));
	/**
	 * From:
 ...
 */
template<typename T> [[nodiscard]] DualQuaternionBase<T> to_dual_quaternion(const MatBase<T, 4, 4> &mat, const MatBase<T, 4, 4> &basemat) {
	/**
	...
	Mat4T R, scale;
	const bool has_scale = !is_orthonormal(mat) || is_negative(mat) || length_squared(to_scale(baseRS) - T(1)) > square(1e-4f);
	if (has_scale) {
...
template<typename T, typename AngleT = AngleRadian> AxisAngleBase<T, AngleT> to_axis_angle(const QuaternionBase<T> &quat) {
	ROSE_assert(is_unit_scale(quat));
	...
	if (sin_half_angle < T(0.0005)) {
		...
		ROSE_assert(abs(cos_half_angle) > 0.0005);
