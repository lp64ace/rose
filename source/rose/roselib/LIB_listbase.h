#ifndef __LIB_LISTBASE_H__
#define __LIB_LISTBASE_H__
 ...
 *
 * \param listbase The ListBase to check.
 *
 * \return `true` if the ListBase contains no Links, `false` otherwise.
 */
 ...
 * \param listbase The ListBase to check.
 *
 * \return `true` if the ListBase contains exactly one Link, `false` otherwise.
 */
 ...
 *
 * \param listbase The ListBase to clear.
 * \note This function only resets the pointers within the ListBase structure.
 *       It does not free the memory occupied by the Links themselves;
 *       memory management must be handled separately.
 */
