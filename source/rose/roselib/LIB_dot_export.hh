...
#include "LIB_dot_export_attribute_enums.hh"
...
public:
	NodePort(Node &node, std::optional<std::string> port_name = {}, std::optional<std::string> port_position = {}) : node_(&node), port_name_(std::move(port_name)), port_position_(std::move(port_position)) {
	}

	void to_dot_string(std::stringstream &ss) const;
};

class Edge : rose::NonCopyable, rose::NonMovable {
protected:
	NodePort a_;
	NodePort b_;
