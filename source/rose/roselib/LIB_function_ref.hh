...
public:
	 ...
	 * another lambda.
	 */
	template<typename Callable, ROSE_ENABLE_IF((!std::is_same_v<std::remove_cv_t<std::remove_reference_t<Callable>>, FunctionRef>)), ROSE_ENABLE_IF((std::is_invocable_r_v<Ret, Callable, Params...>))>
	FunctionRef(Callable &&callable) : callback_(callback_fn<typename std::remove_reference_t<Callable>>), callable_(intptr_t(&callable)) {
	}
