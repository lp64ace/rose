...
ROSE_INLINE void ghash_entry_copy(GHash *dhash, Entry *dentry, const GHash *shash, const Entry *sentry, GHashKeyCopyFP keycopyfp, GHashValCopyFP valcopyfp) {
	dentry->key = (keycopyfp) ? keycopyfp(sentry) : sentry;
...
ROSE_INLINE uint ghash_bucket_index(const GHash *cont, const uint hash) {
#ifdef GHASH_USE_MODULO_BUCKETS
	return hash % cont->nbuckets;
#else
	return hash & cont->bucket_mask;
#endif
}

ROSE_INLINE uint ghash_find_next_bucket_index(const GHash *cont, uint curr_bucket) {
...
static void ghash_buckets_resize(GHash *cont, const uint nbuckets) {
	Entry **buckets_old = cont->buckets;
	Entry **buckets_new;
	const uint nbuckets_old = cont->nbuckets;
	uint i;
...
static void ghash_buckets_expand(GHash *cont, const uint nentries, const bool user_defined) {
	uint new_nbuckets;

	if (LIKELY(cont->buckets && (nentries < cont->limit_grow))) {
		return;
	}
...
static void ghash_buckets_contract(GHash *cont, const uint nentries, const bool user_defined, const bool force_shrink) {
	uint new_nbuckets;
	...
	if (LIKELY(cont->buckets && (nentries > cont->limit_shrink))) {
		return;
	}
...
ROSE_INLINE void ghash_buckets_reset(GHash *cont, const uint nentries) {
...
ROSE_INLINE Entry *ghash_lookup_entry_ex(const GHash *cont, const void *key, const uint bucket_index) {
	Entry *e;
	/**
	...
	for (e = cont->buckets[bucket_index]; e; e = e->next) {
		if (UNLIKELY(cont->cmpfp(key, e->key) == false)) {
			return e;
		}
	}
 ...
 */
ROSE_INLINE Entry *ghash_lookup_entry_prev_ex(GHash *cont, const void *key, Entry **r_e_prev, const uint bucket_index) {
	/**
	 * If we do not store GHash, not worth computing it for each entry here!
	 * Typically, comparison function will be quicker, and since it's needed in the end anyway...
	 */
...
static GHash *ghash_new(GHashHashFP hashfp, GHashCmpFP cmpfp, const char *info, const uint nentries_reserve, const uint flag) {
	...
	cont->entrypool = LIB_mempool_create(GHASH_ENTRY_SIZE(flag & GHASH_FLAG_IS_GSET), 64, 64, ROSE_MEMPOOL_NOP);

	return cont;
}

static void ghash_insert_ex(GHash *cont, void *key, void *val, const uint bucket_index) {
	GHashEntry *e = LIB_mempool_alloc(cont->entrypool);
...
ROSE_INLINE void ghash_insert_ex_keyonly_entry(GHash *cont, void *key, const uint bucket_index, Entry *e) {
	ROSE_assert((cont->flag & GHASH_FLAG_ALLOW_DUPES) || (LIB_ghash_haskey(cont, key) == 0));
...
ROSE_INLINE void ghash_insert_ex_keyonly(GHash *cont, void *key, const uint bucket_index) {
	Entry *e = LIB_mempool_alloc(cont->entrypool);
...
static Entry *ghash_remove_ex(GHash *cont, const void *key, GHashKeyFreeFP keyfreefp, GHashValFreeFP valfreefp, uint bucket_index) {
...
static GHash *ghash_copy(const GHash *cont, GHashKeyCopyFP keycopyfp, GHashValCopyFP valcopyfp) {
	...
	const uint reserve_nentries_new = ROSE_MAX(GHASH_LIMIT_GROW(cont->nbuckets) - 1, cont->nentries);
	
	ROSE_assert(!valcopyfp || !(cont->flag & GHASH_FLAG_IS_GSET));
	
	cont_dup = ghash_new(cont->hashfp, cont->cmpfp, "rose::GHash::duplicate", 0, cont->flag);
	...
	for (i = 0; i < cont->nbuckets; i++) {
		...
		for(e = cont->buckets[i]; e; e = e->next) {
			Entry *e_dup = LIB_mempool_alloc(cont_dup->entrypool);
...
GHash *LIB_ghash_new_ex(GHashHashFP hashfp, GHashCmpFP cmpfp, const char *info, uint reserve) {
...
void LIB_ghash_free(GHash *cont, GHashKeyFreeFP keyfreefp, GHashValFreeFP valfreefp) {
	ROSE_assert(cont->nentries == LIB_mempool_length(cont->entrypool));
	if(keyfreefp || valfreefp) {
		ghash_free_cb(cont, keyfreefp, valfreefp);
	}
	
	MEM_freeN(cont->buckets);
	LIB_mempool_destroy(cont->entrypool);
	MEM_freeN(cont);
}

void LIB_ghash_reserve(GHash *cont, uint reserve) {
...
bool LIB_ghash_ensure_p(GHash *cont, void *key, void ***r_val) {
	...
	if (!haskey) {
		e = LIB_mempool_alloc(cont->entrypool);
...
bool LIB_ghash_remove(GHash *cont, const void *key, GHashKeyFreeFP keyfreefp, GHashValFreeFP valfreefp) {
	...
	if (e) {
		LIB_mempool_free(cont->entrypool, e);
		return true;
	}
	return false;
}
...
void LIB_ghash_clear_ex(GHash *cont, GHashKeyFreeFP keyfreefp, GHashValFreeFP valfreefp, uint reserve) {
	if (keyfreefp || valfreefp) {
		ghash_free_cb(cont, keyfreefp, valfreefp);
	}

	ghash_buckets_reset(cont, reserve);
	LIB_mempool_clear_ex(cont->entrypool, reserve ? (int)reserve : -1);
}
...
uint LIB_ghash_len(const GHash *cont) {
...
void LIB_ghashIterator_init(GHashIterator *iter, GHash *cont) {
	iter->hash = cont;
	iter->curEntry = NULL;
	iter->curBucket = UINT_MAX; /* wraps to zero */
	if (cont->nentries) {
		do {
			iter->curBucket++;
			if (UNLIKELY(iter->curBucket == iter->hash->nbuckets)) {
				break;
			}
			iter->curEntry = iter->hash->buckets[iter->curBucket];
		} while (!iter->curEntry);
	}
}
...
void LIB_ghashIterator_step(GHashIterator *iter) {
	if (iter->curEntry) {
		iter->curEntry = iter->curEntry->next;
		while (!iter->curEntry) {
			iter->curBucket++;
			if (iter->curBucket == iter->hash->nbuckets) {
				break;
			}
			iter->curEntry = iter->hash->buckets[iter->curBucket];
		}
...
GSet *LIB_gset_new_ex(GSetHashFP hashfp, GSetCmpFP cmpfp, const char *info, uint reserve) {
...
void LIB_gset_insert(GSet *set, void *key) {
	...
	const uint bucket_index = ghash_bucket_index((GHash *)set, hash);
...
bool LIB_gset_ensure_p_ex(GSet *set, const void *key, void ***r_key) {
	...
	if (!haskey) {
		/* Pass 'key' in case we resize */
		e = LIB_mempool_alloc(((GHash *)set)->entrypool);
...
bool LIB_gset_pop(GSet *set, GSetIterState *state, void ***r_key) {
	...
	if (e) {
		*r_key = e->key;
		...
		LIB_mempool_free(((GHash *)set)->entrypool, e);
		return true;
	}
...
void LIB_gset_clear_ex(GSet *set, GSetKeyFreeFP keyfreefp, uint reserve) {
...
uint LIB_gset_len(const GSet *set) {
