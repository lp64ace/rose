...
template<typename T> bool is_negative(const MatBase<T, 4, 4> &mat) {
	return Eigen::Map<const Eigen::Matrix<T, 3, 3>, 0, Eigen::Stride<4, 1>>(mat.base_ptr()).determinant() < T(0);
}
...
template<typename T, int Size> MatBase<T, Size, Size> invert(const MatBase<T, Size, Size> &mat, bool &r_success) {
...
template<typename T, int Size> MatBase<T, Size, Size> pseudo_invert(const MatBase<T, Size, Size> &mat, T epsilon) {
...

	{
		...
		/* Rose and Eigen matrices are both column-major by default.
		...
		JacobiSVD<MatrixDynamicT, NoQRPreconditioner> svd(Eigen::Map<const MatrixDynamicT>(mat.base_ptr(), Size, Size), ComputeThinU | ComputeThinV);
 ...
 */
template<typename T> static void polar_decompose(const MatBase<T, 3, 3> &mat3, MatBase<T, 3, 3> &r_U, MatBase<T, 3, 3> &r_P) {
...

	{
		 ...
		 */
		using MatrixDynamicT = Eigen::Matrix<T, Dynamic, Dynamic>;
		JacobiSVD<MatrixDynamicT, NoQRPreconditioner> svd(Eigen::Map<const MatrixDynamicT>(mat3.base_ptr(), 3, 3), ComputeThinU | ComputeThinV);
...
template<typename T> MatBase<T, 3, 3> interpolate(const MatBase<T, 3, 3> &A, const MatBase<T, 3, 3> &B, T t) {
...
template<typename T> MatBase<T, 4, 4> interpolate(const MatBase<T, 4, 4> &A, const MatBase<T, 4, 4> &B, T t) {
	MatBase<T, 4, 4> result = MatBase<T, 4, 4>(interpolate(MatBase<T, 3, 3>(A), MatBase<T, 3, 3>(B), t));
...
template<typename T> MatBase<T, 3, 3> interpolate_fast(const MatBase<T, 3, 3> &a, const MatBase<T, 3, 3> &b, T t) {
...
template<typename T> MatBase<T, 4, 4> interpolate_fast(const MatBase<T, 4, 4> &a, const MatBase<T, 4, 4> &b, T t) {
	using QuaternionT = QuaternionBase<T>;
	using Vec3T = typename MatBase<T, 3, 3>::vec3_type;
