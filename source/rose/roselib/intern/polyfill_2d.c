...
ROSE_INLINE eSign signum_enum(float a) {
	if (a > 0.0f) {
		return CONVEX;
	}
	if (a == 0.0f) {
 ...
 */
ROSE_INLINE float area_tri_signed_v2_alt_2x(const float v1[2], const float v2[2], const float v3[2]) {
	float d2[2], d3[2];
...
static void kdtree2d_init(struct KDTree2D *tree, const uint32_t coords_num, const PolyIndex *indices) {
	...
	ROSE_assert(tree->node_num == (uint32_t)(node - tree->nodes));
}
...
static void kdtree2d_init_mapping(struct KDTree2D *tree) {
	...
	for (i = 0, node = tree->nodes; i < tree->node_num; i++, node++) {
		...
		/* build map */
		ROSE_assert(tree->nodes_map[node->index] == KDNODE_UNSET);
...
static void kdtree2d_node_remove(struct KDTree2D *tree, uint32_t index) {
	...
	ROSE_assert((node->flag & KDNODE_FLAG_REMOVED) == 0);
	node->flag |= KDNODE_FLAG_REMOVED;

	while ((node->neg == KDNODE_UNSET) && (node->pos == KDNODE_UNSET) && (node->parent != KDNODE_UNSET)) {
		KDTreeNode2D *node_parent = &tree->nodes[node->parent];

		ROSE_assert((uint32_t)(node - tree->nodes) == node_index);
		if (node_parent->neg == node_index) {
			node_parent->neg = KDNODE_UNSET;
		}
		else {
			ROSE_assert(node_parent->pos == node_index);
...
static bool kdtree2d_isect_tri_recursive(const struct KDTree2D *tree, const uint32_t tri_index[3], const float *tri_coords[3], const float tri_center[2], const struct KDRange2D bounds[2], const KDTreeNode2D *node) {
	...
	if ((node->flag & KDNODE_FLAG_REMOVED) == 0) {
		...
		if ((co[0] >= bounds[0].min) && (co[0] <= bounds[0].max) && (co[1] >= bounds[1].min) && (co[1] <= bounds[1].max)) {
			if ((span_tri_v2_sign(tri_coords[0], tri_coords[1], co) != CONCAVE) && (span_tri_v2_sign(tri_coords[1], tri_coords[2], co) != CONCAVE) && (span_tri_v2_sign(tri_coords[2], tri_coords[0], co) != CONCAVE)) {
...
#	define KDTREE2D_ISECT_TRI_RECURSE_NEG (((node->neg != KDNODE_UNSET) && (co[node->axis] >= bounds[node->axis].min)) && kdtree2d_isect_tri_recursive(tree, tri_index, tri_coords, tri_center, bounds, &tree->nodes[node->neg]))
#	define KDTREE2D_ISECT_TRI_RECURSE_POS (((node->pos != KDNODE_UNSET) && (co[node->axis] <= bounds[node->axis].max)) && kdtree2d_isect_tri_recursive(tree, tri_index, tri_coords, tri_center, bounds, &tree->nodes[node->pos]))
...
#	undef KDTREE2D_ISECT_TRI_RECURSE_POS
	...
	ROSE_assert(node->index != KDNODE_UNSET);
...
#	ifdef USE_CONVEX_SKIP_TEST
	/* check if counting is wrong */
	{
		...
		ROSE_assert(coords_num_concave_test == pf->coords_num_concave);
	}
#	endif
	...
	if (pi_ear_tip->sign != sign_accept) {
		return false;
	}
...
#	ifndef NDEBUG
		if (coords_sign == 1) {
			ROSE_assert(cross_poly_v2(coords, coords_num) <= 0.0f);
		}
		else {
			ROSE_assert(cross_poly_v2(coords, coords_num) >= 0.0f);
		}
...
void LIB_polyfill_calc_arena(const float (*coords)[2], const uint32_t coords_num, const int coords_sign, uint32_t (*r_tris)[3], MemArena *arena) {
	PolyFill pf;
	PolyIndex *indices = LIB_memory_arena_malloc(arena, sizeof(*indices) * coords_num);

	polyfill_prepare(&pf, coords, coords_num, coords_sign, r_tris, indices);

#ifdef USE_KDTREE
	if (pf.coords_num_concave) {
		pf.kdtree.nodes = LIB_memory_arena_malloc(arena, sizeof(*pf.kdtree.nodes) * pf.coords_num_concave);
		pf.kdtree.nodes_map = memset(LIB_memory_arena_malloc(arena, sizeof(*pf.kdtree.nodes_map) * coords_num), 0xff, sizeof(*pf.kdtree.nodes_map) * coords_num);
	}
	else {
...
void LIB_polyfill_calc(const float (*coords)[2], const uint32_t coords_num, const int coords_sign, uint32_t (*r_tris)[3]) {
	...
	if (coords_num > 8192) {
		...
		MemArena *arena = LIB_memory_arena_create(sizeof(PolyIndex) * coords_num, __func__);
		LIB_polyfill_calc_arena(coords, coords_num, coords_sign, r_tris, arena);
		LIB_memory_arena_destroy(arena);
		return;
	}

	PolyFill pf;
	PolyIndex *indices = LIB_array_alloca(indices, coords_num);
	...
	if (pf.coords_num_concave) {
		...
		pf.kdtree.nodes_map = memset(LIB_array_alloca(pf.kdtree.nodes_map, coords_num), 0xff, sizeof(*pf.kdtree.nodes_map) * coords_num);
	}
	else {
		pf.kdtree.node_num = 0;
	}
