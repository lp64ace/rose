...
bool LIB_rctf_inside_rctf(const rctf *rct_a, const rctf *rct_b) {
	return ((rct_a->xmin <= rct_b->xmin) && (rct_a->xmax >= rct_b->xmax) && (rct_a->ymin <= rct_b->ymin) && (rct_a->ymax >= rct_b->ymax));
}
...
bool LIB_rcti_isect_segment(const rcti *rect, const int s1[2], const int s2[2]) {
	...
	if (LIB_rcti_isect_pt_v(rect, s1) || LIB_rcti_isect_pt_v(rect, s2)) {
		return true;
	}
...
bool LIB_rctf_isect_segment(const rctf *rect, const float s1[2], const float s2[2]) {
	...
	/* if either points intersect then we definitely intersect */
	if (LIB_rctf_isect_pt_v(rect, s1) || LIB_rctf_isect_pt_v(rect, s2)) {
		return true;
	}
...
void LIB_rctf_sanitize(rctf *rect) {
	...
	ROSE_assert(LIB_rctf_is_valid(rect));
}
...
void LIB_rcti_sanitize(rcti *rect) {
	...
	ROSE_assert(LIB_rcti_is_valid(rect));
}
...
void LIB_rctf_pad_y(rctf *rect, const float boundary_size, const float pad_min, const float pad_max) {
	ROSE_assert(pad_max >= 0.0f);
	ROSE_assert(pad_min >= 0.0f);
	ROSE_assert(boundary_size > 0.0f);
...
void print_rctf(const char *str, const rctf *rect) {
	printf("%s: xmin %.8f, xmax %.8f, ymin %.8f, ymax %.8f (%.12fx%.12f)\n", str, rect->xmin, rect->xmax, rect->ymin, rect->ymax, LIB_rctf_size_x(rect), LIB_rctf_size_y(rect));
}

void print_rcti(const char *str, const rcti *rect) {
	printf("%s: xmin %d, xmax %d, ymin %d, ymax %d (%dx%d)\n", str, rect->xmin, rect->xmax, rect->ymin, rect->ymax, LIB_rcti_size_x(rect), LIB_rcti_size_y(rect));
}
...
void LIB_rctf_rotate_expand(rctf *dst, const rctf *src, const float angle) {
	...
	corner[1] *= -1;
	ROTATE_SINCOS(corner_rot, mat2, corner);
	corder_max[0] = ROSE_MAX(corder_max[0], fabsf(corner_rot[0]));
	corder_max[1] = ROSE_MAX(corder_max[1], fabsf(corner_rot[1]));
