...
float closest_seg_seg_v2(float r_closest_a[2], float r_closest_b[2], float *r_lambda_a, float *r_lambda_b, const float a1[2], const float a2[2], const float b1[2], const float b2[2]) {
	...
	const float min_dist_sq = ROSE_MIN(dist_sq1, dist_sq2, dist_sq3, dist_sq4);
	if (min_dist_sq == dist_sq1) {
		...
		*r_lambda_b = 0.0f;
		...
		ROSE_assert(min_dist_sq == dist_sq4);
...
void dist_squared_ray_to_aabb_v3_precalc(struct DistRayAABB_Precalc *neasrest_precalc, const float ray_origin[3], const float ray_direction[3]) {
	...
	for (int i = 0; i < 3; i++) {
		neasrest_precalc->ray_inv_dir[i] = (neasrest_precalc->ray_direction[i] != 0.0f) ? (1.0f / neasrest_precalc->ray_direction[i]) : FLT_MAX;
	}
}
...
float dist_squared_ray_to_aabb_v3(const struct DistRayAABB_Precalc *data, const float bb_min[3], const float bb_max[3], float r_point[3], float *r_depth) {
	...
	float va[3], vb[3];
	/* `rtmin` and `rtmax` are the minimum and maximum distances of the ray hits
	 * on the AABB */
	float rtmin, rtmax;
	int main_axis;
	...
	return dist_squared_ray_to_seg_v3(data->ray_origin, data->ray_direction, va, vb, r_point, r_depth);
}
...
void dist_squared_to_projected_aabb_precalc(struct DistProjectedAABBPrecalc *precalc, const float projmat[4][4], const float winsize[2], const float mval[2]) {
	...
	for (int i = 0; i < 3; i++) {
		precalc->ray_inv_dir[i] = (precalc->ray_direction[i] != 0.0f) ? (1.0f / precalc->ray_direction[i]) : FLT_MAX;
	}
}
...
float dist_seg_seg_v2(const float a1[3], const float a2[3], const float b1[3], const float b2[3]) {
	...
	return sqrtf(ROSE_MIN(d1, d2, d3, d4));
}
...
int isect_seg_seg_v2_point_ex(const float v0[2], const float v1[2], const float v2[2], const float v3[2], const float endpoint_bias, float r_vi[2]) {
	...
	if (d != 0) {
		...
		if ((u >= endpoint_min && u <= endpoint_max) && (v >= endpoint_min && v <= endpoint_max)) {
			...
			/* When 'd' approaches zero, float precision lets non-overlapping
			 * co-linear segments detect as an intersection. So re-calculate 'v' to
			 * ensure the point overlaps both. see T45123 */
...
int isect_line_sphere_v3(const float l1[3], const float l2[3], const float sp[3], const float r, float r_p1[3], float r_p2[3]) {
	/* adapted for use in rose by Campbell Barton - 2011
	 *
	...
	const float b = 2.0f * (ldir[0] * (l1[0] - sp[0]) + ldir[1] * (l1[1] - sp[1]) + ldir[2] * (l1[2] - sp[2]));
...
bool isect_point_poly_v2(const float pt[2], const float verts[][2], const unsigned int nr, const bool _unused) {
	/* Keep in sync with #isect_point_poly_v2_int. */

	unsigned int i, j;
	bool isect = false;
	for (i = 0, j = nr - 1; i < nr; j = i++) {
		if (((verts[i][1] > pt[1]) != (verts[j][1] > pt[1])) && (pt[0] < (verts[j][0] - verts[i][0]) * (pt[1] - verts[i][1]) / (verts[j][1] - verts[i][1]) + verts[i][0])) {
			isect = !isect;
		}
	}
	return isect;
}
bool isect_point_poly_v2_int(const int pt[2], const int verts[][2], const unsigned int nr, const bool _unused) {
...
bool isect_ray_tri_v3(const float ray_origin[3], const float ray_direction[3], const float v0[3], const float v1[3], const float v2[3], float *r_lambda, float r_uv[2]) {
	/* NOTE(@campbellbarton): these values were 0.000001 in 2.4x but for
	 * projection snapping on a human head `(1BU == 1m)`, subdivision-surface
	 * level 2, this gave many errors. */
			...
			for (int k = j + 1; k < planes_len; k++) {
				...
				for (i_test = 0; i_test < planes_len; i_test++) {
					...
					if (((dot_v3v3(np_test, co_test) + np_test[3]) > eps_isect)) {
						/* For low epsilon values the point could intersect
						 * its own plane. */
...
bool isect_tri_tri_v3_ex(const float tri_a[3][3], const float tri_b[3][3], float r_i1[3], float r_i2[3], int *r_tri_a_edge_isect_count) {
	...
	*r_tri_a_edge_isect_count = 0;
	...
	if ((side[1][0] && side[1][1] && side[1][2]) && (side[1][0] < 0.0f) == (side[1][1] < 0.0f) && (side[1][0] < 0.0f) == (side[1][2] < 0.0f)) {
	...
	if ((side[0][0] && side[0][1] && side[0][2]) && (side[0][0] < 0.0f) == (side[0][1] < 0.0f) && (side[0][0] < 0.0f) == (side[0][2] < 0.0f)) {
	...
	if ((range[0].max > range[1].min) && (range[0].min < range[1].max)) {
		 ...
		 * Now identify the two points of intersection that are in the middle to get
		 * the actual intersection between the triangles. (B--C from A--B--C--D) */
...
int isect_line_line_epsilon_v3(const float v1[3], const float v2[3], const float v3[3], const float v4[3], float r_i1[3], float r_i2[3], const float epsilon) {
	...
	/* for the second line, just subtract the offset from the first intersection
	 * point */
...
bool isect_ray_ray_epsilon_v3(const float ray_origin_a[3], const float ray_direction_a[3], const float ray_origin_b[3], const float ray_direction_b[3], const float epsilon, float *r_lambda_a, float *r_lambda_b) {
	ROSE_assert(r_lambda_a || r_lambda_b);
	float n[3];
...
bool isect_ray_ray_v3(const float ray_origin_a[3], const float ray_direction_a[3], const float ray_origin_b[3], const float ray_direction_b[3], float *r_lambda_a, float *r_lambda_b) {
	return isect_ray_ray_epsilon_v3(ray_origin_a, ray_direction_a, ray_origin_b, ray_direction_b, FLT_MIN, r_lambda_a, r_lambda_b);
}

bool isect_aabb_aabb_v3(const float min1[3], const float max1[3], const float min2[3], const float max2[3]) {
	return (min1[0] < max2[0] && min1[1] < max2[1] && min1[2] < max2[2] && min2[0] < max1[0] && min2[1] < max1[1] && min2[2] < max1[2]);
}
...
static bool point_in_slice(const float p[3], const float v1[3], const float l1[3], const float l2[3]) {
	 ...
	 * define a subset of R3 delimited by planes parallel to the line and
	 * orthogonal to the (point --> line) distance vector, one plane on the line
	 * one on the point, the room inside usually is rather small compared to R3
	 * though still infinite useful for restricting (speeding up) searches e.g.
	 * all points of triangular prism are within the intersection of 3 'slices'
	 * another trivial case : cube
	 * but see a 'spat' which is a deformed cube with paired parallel planes needs
	 * only 3 slices too
	 */
...
bool isect_point_tri_v3(const float p[3], const float v1[3], const float v2[3], const float v3[3], float r_isect_co[3]) {
	if (isect_point_tri_prism_v3(p, v1, v2, v3)) {
		...
		ROSE_assert(len_squared_v3(no) != 0.0f);
...
void axis_dominant_v3_to_m3(float r_mat[3][3], const float normal[3]) {
	...
	ROSE_assert(!is_negative_m3(r_mat));
	ROSE_assert((fabsf(dot_m3_v3_row_z(r_mat, normal) - 1.0f) < FLT_EPSILON) || is_zero_v3(normal));
}
...
void axis_dominant_v3_to_m3_negate(float r_mat[3][3], const float normal[3]) {
	...
	ROSE_assert(!is_negative_m3(r_mat));
	ROSE_assert((dot_m3_v3_row_z(r_mat, normal) < FLT_EPSILON) || is_zero_v3(normal));
}
...
#if defined(DEBUG) || defined(_DEBUG) /* Avoid floating point exception when debugging. */
	if (wtot != 0.0f)
#endif
	{
...
int barycentric_inside_triangle_v2(const float w[3]) {
	...
	if (IN_RANGE_INCL(w[0], 0.0f, 1.0f) && IN_RANGE_INCL(w[1], 0.0f, 1.0f) && IN_RANGE_INCL(w[2], 0.0f, 1.0f)) {
		return 2;
	}
...
void barycentric_weights_v2_quad(const float v1[2], const float v2[2], const float v3[2], const float v4[2], const float co[2], float w[4]) {
	 ...
	 * gives unreliable results without adding `absf()`. If this becomes an issue
	 * for more general usage we could have this optional or use a different
	 * function. */
#define MEAN_VALUE_HALF_TAN_V2(_area, i1, i2) ((_area = cross_v2v2(dirs[i1], dirs[i2])) != 0.0f ? fabsf(((lens[i1] * lens[i2]) - dot_v2v2(dirs[i1], dirs[i2])) / _area) : 0.0f)
...
void transform_point_by_tri_v3(float pt_tar[3], float const pt_src[3], const float tri_tar_p1[3], const float tri_tar_p2[3], const float tri_tar_p3[3], const float tri_src_p1[3], const float tri_src_p2[3], const float tri_src_p3[3]) {
	 ...
	 * axis where its barycentric weights can be calculated in 2D and its Z offset
	 * can be re-applied. The weights are applied directly to the targets 3D
	 * points and the z-depth is used to scale the targets normal as an offset.
...
int interp_sparse_array(float *array, const int list_size, const float skipval) {
	...
	float *array_up = (float *)MEM_callocN(sizeof(float) * (size_t)list_size, "interp_sparse_array up");
	float *array_down = (float *)MEM_callocN(sizeof(float) * (size_t)list_size, "interp_sparse_array up");

	int *ofs_tot_up = (int *)MEM_callocN(sizeof(int) * (size_t)list_size, "interp_sparse_array tup");
	int *ofs_tot_down = (int *)MEM_callocN(sizeof(int) * (size_t)list_size, "interp_sparse_array tdown");
	...
	for (i = 0; i < list_size; i++) {
		if (array[i] == skipval) {
			if (array_up[i] != skipval && array_down[i] != skipval) {
				array[i] = ((array_up[i] * (float)ofs_tot_down[i]) + (array_down[i] * (float)ofs_tot_up[i])) / (float)(ofs_tot_down[i] + ofs_tot_up[i]);
			}
...
void interp_weights_poly_v3(float *w, float v[][3], const int n, const float co[3]) {
	/* Before starting to calculate the weight, we need to figure out the floating
	 * point precision we can expect from the supplied data. */
	float max_value = 0;
	...
	/* These to values we derived by empirically testing different values that
	 * works for the test files in D7772. */
	...
	while (i_next < n) {
		/* Mark Mayer et al algorithm that is used here does not operate well if
		 * vertex is close to borders of face. In that case, do simple linear
		 * interpolation between the two edge vertices */
...
void resolve_tri_uv_v2(float r_uv[2], const float st[2], const float st0[2], const float st1[2], const float st2[2]) {
	...
	/* det should never be zero since the determinant is the signed ST area of the
	 * triangle. */
	if (IS_ZERO(det) == 0) {
...
void resolve_quad_uv_v2_deriv(float r_uv[2], float r_deriv[2][2], const float st[2], const float st0[2], const float st1[2], const float st2[2], const float st3[2]) {
	const double signed_area = (st0[0] * st1[1] - st0[1] * st1[0]) + (st1[0] * st2[1] - st1[1] * st2[0]) + (st2[0] * st3[1] - st2[1] * st3[0]) + (st3[0] * st0[1] - st3[1] * st0[0]);
	...
	const double b = 0.5 * (double)(((st0[0] - st[0]) * (st1[1] - st2[1]) - (st0[1] - st[1]) * (st1[0] - st2[0])) + ((st1[0] - st[0]) * (st0[1] - st3[1]) - (st1[1] - st[1]) * (st0[0] - st3[0])));
...
void perspective_m4_fov(float mat[4][4], const float angle_left, const float angle_right, const float angle_up, const float angle_down, const float nearClip, const float farClip) {
	...
	perspective_m4(mat, tan_angle_left, tan_angle_right, tan_angle_top, tan_angle_bottom, nearClip, farClip);
	mat[0][0] /= nearClip;
	mat[1][1] /= nearClip;
}
...
static void i_multmatrix(const float icand[4][4], float mat[4][4]) {
	...
	for (row = 0; row < 4; row++) {
		for (col = 0; col < 4; col++) {
			temp[row][col] = (icand[row][0] * mat[0][col] + icand[row][1] * mat[1][col] + icand[row][2] * mat[2][col] + icand[row][3] * mat[3][col]);
		}
	}
	copy_m4_m4(mat, temp);
}
...
void vcloud_estimate_transform_v3(const int list_size, const float (*pos)[3], const float *weight, const float (*rpos)[3], const float *rweight, float lloc[3], float rloc[3], float lrot[3][3], float lscale[3][3]) {
	...
	if (pos && rpos && (list_size > 0)) { /* paranoia check */
		...
		if (lrot || lscale) { /* caller does not want rot nor scale, strange but legal */
			...
			/* This is pretty much Polar-decompose 'inline' the algorithm based on
			 * Higham's thesis without the far case ... but seems to work here pretty
			 * neat. */
			odet = 0.0f;
...
#	include "roselib/roselib_simd.h"
...
bool is_quad_convex_v3(const float v1[3], const float v2[3], const float v3[3], const float v4[3]) {
	/**
	 * Method projects points onto a plane and checks its convex using following
	 * method:
	 *
	 * - Create a plane from the cross-product of both diagonal vectors.
	 * - Project all points onto the plane.
	 * - Subtract for direction vectors.
	 * - Return true if all corners cross-products point the direction of the
	 * plane.
	 */
...
#define CROSS_SIGN(dir_a, dir_b) ((void)cross_v3_v3v3(test_dir, dir_a, dir_b), (dot_v3v3(plane, test_dir) > 0.0f))
	...
	return (CROSS_SIGN(quad_dirs[0], quad_dirs[1]) && CROSS_SIGN(quad_dirs[1], quad_dirs[2]) && CROSS_SIGN(quad_dirs[2], quad_dirs[3]) && CROSS_SIGN(quad_dirs[3], quad_dirs[0]));
...
float geodesic_distance_propagate_across_triangle(const float v0[3], const float v1[3], const float v2[3], const float dist1, const float dist2) {
	...
	if (dist1 != 0.0f && dist2 != 0.0f) {
		...
		if (d12 * d12 > 0.0f) {
			...
			/* Compute virtual source point in local coordinates, that we estimate the
			 * geodesic distance is being computed from. See figure 9 in the paper for
			 * the derivation. */
