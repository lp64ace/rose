...
float dist_signed_squared_to_corner_v3v3v3(const float p[3], const float v1[3], const float v2[3], const float v3[3], const float axis_ref[3]) {
	...
	if (flip) {
		return ROSE_MIN(dist_a, dist_b);
	}

	return ROSE_MAX(dist_a, dist_b);
}
...
float dist_squared_ray_to_seg_v3(const float ray_origin[3], const float ray_direction[3], const float v0[3], const float v1[3], float r_point[3], float *r_depth) {
	...
	return len_squared_v3(dvec) - depth * depth;
}
...
DistRayAABB_Precalc dist_squared_ray_to_aabb_v3_precalc(const float ray_origin[3], const float ray_direction[3]) {
	...
	memset(&nearest_precalc, 0, sizeof(DistRayAABB_Precalc));
	copy_v3_v3(nearest_precalc.ray_origin, ray_origin);
	copy_v3_v3(nearest_precalc.ray_direction, ray_direction);

	for (int i = 0; i < 3; i++) {
		nearest_precalc.ray_inv_dir[i] = (nearest_precalc.ray_direction[i] != 0.0f) ? (1.0f / nearest_precalc.ray_direction[i]) : FLT_MAX;
	}
...
void dist_squared_to_projected_aabb_precalc(DistProjectedAABBPrecalc *precalc, const float projmat[4][4], const float winsize[2], const float mval[2]) {
	...
	if (!isect_plane_plane_plane_v3(projmat_trans[0], projmat_trans[1], projmat_trans[3], precalc->ray_origin)) {
...
int isect_seg_seg_v2_point_ex(const float v0[2], const float v1[2], const float v2[2], const float v3[2], const float endpoint_bias, float r_vi[2]) {
	...
	if ((cross_v2v2(s10, s30) == 0.0f) && (cross_v2v2(s32, s30) == 0.0f)) {
		...
		if (equals_v2_v2(v0, v1)) {
			if (len_squared_v2v2(v2, v3) > eps * eps) {
				...
				if (equals_v2_v2(v0, v2)) { /* points are equal */
					copy_v2_v2(r_vi, v0);
					return 1;
				}
		...
		if (ROSE_MAX(0.0f, u_a) == ROSE_MIN(1.0f, u_b)) {
			/* one common point: can return result */
			madd_v2_v2v2fl(r_vi, v0, s10, ROSE_MAX(0, u_a));
			return 1;
		}
	}
...
bool isect_tri_tri_v3_ex(const float tri_a[3][3], const float tri_b[3][3], float r_i1[3], float r_i2[3], int *r_tri_a_edge_isect_count) {
	...
	*r_tri_a_edge_isect_count = 0;
	...
	plane_a[3] = -dot_v3v3(plane_a, tri_a[1]);
	side[1][0] = dot_v3v3(plane_a, tri_b[0]) + plane_a[3];
	side[1][1] = dot_v3v3(plane_a, tri_b[1]) + plane_a[3];
	side[1][2] = dot_v3v3(plane_a, tri_b[2]) + plane_a[3];
	...
	plane_b[3] = -dot_v3v3(plane_b, tri_b[1]);
	side[0][0] = dot_v3v3(plane_b, tri_a[0]) + plane_b[3];
	side[0][1] = dot_v3v3(plane_b, tri_a[1]) + plane_b[3];
	side[0][2] = dot_v3v3(plane_b, tri_a[2]) + plane_b[3];
...
bool isect_ray_aabb_v3_simple(const float orig[3], const float dir[3], const float bb_min[3], const float bb_max[3], float *tmin, float *tmax) {
	double t[6];
	float hit_dist[2];
	const float invdirx = (dir[0] > FLT_EPSILON || dir[0] < -FLT_EPSILON) ? 1.0 / dir[0] : DBL_MAX;
	const float invdiry = (dir[1] > FLT_EPSILON || dir[1] < -FLT_EPSILON) ? 1.0 / dir[1] : DBL_MAX;
	const float invdirz = (dir[2] > FLT_EPSILON || dir[2] < -FLT_EPSILON) ? 1.0 / dir[2] : DBL_MAX;
	t[0] = (bb_min[0] - orig[0]) * invdirx;
	t[1] = (bb_max[0] - orig[0]) * invdirx;
	t[2] = (bb_min[1] - orig[1]) * invdiry;
	t[3] = (bb_max[1] - orig[1]) * invdiry;
	t[4] = (bb_min[2] - orig[2]) * invdirz;
	t[5] = (bb_max[2] - orig[2]) * invdirz;
	hit_dist[0] = (ROSE_MAX(ROSE_MAX(ROSE_MIN(t[0], t[1]), ROSE_MIN(t[2], t[3])), ROSE_MIN(t[4], t[5])));
	hit_dist[1] = (ROSE_MIN(ROSE_MIN(ROSE_MAX(t[0], t[1]), ROSE_MAX(t[2], t[3])), ROSE_MAX(t[4], t[5])));
...
void interp_weights_quad_v3(float w[4], const float v1[3], const float v2[3], const float v3[3], const float v4[3], const float co[3]) {
	...
	if (equals_v3_v3(co, v1)) {
		w[0] = 1.0f;
	}
	else if (equals_v3_v3(co, v2)) {
		w[1] = 1.0f;
	}
	else if (equals_v3_v3(co, v3)) {
		w[2] = 1.0f;
	}
	else if (equals_v3_v3(co, v4)) {
		w[3] = 1.0f;
	}
	else {
...
void barycentric_weights_v2_clamped(const float v1[2], const float v2[2], const float v3[2], const float co[2], float w[3]) {
	...
	w[0] = ROSE_MAX(cross_tri_v2(v2, v3, co), 0.0f);
	w[1] = ROSE_MAX(cross_tri_v2(v3, v1, co), 0.0f);
	w[2] = ROSE_MAX(cross_tri_v2(v1, v2, co), 0.0f);
	wtot = w[0] + w[1] + w[2];
...
#define MEAN_VALUE_HALF_TAN_V2(_area, i1, i2) ((_area = cross_v2v2(dirs[i1], dirs[i2])) != 0.0f ? fabsf(((lens[i1] * lens[i2]) - dot_v2v2(dirs[i1], dirs[i2])) / _area) : 0.0f)
	...
	if (lens[0] < FLT_EPSILON) {
		w[0] = 1.0f;
		w[1] = w[2] = w[3] = 0.0f;
	}
	else if (lens[1] < FLT_EPSILON) {
		w[1] = 1.0f;
		w[0] = w[2] = w[3] = 0.0f;
	}
	else if (lens[2] < FLT_EPSILON) {
		w[2] = 1.0f;
		w[0] = w[1] = w[3] = 0.0f;
	}
	else if (lens[3] < FLT_EPSILON) {
		w[3] = 1.0f;
		w[0] = w[1] = w[2] = 0.0f;
	}
	else {
...
typedef struct Float3_Len {
	float dir[3], len;
} Float3_Len;

typedef struct Double2_Len {
	double dir[2], len;
} Double2_Len;
...
void interp_weights_poly_v3(float *w, float v[][3], const int n, const float co[3]) {
	...
	for (int i = 0; i < n; i++) {
		max_value = ROSE_MAX(max_value, fabsf(v[i][0] - co[0]));
		max_value = ROSE_MAX(max_value, fabsf(v[i][1] - co[1]));
		max_value = ROSE_MAX(max_value, fabsf(v[i][2] - co[2]));
	}
	...
	while (i_next < n) {
		...
		if (dist_squared_to_line_segment_v3(co, v_curr, v_next) < eps_sq) {
...
void interp_weights_poly_v2(float *w, float v[][2], const int n, const float co[2]) {
	...
	while (i_next < n) {
		...
		if (dist_squared_to_line_segment_v2(co, v_curr, v_next) < eps_sq) {
			ix_flag = IS_SEGMENT_IX;
			break;
		}
		...
		w[i_curr] = (d_curr.len == 0.0) ? 0.0f : ((ht_prev + ht) / d_curr.len);
		totweight += w[i_curr];
...
void resolve_tri_uv_v2(float r_uv[2], const float st[2], const float st0[2], const float st1[2], const float st2[2]) {
	...
	if (IS_ZERO(det) == 0) {
		...
		r_uv[0] = ((d * x[0] - b * x[1]) / det);
		r_uv[1] = (((-c) * x[0] + a * x[1]) / det);
	}
	else {
		zero_v2(r_uv);
	}
...
void resolve_quad_uv_v2_deriv(float r_uv[2], float r_deriv[2][2], const float st[2], const float st0[2], const float st1[2], const float st2[2], const float st3[2]) {
	...
	const double b = 0.5 * (((st0[0] - st[0]) * (st1[1] - st2[1]) - (st0[1] - st[1]) * (st1[0] - st2[0])) + ((st1[0] - st[0]) * (st0[1] - st3[1]) - (st1[1] - st[1]) * (st0[0] - st3[0])));
	 ...
	 * fST = (1-u)(1-v) * ST0 + u * (1-v) * ST1 + u * v * ST2 + (1-u) * v * ST3 */
	{
		...
		if (IS_ZERO(denom) == 0) {
			r_uv[1] = (((1.0f - r_uv[0]) * (st0[i] - st[i]) + r_uv[0] * (st1[i] - st[i])) / denom);
		}
	}
...
void projmat_from_subregion(const float projmat[4][4], const int win_size[2], const int x_min, const int x_max, const int y_min, const int y_max, float r_projmat[4][4]) {
	float rect_width = (x_max - x_min);
	float rect_height = (y_max - y_min);

	float x_sca = ((float)win_size[0]) / rect_width;
	float y_sca = ((float)win_size[1]) / rect_height;

	float x_fac = ((float)(x_min + x_max) - (float)win_size[0]) / rect_width;
	float y_fac = ((float)(y_min + y_max) - (float)win_size[1]) / rect_height;
...
bool map_to_tube(float *r_u, float *r_v, const float x, const float y, const float z) {
	...
	if (x * x + y * y < 1e-6f * 1e-6f) {
		...
		*r_u = snap_coordinate(atan2f(x, -y) / (2.0f * M_PI));
	}
	*r_v = (z + 1.0f) / 2.0f;
	return regular;
}
...
bool map_to_sphere(float *r_u, float *r_v, const float x, const float y, const float z) {
	...
	*r_v = snap_coordinate(atan2f(len_xy, -z) / (M_PI));
	return regular;
}
...
void accumulate_vertex_normals_tri_v3(float n1[3], float n2[3], float n3[3], const float f_no[3], const float co1[3], const float co2[3], const float co3[3]) {
	...
	/* accumulate angle weighted face normal */
	{
		...
		for (i = 0; i < nverts; i++) {
			...
			const float x = -dot_v3v3(cur_edge, prev_edge);
			...
			const float fac = x < 0.0f ? M_PI - a : a;
...
float geodesic_distance_propagate_across_triangle(const float v0[3], const float v1[3], const float v2[3], const float dist1, const float dist2) {
	...
	return ROSE_MIN(dist1 + len_v3(v10), dist2 + len_v3v3(v0, v2));
}
