 ...
 *
 * NOTE: Are to be called ONLY from inside `#if ROSE_HAVE_SSE2` !!!
 * \{ */
...
MALWAYS_INLINE __m128 _bli_math_rsqrt(__m128 in) {
	 ...
	 * emulation. */
#		if defined(__SSE2__)
	r = _mm_add_ps(_mm_mul_ps(_mm_set1_ps(1.5f), r), _mm_mul_ps(_mm_mul_ps(_mm_mul_ps(in, _mm_set1_ps(-0.5f)), r), _mm_mul_ps(r, r)));
#		endif
	return r;
}
...
MALWAYS_INLINE __m128 linearrgb_to_srgb_v4_simd(const __m128 c) {
	...
	__m128 gte = _mm_add_ps(_mm_mul_ps(_mm_set1_ps(1.055f), _bli_math_fastpow512(c)), _mm_set1_ps(-0.055f));
	return _bli_math_blend_sse(cmp, lt, gte);
}
...
extern float ROSE_color_from_srgb_table[256];
extern unsigned short ROSE_color_to_srgb_table[0x10000];
...
#	ifdef __BIG_ENDIAN__
	return ROSE_color_to_srgb_table[tmp.us[0]];
#	else
	return ROSE_color_to_srgb_table[tmp.us[1]];
#	endif
}
...
MINLINE void srgb_to_linearrgb_uchar4(float linear[4], const unsigned char srgb[4]) {
	linear[0] = ROSE_color_from_srgb_table[srgb[0]];
	linear[1] = ROSE_color_from_srgb_table[srgb[1]];
	linear[2] = ROSE_color_from_srgb_table[srgb[2]];
...
MINLINE float dither_random_value(float s, float t) {
	 ...
	 * See Banding in Games:A Noisy Rant(revision 5) Mikkel Gjol, Playdead (slide 27) */
