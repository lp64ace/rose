...
uint LIB_ghashutil_strhash_p_murmur(const void *ptr) {
	...
	return LIB_hash_mm2(key, LIB_strlen((const char *)key) + 1, 0);
}
...
GHash *LIB_ghash_ptr_new_ex(const char *info, uint reserve) {
...
GHash *LIB_ghash_str_new_ex(const char *info, uint reserve) {
...
GSet *LIB_gset_ptr_new_ex(const char *info, uint reserve) {
...
GSet *LIB_gset_str_new_ex(const char *info, uint reserve) {
