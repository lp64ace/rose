...
GVectorArray::GVectorArray(const CPPType &type, const size_t array_size) : type_(type), element_size_(type.size()), items_(array_size) {
}
...
void GVectorArray::append(const size_t index, const void *value) {
...
void GVectorArray::extend(const size_t index, const GVArray &values) {
	ROSE_assert(values.type() == type_);
...
GMutableSpan GVectorArray::operator[](const size_t index) {
	Item &item = items_[index];
	return GMutableSpan{type_, item.start, item.length};
}

GSpan GVectorArray::operator[](const size_t index) const {
	const Item &item = items_[index];
	return GSpan{type_, item.start, item.length};
}

void GVectorArray::realloc_to_at_least(Item &item, size_t min_capacity) {
	const size_t new_capacity = std::max(min_capacity, item.length * 2);
