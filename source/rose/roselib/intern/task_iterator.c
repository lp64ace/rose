...
ROSE_INLINE void task_parallel_calc_chunk_size(const TaskParallelSettings *settings, const int items_num, int tasks_num, int *r_chunk_size) {
	int chunk_size = 0;
	...
	if (!settings->use_threading) {
		 ...
		 * The idea here is to increase the chunk size to compensate for a rather measurable
		 * threading overhead caused by fetching tasks. With too many CPU threads we are starting to
		 * spend too much time in those overheads. First values are: 1 if tasks_num < 16; else 2 if
		 * tasks_num < 32; else 3 if tasks_num < 48; else 4 if tasks_num < 64; etc. NOTE: If we
		 * wanted to keep the 'power of two' multiplier, we'd need something like: 1 << ROSE_MAX(0,
		 * (int)(sizeof(int) * 8) - 1 - bitscan_reverse_i(tasks_num) - 3)
		 */
		const int tasks_num_factor = ROSE_MAX(1, tasks_num >> 3);

		/* We could make that 'base' 32 number configurable in TaskParallelSettings too, or maybe
		 * just always use that heuristic using TaskParallelSettings.min_iter_per_thread as basis?
		 */
		...
		if (items_num > 0 && items_num < ROSE_MAX(256, chunk_size * 2)) {
			chunk_size = items_num;
		}
	}

	ROSE_assert(chunk_size > 0);
...
static void parallel_iterator_func_do(TaskParallelIteratorState *__restrict state, void *userdata_chunk) {
	...
	const size_t indices_size = sizeof(*current_chunk_indices) * (size_t)state->iter_shared.chunk_size;
	...
	for (bool do_abort = false; !do_abort;) {
		if (state->iter_shared.spin_lock != NULL) {
			LIB_spin_lock(state->iter_shared.spin_lock);
		}
		...
		if (state->iter_shared.spin_lock != NULL) {
			LIB_spin_unlock(state->iter_shared.spin_lock);
		}
...
static void parallel_iterator_func(TaskPool *__restrict pool, void *userdata_chunk) {
	TaskParallelIteratorState *__restrict state = LIB_task_pool_user_data(pool);
...
static void task_parallel_iterator_do(const TaskParallelSettings *settings, TaskParallelIteratorState *state) {
	...
	task_parallel_calc_chunk_size(settings, state->items_num, threads_num, &state->iter_shared.chunk_size);
	...
	const size_t tasks_num = items_num >= 0 ? (size_t)ROSE_MIN(threads_num, state->items_num / chunk_size) : (size_t)threads_num;

	ROSE_assert(tasks_num > 0);
	if (tasks_num == 1) {
		task_parallel_iterator_no_threads(settings, state);
		return;
	}
	...
	for (size_t i = 0; i < tasks_num; i++) {
		...
		LIB_task_pool_push(task_pool, parallel_iterator_func, userdata_chunk_local, false, NULL);
	}
	...
	LIB_spin_end(&spin_lock);
	state->iter_shared.spin_lock = NULL;
}

void LIB_task_parallel_iterator(void *userdata, TaskParallelIteratorIterFunc iter_func, void *init_item, const int init_index, const int items_num, TaskParallelIteratorFunc func, const TaskParallelSettings *settings) {
...
static void task_parallel_listbase_get(void *__restrict userdata, const TaskParallelTLS *__restrict tls, void **r_next_item, int *r_next_index, bool *r_do_abort) {
	/* Get current status. */
...
void LIB_task_parallel_listbase(ListBase *listbase, void *userdata, TaskParallelIteratorFunc func, const TaskParallelSettings *settings) {
	if (LIB_listbase_is_empty(listbase)) {
		return;
	}

	TaskParallelIteratorState state = {0};

	state.items_num = LIB_listbase_count(listbase);
...
static void parallel_mempool_func(TaskPool *__restrict pool, void *taskdata) {
	...
	MemPoolThreadSafeIter *iter = &((ParallelMempoolTaskData *)taskdata)->ts_iter;
...
void LIB_task_parallel_mempool(MemPool *mempool, void *userdata, TaskParallelMempoolFunc func, const TaskParallelSettings *settings) {
	if (LIB_memory_pool_length(mempool) == 0) {
		return;
	}
	...
	if (!settings->use_threading) {
		...
		MemPoolIter iter;
		LIB_memory_pool_iternew(mempool, &iter);

		void *item;
		while ((item = LIB_memory_pool_iterstep(&iter))) {
	...
	ParallelMempoolTaskData *mempool_iterator_data = mempool_iter_threadsafe_create(mempool, (size_t)tasks_num);
	...
	for (int i = 0; i < tasks_num; i++) {
		...
		/* Use this pool's pre-allocated tasks. */
		LIB_task_pool_push(task_pool, parallel_mempool_func, &mempool_iterator_data[i], false, NULL);
	}
		...
		if ((settings->func_free != NULL) || (settings->func_reduce != NULL)) {
			for (int i = 0; i < tasks_num; i++) {
				if (settings->func_reduce) {
					settings->func_reduce(userdata, userdata_chunk, mempool_iterator_data[i].tls.userdata_chunk);
				}
