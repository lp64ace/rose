...
static bool zstd_read_seek_table(ZstdReader *zstd) {
	...
	zstd->seek.compressed_ofs = static_cast<size_t *>(MEM_mallocN(sizeof(size_t) * (frames_num + 1), __func__));
	zstd->seek.uncompressed_ofs = static_cast<size_t *>(MEM_mallocN(sizeof(size_t) * (frames_num + 1), __func__));
...
static const char *zstd_ensure_cache(ZstdReader *zstd, int frame) {
	...
	size_t uncompressed_size = zstd->seek.uncompressed_ofs[frame + 1] - zstd->seek.uncompressed_ofs[frame];

	char *uncompressed_data = static_cast<char *>(MEM_mallocN(sizeof(char) * uncompressed_size, __func__));
	char *compressed_data = static_cast<char *>(MEM_mallocN(sizeof(char) * compressed_size, __func__));
	if (zstd->base->seek(zstd->base, zstd->seek.compressed_ofs[frame], SEEK_SET) < 0 || zstd->base->read(zstd->base, compressed_data, compressed_size) < compressed_size) {
		MEM_freeN(compressed_data);
		MEM_freeN(uncompressed_data);
		return nullptr;
	}

	size_t res = ZSTD_decompressDCtx(zstd->ctx, uncompressed_data, uncompressed_size, compressed_data, compressed_size);
	MEM_freeN(compressed_data);
...
static uint64_t zstd_read_seekable(FileReader *reader, void *buffer, size_t size) {
...
static uint64_t zstd_seek(FileReader *reader, uint64_t offset, int whence) {
	ZstdReader *zstd = reinterpret_cast<ZstdReader *>(reader);
	uint64_t new_pos;
	if (whence == SEEK_SET) {
		new_pos = offset;
...
static uint64_t zstd_read(FileReader *reader, void *buffer, size_t size) {
	ZstdReader *zstd = reinterpret_cast<ZstdReader *>(reader);
	...
	while (output.pos < output.size) {
		if (zstd->in_buf.pos == zstd->in_buf.size) {
			...
			zstd->in_buf.pos = 0;
			uint64_t readsize = zstd->base->read(zstd->base, static_cast<char *>(const_cast<void *>(zstd->in_buf.src)), zstd->in_buf_max_size);
...
FileReader *LIB_filereader_new_zstd(FileReader *base) {
	ZstdReader *zstd = MEM_new<ZstdReader>("ZstdReader");
