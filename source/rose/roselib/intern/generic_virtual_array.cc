...
void GVArrayImpl::materialize(const IndexMask &mask, void *dst) const {
	mask.foreach_index_optimized<size_t>([&](const size_t i) {
...
void GVArrayImpl::materialize_compressed(const IndexMask &mask, void *dst) const {
	mask.foreach_index_optimized<size_t>([&](const size_t i, const size_t pos) {
...
void GVArrayImpl::get(const size_t index, void *r_value) const {
...
void GVMutableArrayImpl::set_by_copy(const size_t index, const void *value) {
...
void GVMutableArrayImpl::set_by_relocate(const size_t index, void *value) {
...
void GVMutableArray::fill(const void *value) {
	...
	if (info.type == CommonVArrayInfo::Type::Span) {
		...
		for (size_t i : IndexRange(this->size())) {
...
void GVArrayImpl_For_GSpan::get(const size_t index, void *r_value) const {
	type_->copy_assign(POINTER_OFFSET(data_, element_size_ * index), r_value);
}

void GVArrayImpl_For_GSpan::get_to_uninitialized(const size_t index, void *r_value) const {
	type_->copy_construct(POINTER_OFFSET(data_, element_size_ * index), r_value);
}

void GVArrayImpl_For_GSpan::set_by_copy(const size_t index, const void *value) {
	type_->copy_assign(value, POINTER_OFFSET(data_, element_size_ * index));
}

void GVArrayImpl_For_GSpan::set_by_move(const size_t index, void *value) {
	type_->move_construct(value, POINTER_OFFSET(data_, element_size_ * index));
}

void GVArrayImpl_For_GSpan::set_by_relocate(const size_t index, void *value) {
...
void GVArrayImpl_For_GSpan::materialize_compressed_to_uninitialized(const IndexMask &mask, void *dst) const {
...
void GVArrayImpl_For_SingleValueRef::get(const size_t index, void *r_value) const {
	type_->copy_assign(value_, r_value);
}
void GVArrayImpl_For_SingleValueRef::get_to_uninitialized(const size_t index, void *r_value) const {
...
void GVArrayImpl_For_SingleValueRef::materialize_to_uninitialized(const IndexMask &mask, void *dst) const {
...
void GVArrayImpl_For_SingleValueRef::materialize_compressed_to_uninitialized(const IndexMask &mask, void *dst) const {
...
public:
	GVArrayImpl_For_SingleValue(const CPPType &type, const size_t size, const void *value) : GVArrayImpl_For_SingleValueRef(type, size) {
...
public:
	GVArrayImpl_For_SmallTrivialSingleValue(const CPPType &type, const size_t size, const void *value) : GVArrayImpl(type, size) {
		ROSE_assert(type.is_trivial());
		ROSE_assert(type.alignment() <= 8);
		ROSE_assert(type.size() <= BufferSize);
		type.copy_construct(value, &buffer_);
	}

private:
	void get(const size_t index, void *r_value) const final {
		this->get_to_uninitialized(index, r_value);
	}
	void get_to_uninitialized(const size_t index, void *r_value) const final {
...
GVArraySpan::GVArraySpan(GVArray varray) : GSpan(varray ? &varray.type() : nullptr), varray_(std::move(varray)) {
	if (!varray_) {
		return;
	}
...
GVArraySpan::GVArraySpan(GVArraySpan &&other) : GSpan(other.type_ptr()), varray_(std::move(other.varray_)), owned_data_(other.owned_data_) {
	if (!varray_) {
		return;
	}
...
GMutableVArraySpan::GMutableVArraySpan(GVMutableArray varray, const bool copy_values_to_span) : GMutableSpan(varray ? &varray.type() : nullptr), varray_(std::move(varray)) {
	if (!varray_) {
		return;
	}
	size_ = varray_.size();
...
GMutableVArraySpan::GMutableVArraySpan(GMutableVArraySpan &&other) : GMutableSpan(other.type_ptr()), varray_(std::move(other.varray_)), owned_data_(other.owned_data_), show_not_saved_warning_(other.show_not_saved_warning_) {
	if (!varray_) {
		return;
	}
	size_ = varray_.size();
...
public:
	GVArrayImpl_For_SlicedGVArray(GVArray varray, const IndexRange slice) : GVArrayImpl(varray.type(), slice.size()), varray_(std::move(varray)), offset_(slice.start()), slice_(slice) {
		ROSE_assert(slice.one_after_last() <= varray_.size());
	}
	...
	CommonVArrayInfo common_info() const override {
		...
		switch (internal_info.type) {
			...
			case CommonVArrayInfo::Type::Span: {
				return CommonVArrayInfo(CommonVArrayInfo::Type::Span, internal_info.may_have_ownership, POINTER_OFFSET(internal_info.data, type_->size() * offset_));
			}
...
void GVArrayCommon::materialize_to_uninitialized(const IndexMask &mask, void *dst) const {
	ROSE_assert(mask.min_array_size() <= impl_->size());
...
GVArray::GVArray(varray_tag::single tag, const CPPType &type, size_t size, const void *value) {
...
GVArray GVArray::ForSingle(const CPPType &type, const size_t size, const void *value) {
	return GVArray(varray_tag::single{}, type, size, value);
}

GVArray GVArray::ForSingleRef(const CPPType &type, const size_t size, const void *value) {
	return GVArray(varray_tag::single_ref{}, type, size, value);
}

GVArray GVArray::ForSingleDefault(const CPPType &type, const size_t size) {
...
public:
	GVArrayImpl_For_GArray(GArray<> array) : GVArrayImpl_For_GSpan(array.as_mutable_span()), array_(std::move(array)) {
	}
};
...
GVMutableArray::GVMutableArray(std::shared_ptr<GVMutableArrayImpl> impl) : GVArrayCommon(std::move(impl)) {
}
