...
#include "LIB_bit_group_vector.hh"
 ...
 */
constexpr size_t inline_expr_array_size = 16;
...
static void sort_course_boundaries(MutableSpan<CourseBoundary> boundaries) {
	std::sort(boundaries.begin(), boundaries.end(), [](const CourseBoundary &a, const CourseBoundary &b) { return a.index < b.index; });
}

static void sort_course_boundaries(MutableSpan<DifferenceCourseBoundary> boundaries) {
	std::sort(boundaries.begin(), boundaries.end(), [](const DifferenceCourseBoundary &a, const DifferenceCourseBoundary &b) { return a.index < b.index; });
}

/** Smaller segments should generally be merged together. */
static constexpr size_t segment_size_threshold = 32;

/** Extends a previous full segment or appends a new one. */
static CoarseSegment &add_coarse_segment__full(CoarseSegment *prev_segment, const size_t prev_boundary_index, const size_t current_boundary_index, CoarseResult &result) {
	const size_t size = current_boundary_index - prev_boundary_index;
	if (prev_segment) {
		if (prev_segment->type == CoarseSegment::Type::Full && prev_segment->bounds.one_after_last() == prev_boundary_index) {
	...
	result.segments.append({CoarseSegment::Type::Full, IndexRange::from_begin_size(prev_boundary_index, size)});
	return result.segments.last();
}

/** Extends a previous unknown segment or appends a new one. */
static CoarseSegment &add_coarse_segment__unknown(CoarseSegment *prev_segment, const size_t prev_boundary_index, const size_t current_boundary_index, CoarseResult &result) {
	if (prev_segment) {
	...
	result.segments.append({CoarseSegment::Type::Unknown, IndexRange::from_begin_end(prev_boundary_index, current_boundary_index)});
	return result.segments.last();
}

/** Extends a previous copy segment or appends a new one. */
static CoarseSegment &add_coarse_segment__copy(CoarseSegment *prev_segment, const size_t prev_boundary_index, const size_t current_boundary_index, const IndexMask &copy_from_mask, CoarseResult &result) {
	if (prev_segment) {
		if (prev_segment->type == CoarseSegment::Type::Copy && prev_segment->bounds.one_after_last() == prev_boundary_index && prev_segment->mask == &copy_from_mask) {
	...
	result.segments.append({CoarseSegment::Type::Copy, IndexRange::from_begin_end(prev_boundary_index, current_boundary_index), &copy_from_mask});
	return result.segments.last();
}
...
static void evaluate_coarse_union(const Span<CourseBoundary> boundaries, CoarseResult &r_result) {
	...
	size_t prev_boundary_index = boundaries[0].index;
	...
	for (const CourseBoundary &boundary : boundaries) {
		if (prev_boundary_index < boundary.index) {
			...
			if (has_full) {
				prev_segment = &add_coarse_segment__full(prev_segment, prev_boundary_index, boundary.index, result);
			}
			else if (has_unknown || !copy_from_single_mask) {
				prev_segment = &add_coarse_segment__unknown(prev_segment, prev_boundary_index, boundary.index, result);
			}
			else if (copy_from_mask != nullptr && copy_from_single_mask) {
				prev_segment = &add_coarse_segment__copy(prev_segment, prev_boundary_index, boundary.index, *copy_from_mask, result);
			}
...
static void evaluate_coarse_intersection(const Span<CourseBoundary> boundaries, const size_t terms_num, CoarseResult &r_result) {
	if (boundaries.is_empty()) {
		return;
	}
	...
	for (const CourseBoundary &boundary : boundaries) {
		if (prev_boundary_index < boundary.index) {
			...
			if (active_segments.size() == terms_num) {
				...
				/* Determine the resulting coarse segment type based on the properties computed above. */
				ROSE_assert(full_count + unknown_count + copy_count == terms_num);
...
static void evaluate_coarse_difference(const Span<DifferenceCourseBoundary> boundaries, CoarseResult &r_result) {
	...
	for (const DifferenceCourseBoundary &boundary : boundaries) {
		if (prev_boundary_index < boundary.index) {
			...
			ROSE_assert(active_main_segments.size() <= 1);
			if (active_main_segments.size() == 1) {
				...
				if (has_subtract_full) {
					...
					switch (active_main_segment.type) {
						...
						case CoarseSegment::Type::Copy: {
							if (active_subtract_segments.is_empty()) {
								prev_segment = &add_coarse_segment__copy(prev_segment, prev_boundary_index, boundary.index, *active_main_segment.mask, result);
							}
...
static CoarseResult evaluate_coarse(const Expr &root_expression, const Span<const Expr *> eval_order, const std::optional<IndexRange> eval_bounds = std::nullopt) {
	...
	Array<std::optional<CoarseResult>, inline_expr_array_size> expression_results(root_expression.expression_array_size());
...
static Span<int16_t> bits_to_indices(const BoundedBitSpan bits, LinearAllocator<> &allocator) {
	...
	bits::foreach_1_index(bits, [&](const size_t i) {
		ROSE_assert(i < max_segment_size);
...
static IndexMaskSegment evaluate_exact_with_bits(const Expr &root_expression, LinearAllocator<> &allocator, const IndexRange bounds, const Span<const Expr *> eval_order) {
	ROSE_assert(bounds.size() <= max_segment_size);
	const size_t bounds_min = bounds.start();
	...
	const size_t ints_in_bounds = ceil_division(bounds.size(), bits::BitsPerInt);
	BitGroupVector<16 * 1024> expression_results(expr_array_size, ints_in_bounds * bits::BitsPerInt, false);
	...
	for (const Expr *expression : eval_order) {
		...
		switch (expression->type) {
			...
			case Expr::Type::Difference: {
				bits::copy_from_or(expr_result, expression_results[expression->terms[0]->index]);
				for (const Expr *term : expression->terms.as_span().drop_front(1)) {
					bits::mix_into_first_expr([](const bits::BitInt a, const bits::BitInt b) { return a & ~b; }, expr_result, expression_results[term->index]);
				}
				break;
			}
		}
...
static IndexMaskSegment union_index_mask_segments(const Span<IndexMaskSegment> segments, const size_t bounds_min, int16_t *r_values) {
	...
	if (segments.size() == 2) {
		...
		const size_t size = std::set_union(a.begin(), a.end(), b.begin(), b.end(), r_values) - r_values;
	...
	std::sort(sorted_segments.begin(), sorted_segments.end(), [](const IndexMaskSegment &a, const IndexMaskSegment &b) { return a.size() < b.size(); });
	 ...
	 * faster in the end. */
	for (const size_t segment_i : sorted_segments.index_range().drop_front(2)) {
...
static IndexMaskSegment intersect_index_mask_segments(const Span<IndexMaskSegment> segments, const size_t bounds_min, int16_t *r_values) {
	if (segments.is_empty()) {
		return {};
	}
	if (segments.size() == 1) {
	...
	if (segments.size() == 2) {
		const IndexMaskSegment a = segments[0].shift(-bounds_min);
		const IndexMaskSegment b = segments[1].shift(-bounds_min);
		const size_t size = std::set_intersection(a.begin(), a.end(), b.begin(), b.end(), r_values) - r_values;
		return {bounds_min, {r_values, size}};
	}
 ...
 * subtract-segments.
 */
static IndexMaskSegment difference_index_mask_segments(const IndexMaskSegment main_segment, const Span<IndexMaskSegment> subtract_segments, const size_t bounds_min, int16_t *r_values) {
	...
	if (subtract_segments.size() == 1) {
		...
		const size_t size = std::set_difference(shifted_main_segment.begin(), shifted_main_segment.end(), subtract_segment.begin(), subtract_segment.end(), r_values) - r_values;
		return {bounds_min, {r_values, size}};
	}

	size_t subtract_count = 0;
	...
	if (subtract_count < main_segment.size() / 2) {
		...
		const IndexMaskSegment shifted_main_segment = main_segment.shift(-bounds_min);
		const IndexMaskSegment unioned_subtract_segment = union_index_mask_segments(subtract_segments, bounds_min, union_indices.data()).shift(-bounds_min);
		const size_t size = std::set_difference(shifted_main_segment.begin(), shifted_main_segment.end(), unioned_subtract_segment.begin(), unioned_subtract_segment.end(), r_values) - r_values;
		return {bounds_min, {r_values, size}};
	}
	...
	std::sort(sorted_subtract_segments.begin(), sorted_subtract_segments.end(), [](const IndexMaskSegment &a, const IndexMaskSegment &b) { return a.size() > b.size(); });
	...
	size_t count = 0;
	{
		...
		int16_t *dst = buffer_a;
		count = std::set_difference(shifted_main_segment.begin(), shifted_main_segment.end(), subtract_segment.begin(), subtract_segment.end(), dst) - dst;
	}

	for (const size_t segment_i : sorted_subtract_segments.index_range().drop_front(1)) {
		const IndexMaskSegment &subtract_segment = sorted_subtract_segments[segment_i].shift(-bounds_min);
		...
		count = std::set_difference(buffer_a, buffer_a + count, subtract_segment.begin(), subtract_segment.end(), dst) - dst;
		std::swap(buffer_a, buffer_b);
	}
	return {bounds_min, {r_values, count}};
}
	...
	for (const Expr *expression : eval_order) {
		switch (expression->type) {
			case Expr::Type::Atomic: {
				...
				ROSE_assert(mask.segments_num() <= 1);
			...
			case Expr::Type::Union: {
				...
				size_t result_size_upper_bound = 0;
				bool used_short_circuit = false;
				for (const size_t term_i : expr.terms.index_range()) {
				...
				const IndexMaskSegment result_segment = union_index_mask_segments(term_segments, bounds_min, dst.data());
				allocator.free_end_of_previous_allocation(dst.size_in_bytes(), result_segment.base_span().end());
			...
			case Expr::Type::Intersection: {
				const IntersectionExpr &expr = expression->as_intersection();
				Array<IndexMaskSegment> term_segments(expr.terms.size());
				size_t result_size_upper_bound = bounds.size();
				bool used_short_circuit = false;
				for (const size_t term_i : expr.terms.index_range()) {
					const Expr &term = *expr.terms[term_i];
					const IndexMaskSegment term_segment = results[term.index];
				...
				MutableSpan<int16_t> dst = allocator.allocate_array<int16_t>(result_size_upper_bound);
				const IndexMaskSegment result_segment = intersect_index_mask_segments(term_segments, bounds_min, dst.data());
				allocator.free_end_of_previous_allocation(dst.size_in_bytes(), result_segment.base_span().end());
				results[expression->index] = result_segment;
				break;
			}
			case Expr::Type::Difference: {
				...
				size_t result_size_upper_bound = main_segment.size();
				bool used_short_circuit = false;
				Array<IndexMaskSegment> subtract_segments(expr.terms.size() - 1);
				for (const size_t term_i : expr.terms.index_range().drop_front(1)) {
					...
					result_size_upper_bound = std::min(result_size_upper_bound, bounds.size() - term_segment.size());
				...
				MutableSpan<int16_t> dst = allocator.allocate_array<int16_t>(result_size_upper_bound);
				const IndexMaskSegment result_segment = difference_index_mask_segments(main_segment, subtract_segments, bounds_min, dst.data());
				allocator.free_end_of_previous_allocation(dst.size_in_bytes(), result_segment.base_span().end());
				results[expression->index] = result_segment;
				break;
			}
	...
	for (const EvaluatedSegment &evaluated_segment : evaluated_segments) {
		switch (evaluated_segment.type) {
			case EvaluatedSegment::Type::Full: {
				const size_t full_size = evaluated_segment.bounds.size();
				for (size_t i = 0; i < full_size; i += max_segment_size) {
					const size_t size = std::min(i + max_segment_size, full_size) - i;
					result_mask_segments.append(IndexMaskSegment(evaluated_segment.bounds.first() + i, Span(static_indices_array).take_front(size)));
				}
				break;
			}
			case EvaluatedSegment::Type::Copy: {
				const IndexMask sliced_mask = evaluated_segment.copy_mask->slice_content(evaluated_segment.bounds);
				sliced_mask.foreach_segment([&](const IndexMaskSegment &segment) { result_mask_segments.append(segment); });
				break;
			}
...
static Vector<const Expr *, inline_expr_array_size> compute_eval_order(const Expr &root_expression) {
	...
	Array<bool, inline_expr_array_size> is_evaluated_states(root_expression.expression_array_size(), false);
...
static void evaluate_coarse_and_split_until_segments_are_short(const Expr &root_expression, const Span<const Expr *> eval_order, Vector<EvaluatedSegment, 16> &r_evaluated_segments, Vector<IndexRange, 16> &r_short_unknown_segments) {
	...
	const size_t coarse_segment_size_threshold = max_segment_size;
	...
	auto handle_coarse_result = [&](const CoarseResult &coarse_result) {
		for (const CoarseSegment &segment : coarse_result.segments) {
			switch (segment.type) {
				...
				case CoarseSegment::Type::Copy: {
					ROSE_assert(segment.mask);
					r_evaluated_segments.append({EvaluatedSegment::Type::Copy, segment.bounds, segment.mask});
					break;
				}
				case CoarseSegment::Type::Full: {
	...
	while (!long_unknown_segments.is_empty()) {
		...
		const size_t split_pos = unknown_bounds.size() / 2;
...
static void evaluate_short_unknown_segments_exactly(const Expr &root_expression, const ExactEvalMode exact_eval_mode, const Span<const Expr *> eval_order, const Span<IndexRange> short_unknown_segments, IndexMaskMemory &memory, Vector<EvaluatedSegment, 16> &r_evaluated_segments) {
	...
	auto evaluate_unknown_segment = [&](const IndexRange bounds, LinearAllocator<> &allocator, Vector<EvaluatedSegment, 16> &r_local_evaluated_segments) {
		/* Use the predetermined evaluation mode. */
		switch (exact_eval_mode) {
			case ExactEvalMode::Bits: {
				const IndexMaskSegment indices = evaluate_exact_with_bits(root_expression, allocator, bounds, eval_order);
				if (!indices.is_empty()) {
					r_local_evaluated_segments.append({EvaluatedSegment::Type::Indices, bounds, nullptr, indices});
				}
				break;
			}
			case ExactEvalMode::Indices: {
				...
				Vector<size_t, 16> split_indices;
				...
				for (const size_t eval_order_i : eval_order.index_range()) {
					...
					const size_t segments_num = mask.segments_num();
					if (segments_num <= 1) {
					...
					for (const size_t segment_i : IndexRange(segments_num).drop_front(1)) {
				...
				for (const size_t boundary_i : split_indices.index_range().drop_back(1)) {
					const IndexRange sub_bounds = IndexRange::from_begin_end(split_indices[boundary_i], split_indices[boundary_i + 1]);
					if (sub_bounds.is_empty()) {
						continue;
					}
					const IndexMaskSegment indices = evaluate_exact_with_indices(root_expression, allocator, sub_bounds, eval_order);
					if (!indices.is_empty()) {
						r_local_evaluated_segments.append({EvaluatedSegment::Type::Indices, sub_bounds, nullptr, indices});
					}
				}
				break;
			}
	...
	const size_t unknown_segment_eval_grain_size = 8;
	if (short_unknown_segments.size() < unknown_segment_eval_grain_size) {
		...
		threading::parallel_for(short_unknown_segments.index_range(), unknown_segment_eval_grain_size, [&](const IndexRange range) {
...
static IndexMask evaluated_segments_to_index_mask(MutableSpan<EvaluatedSegment> evaluated_segments, IndexMaskMemory &memory) {
	...
	std::sort(evaluated_segments.begin(), evaluated_segments.end(), [](const EvaluatedSegment &a, const EvaluatedSegment &b) { return a.bounds.start() < b.bounds.start(); });
...
static IndexMask evaluate_expression_impl(const Expr &root_expression, IndexMaskMemory &memory, const ExactEvalMode exact_eval_mode) {
	 ...
	 * algorithm. */
	const Vector<const Expr *, inline_expr_array_size> eval_order = compute_eval_order(root_expression);
	...
	evaluate_coarse_and_split_until_segments_are_short(root_expression, eval_order, evaluated_segments, short_unknown_segments);
	evaluate_short_unknown_segments_exactly(root_expression, exact_eval_mode, eval_order, short_unknown_segments, memory, evaluated_segments);
...
IndexMask evaluate_expression(const Expr &expression, IndexMaskMemory &memory) {
...
#ifndef NDEBUG
	{
		...
		const ExactEvalMode other_exact_eval_mode = (exact_eval_mode == ExactEvalMode::Bits) ? ExactEvalMode::Indices : ExactEvalMode::Bits;
		IndexMask other_mask = evaluate_expression_impl(expression, memory, other_exact_eval_mode);
		ROSE_assert(mask == other_mask);
	}
#endif
	return mask;
}
