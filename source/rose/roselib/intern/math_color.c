...
void rgb_to_yuv(float r, float g, float b, float *r_y, float *r_u, float *r_v, int colorspace) {
	...
	switch (colorspace) {
		case ROSE_YUV_ITU_BT601:
			y = 0.299f * r + 0.587f * g + 0.114f * b;
			u = -0.147f * r - 0.289f * g + 0.436f * b;
			v = 0.615f * r - 0.515f * g - 0.100f * b;
			break;
		case ROSE_YUV_ITU_BT709:
		default:
			ROSE_assert(colorspace == ROSE_YUV_ITU_BT709);
...
void ycc_to_rgb(float y, float cb, float cr, float *r_r, float *r_g, float *r_b, int colorspace) {
	 ...
	 * ROSE_YCC_ITU_BT601 y 16.0 cr 16.0 -> r -0.7009. */
...
void rgb_to_hsl(float r, float g, float b, float *r_h, float *r_s, float *r_l) {
	const float cmax = ROSE_MAX(r, g, b);
	const float cmin = ROSE_MIN(r, g, b);
	float h, s, l = ROSE_MIN(1.0f, (cmax + cmin) / 2.0f);
...
uint rgb_to_cpack(float r, float g, float b) {
	...
	ir = (uint)floorf(255.0f * ROSE_MAX(r, 0.0f));
	ig = (uint)floorf(255.0f * ROSE_MAX(g, 0.0f));
	ib = (uint)floorf(255.0f * ROSE_MAX(b, 0.0f));
...
int constrain_rgb(float *r, float *g, float *b) {
	...
	const float w = -ROSE_MIN(0.0f, *r, *g, *b);
...
float _lib_color_from_srgb_table[256];
unsigned short _lib_color_to_srgb_table[0x10000];
...
void LIB_init_srgb_conversion(void) {
	...
	for (i = 0; i < 0x10000; i++) {
		...
		if (f <= 0) {
			_lib_color_to_srgb_table[i] = 0;
		}
		else if (f < 255) {
			_lib_color_to_srgb_table[i] = (ushort)(f * 0x100 + 0.5f);
		}
		else {
			_lib_color_to_srgb_table[i] = 0xff00;
		}
	}
	...
	for (b = 0; b <= 255; b++) {
		...
		_lib_color_from_srgb_table[b] = f;
		i = hipart(f);
		/* replace entries so byte->float->byte does not change the data: */
		_lib_color_to_srgb_table[i] = (ushort)(b * 0x100);
	}
}
