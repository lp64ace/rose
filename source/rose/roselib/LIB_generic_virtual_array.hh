...
public:
	GVArrayImpl(const CPPType &type, size_t size);
	...
	virtual void get(size_t index, void *r_value) const;
	virtual void get_to_uninitialized(size_t index, void *r_value) const = 0;
...
public:
	GVMutableArrayImpl(const CPPType &type, size_t size) : GVArrayImpl(type, size) {
	}

	virtual void set_by_copy(size_t index, const void *value);
	virtual void set_by_relocate(size_t index, void *value);
	virtual void set_by_move(size_t index, void *value) = 0;
...
struct GVArrayAnyExtraInfo {
	const GVArrayImpl *(*get_varray)(const void *buffer) = [](const void * /*buffer*/) -> const GVArrayImpl * { return nullptr; };
...
public:
	...
	void get(size_t index, void *r_value) const;
	/**
	...
	void get_to_uninitialized(size_t index, void *r_value) const;
};
...
public:
	...
	GVArray(varray_tag::single_ref /* tag */, const CPPType &type, size_t size, const void *value);
	GVArray(varray_tag::single /* tag */, const CPPType &type, size_t size, const void *value);
	...
	static GVArray ForSingle(const CPPType &type, size_t size, const void *value);
	static GVArray ForSingleRef(const CPPType &type, size_t size, const void *value);
	static GVArray ForSingleDefault(const CPPType &type, size_t size);
...
public:
	...
	void set_by_copy(size_t index, const void *value);
	void set_by_move(size_t index, void *value);
	void set_by_relocate(size_t index, void *value);
...
public:
	GVArrayImpl_For_VArray(VArray<T> varray) : GVArrayImpl(CPPType::get<T>(), varray.size()), varray_(std::move(varray)) {
	}

protected:
	void get(const size_t index, void *r_value) const override {
		*static_cast<T *>(r_value) = varray_[index];
	}

	void get_to_uninitialized(const size_t index, void *r_value) const override {
	...
	void materialize_to_uninitialized(const IndexMask &mask, void *dst) const override {
		varray_.materialize_to_uninitialized(mask, MutableSpan(static_cast<T *>(dst), mask.min_array_size()));
	}
	...
	void materialize_compressed_to_uninitialized(const IndexMask &mask, void *dst) const override {
		varray_.materialize_compressed_to_uninitialized(mask, MutableSpan(static_cast<T *>(dst), mask.size()));
	}
...
public:
	VArrayImpl_For_GVArray(GVArray varray) : VArrayImpl<T>(varray.size()), varray_(std::move(varray)) {
		ROSE_assert(varray_);
		ROSE_assert(varray_.type().template is<T>());
	}
...
public:
	GVMutableArrayImpl_For_VMutableArray(VMutableArray<T> varray) : GVMutableArrayImpl(CPPType::get<T>(), varray.size()), varray_(std::move(varray)) {
	}
...
protected:
	...
	void set_by_copy(const size_t index, const void *value) override {
		const T &value_ = *(const T *)value;
		varray_.set(index, value_);
	}

	void set_by_relocate(const size_t index, void *value) override {
		T &value_ = *static_cast<T *>(value);
		varray_.set(index, std::move(value_));
		value_.~T();
	}

	void set_by_move(const size_t index, void *value) override {
		T &value_ = *static_cast<T *>(value);
		varray_.set(index, std::move(value_));
	}
...
public:
	VMutableArrayImpl_For_GVMutableArray(GVMutableArray varray) : VMutableArrayImpl<T>(varray.size()), varray_(varray) {
		ROSE_assert(varray_);
		ROSE_assert(varray_.type().template is<T>());
	}
...
private:
	...
	void set(const size_t index, T value) override {
...
public:
	GVArrayImpl_For_GSpan(const GMutableSpan span) : GVMutableArrayImpl(span.type(), span.size()), data_(span.data()), element_size_(span.type().size()) {
	}

protected:
	GVArrayImpl_For_GSpan(const CPPType &type, size_t size) : GVMutableArrayImpl(type, size), element_size_(type.size()) {
	}

public:
	void get(size_t index, void *r_value) const override;
	void get_to_uninitialized(size_t index, void *r_value) const override;

	void set_by_copy(size_t index, const void *value) override;
	void set_by_move(size_t index, void *value) override;
	void set_by_relocate(size_t index, void *value) override;
...
public:
	GVArrayImpl_For_SingleValueRef(const CPPType &type, const size_t size, const void *value) : GVArrayImpl(type, size), value_(value) {
	}

protected:
	GVArrayImpl_For_SingleValueRef(const CPPType &type, const size_t size) : GVArrayImpl(type, size) {
	}
...
inline GVArrayImpl::GVArrayImpl(const CPPType &type, const size_t size) : type_(&type), size_(size) {
	ROSE_assert(size_ >= 0);
}
...
inline size_t GVArrayImpl::size() const {
	return size_;
}
...
inline void GVMutableArray::set_by_copy(const size_t index, const void *value) {
	ROSE_assert(index >= 0);
	ROSE_assert(index < this->size());
	this->get_impl()->set_by_copy(index, value);
}

inline void GVMutableArray::set_by_move(const size_t index, void *value) {
	ROSE_assert(index >= 0);
	ROSE_assert(index < this->size());
	this->get_impl()->set_by_move(index, value);
}

inline void GVMutableArray::set_by_relocate(const size_t index, void *value) {
	ROSE_assert(index >= 0);
	ROSE_assert(index < this->size());
	this->get_impl()->set_by_relocate(index, value);
}

template<typename T> inline bool GVMutableArray::try_assign_VMutableArray(VMutableArray<T> &varray) const {
	ROSE_assert(impl_->type().is<T>());
...
inline void GVArrayCommon::get(const size_t index, void *r_value) const {
	ROSE_assert(index >= 0);
	ROSE_assert(index < this->size());
	impl_->get(index, r_value);
}

template<typename T> inline T GVArrayCommon::get(const size_t index) const {
	ROSE_assert(index >= 0);
	ROSE_assert(index < this->size());
	ROSE_assert(this->type().is<T>());
	T value{};
...
inline void GVArrayCommon::get_to_uninitialized(const size_t index, void *r_value) const {
	ROSE_assert(index >= 0);
	ROSE_assert(index < this->size());
...
inline size_t GVArrayCommon::size() const {
	if (impl_ == nullptr) {
		return 0;
	}
	return impl_->size();
...
template<typename T, bool UseSingle, bool UseSpan> struct GVArrayDevirtualizer {
	...
	template<typename Fn> bool devirtualize(const Fn &fn) const {
		...
		const size_t size = this->varray_impl.size();
...
inline GVArray::GVArray(varray_tag::single_ref /* tag */, const CPPType &type, const size_t size, const void *value) {
...
template<typename StorageT> constexpr GVArrayAnyExtraInfo GVArrayAnyExtraInfo::get() {
	static_assert(std::is_base_of_v<GVArrayImpl, StorageT> || is_same_any_v<StorageT, const GVArrayImpl *, std::shared_ptr<const GVArrayImpl>>);

	if constexpr (std::is_base_of_v<GVArrayImpl, StorageT>) {
		return {[](const void *buffer) { return static_cast<const GVArrayImpl *>((const StorageT *)buffer); }};
	}
...
template<typename ImplT, typename... Args> inline GVMutableArray GVMutableArray::For(Args &&...args) {
...
template<typename T> inline GVMutableArray::GVMutableArray(const VMutableArray<T> &varray) {
	...
	const CommonVArrayInfo info = varray.common_info();
	if (info.type == CommonVArrayInfo::Type::Span && !info.may_have_ownership) {
		*this = GVMutableArray::ForSpan(GMutableSpan(CPPType::get<T>(), const_cast<void *>(info.data), varray.size()));
		return;
	}
...
template<typename T> inline VMutableArray<T> GVMutableArray::typed() const {
	...
	const CommonVArrayInfo info = this->common_info();
	if (info.type == CommonVArrayInfo::Type::Span && !info.may_have_ownership) {
		return VMutableArray<T>::ForSpan(MutableSpan<T>(const_cast<T *>(static_cast<const T *>(info.data)), this->size()));
	}
