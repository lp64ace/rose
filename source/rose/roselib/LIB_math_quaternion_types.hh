#ifndef LIB_MATH_QUATERNION_TYPES_HH
#define LIB_MATH_QUATERNION_TYPES_HH
...
/**
 * A `rose::math::QuaternionBase<T>` represents either an orientation or a rotation.
 *
 * Mainly used for rigging and armature deformations as they have nice mathematical properties
 * (eg: smooth shortest path interpolation). A `rose::math::QuaternionBase<T>` is cheaper to
...
template<typename T> struct QuaternionBase {
	...
	QuaternionBase(const T &new_w, const T &new_x, const T &new_y, const T &new_z) : w(new_w), x(new_x), y(new_y), z(new_z) {};
	 ...
	 */
	QuaternionBase(const T &real, const VecBase<T, 3> &imaginary) : QuaternionBase(real, UNPACK3(imaginary)) {};
	...
	ROSE_STRUCT_EQUALITY_OPERATORS_4(QuaternionBase, w, x, y, z)
...
/**
 * A `rose::math::DualQuaternionBase<T>` implements dual-quaternion skinning with scale aware
 * transformation. It allows volume preserving deformation for skinning.
 *
 * The type is implemented so that multiple weighted `rose::math::DualQuaternionBase<T>`
...
template<typename T> struct DualQuaternionBase {
	...
	ROSE_STRUCT_EQUALITY_OPERATORS_5(DualQuaternionBase, quat, trans, quat_weight, scale_weight, scale)
...
#endif	// LIB_MATH_QUATERNION_TYPES_HH
