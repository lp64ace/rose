...
struct GrainSize {
	...
	explicit constexpr GrainSize(const size_t grain_size) : value(grain_size) {
	}
};

}  // namespace rose

namespace rose::threading {

template<typename Range, typename Function> inline void parallel_for_each(Range &&range, const Function &function) {
#ifdef WITH_TBB
...
template<typename Function> inline void parallel_for(IndexRange range, size_t grain_size, const Function &function) {
	if (range.is_empty()) {
		return;
	}
 ...
 * remain fixed though.
 */
inline IndexRange align_sub_range(const IndexRange unaligned_range, const size_t alignment, const IndexRange global_range) {
	const size_t global_begin = global_range.start();
	const size_t global_end = global_range.one_after_last();
	const size_t alignment_mask = ~(alignment - 1);

	const size_t unaligned_begin = unaligned_range.start();
	const size_t unaligned_end = unaligned_range.one_after_last();
	const size_t aligned_begin = std::max(global_begin, unaligned_begin & alignment_mask);
	const size_t aligned_end = unaligned_end == global_end ? unaligned_end : std::max(global_begin, unaligned_end & alignment_mask);
 ...
 */
template<typename Function> inline void parallel_for_aligned(const IndexRange range, const size_t grain_size, const size_t alignment, const Function &function) {
...
template<typename Value, typename Function, typename Reduction> inline Value parallel_reduce(IndexRange range, size_t grain_size, const Value &identity, const Function &function, const Reduction &reduction) {
#ifdef WITH_TBB
	if (range.size() >= grain_size) {
		lazy_threading::send_hint();
		return tbb::parallel_reduce(tbb::blocked_range<size_t>(range.first(), range.one_after_last(), grain_size), identity, [&](const tbb::blocked_range<size_t> &subrange, const Value &ident) { return function(IndexRange(subrange.begin(), subrange.size()), ident); }, reduction);
	}
#else
...
template<typename Value, typename Function, typename Reduction> inline Value parallel_reduce_aligned(const IndexRange range, const size_t grain_size, const size_t alignment, const Value &identity, const Function &function, const Reduction &reduction) {
	parallel_reduce(
		range,
		grain_size,
		identity,
 ...
 * tasks.
 */
template<typename... Functions> inline void parallel_invoke(const bool use_threading, Functions &&...functions) {
	if (use_threading) {
		lazy_threading::send_hint();
...
/** See #LIB_task_isolate for a description of what isolating a task means. */
