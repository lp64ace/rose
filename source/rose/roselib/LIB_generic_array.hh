#ifndef LIB_GENERIC_ARRAY_HH
#define LIB_GENERIC_ARRAY_HH
...
public:
	 ...
	 */
	GArray(const CPPType &type, size_t size, Allocator allocator = {}) : GArray(type, allocator) {
		ROSE_assert(size >= 0);
		size_ = size;
	 ...
	 */
	GArray(const CPPType &type, void *buffer, size_t size, Allocator allocator = {}) : GArray(type, allocator) {
		ROSE_assert(size >= 0);
		ROSE_assert(buffer != nullptr || size == 0);
		ROSE_assert(type_->pointer_has_valid_alignment(buffer));
	 ...
	 */
	GArray(GArray &&other) : type_(other.type_), data_(other.data_), size_(other.size_), allocator_(other.allocator_) {
		other.data_ = nullptr;
		other.size_ = 0;
	}
...
private:
	void *allocate(size_t size) {
		const size_t item_size = type_->size();
		const size_t alignment = type_->alignment();
		return allocator_.allocate(size * item_size, alignment, AT);
	}
