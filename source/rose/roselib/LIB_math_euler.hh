#ifndef LIB_MATH_EULER_HH
#define LIB_MATH_EULER_HH
...
template<typename T> EulerXYZBase<T> EulerXYZBase<T>::wrapped_around(const EulerXYZBase &reference) const {
	EulerXYZBase<T> result(*this);
...
template<typename T, typename AngleT = AngleRadian> AxisAngleBase<T, AngleT> to_axis_angle(const EulerXYZBase<T> &euler) {
	/* Use quaternions as intermediate representation for now... */
	return to_axis_angle<T, AngleT>(to_quaternion(euler));
}

template<typename T, typename AngleT = AngleRadian> AxisAngleBase<T, AngleT> to_axis_angle(const Euler3Base<T> &euler) {
	/* Use quaternions as intermediate representation for now... */
	return to_axis_angle<T, AngleT>(to_quaternion(euler));
}
