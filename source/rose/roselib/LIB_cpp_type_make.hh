#ifndef LIB_CPP_TYPES_MAKE_HH
#define LIB_CPP_TYPES_MAKE_HH

#include "LIB_cpp_type.hh"
#include "LIB_utildefines.h"
#include <sstream>

namespace rose::cpp_type_util {
...
template<typename T> void default_construct_indices_cb(void *ptr, const IndexMask &mask) {
	...
	mask.foreach_index_optimized<size_t>([&](size_t i) { new (static_cast<T *>(ptr) + i) T; });
}
...
template<typename T> void destruct_indices_cb(void *ptr, const IndexMask &mask) {
	...
	mask.foreach_index_optimized<size_t>([&](size_t i) { ptr_[i].~T(); });
}
...
template<typename T> void copy_assign_indices_cb(const void *src, void *dst, const IndexMask &mask) {
	...
	mask.foreach_index_optimized<size_t>([&](size_t i) { dst_[i] = src_[i]; });
}
template<typename T> void copy_assign_compressed_cb(const void *src, void *dst, const IndexMask &mask) {
	const T *src_ = static_cast<const T *>(src);
	T *dst_ = static_cast<T *>(dst);

	mask.foreach_index_optimized<size_t>([&](const size_t i, const size_t pos) { dst_[pos] = src_[i]; });
}

template<typename T> void copy_construct_cb(const void *src, void *dst) {
	rose::uninitialized_copy_n(static_cast<const T *>(src), 1, static_cast<T *>(dst));
}
template<typename T> void copy_construct_indices_cb(const void *src, void *dst, const IndexMask &mask) {
	const T *src_ = static_cast<const T *>(src);
	T *dst_ = static_cast<T *>(dst);

	mask.foreach_index_optimized<size_t>([&](size_t i) { new (dst_ + i) T(src_[i]); });
}
template<typename T> void copy_construct_compressed_cb(const void *src, void *dst, const IndexMask &mask) {
	const T *src_ = static_cast<const T *>(src);
	T *dst_ = static_cast<T *>(dst);

	mask.foreach_index_optimized<size_t>([&](const size_t i, const size_t pos) { new (dst_ + pos) T(src_[i]); });
}

template<typename T> void move_assign_cb(void *src, void *dst) {
	rose::initialized_move_n(static_cast<T *>(src), 1, static_cast<T *>(dst));
}
template<typename T> void move_assign_indices_cb(void *src, void *dst, const IndexMask &mask) {
	T *src_ = static_cast<T *>(src);
	T *dst_ = static_cast<T *>(dst);

	mask.foreach_index_optimized<size_t>([&](size_t i) { dst_[i] = std::move(src_[i]); });
}

template<typename T> void move_construct_cb(void *src, void *dst) {
	rose::uninitialized_move_n(static_cast<T *>(src), 1, static_cast<T *>(dst));
}
template<typename T> void move_construct_indices_cb(void *src, void *dst, const IndexMask &mask) {
	T *src_ = static_cast<T *>(src);
	T *dst_ = static_cast<T *>(dst);

	mask.foreach_index_optimized<size_t>([&](size_t i) { new (dst_ + i) T(std::move(src_[i])); });
}
...
template<typename T> void relocate_assign_indices_cb(void *src, void *dst, const IndexMask &mask) {
	...
	mask.foreach_index_optimized<size_t>([&](size_t i) {
...
template<typename T> void relocate_construct_indices_cb(void *src, void *dst, const IndexMask &mask) {
	T *src_ = static_cast<T *>(src);
	T *dst_ = static_cast<T *>(dst);
...
template<typename T> void fill_assign_cb(const void *value, void *dst, size_t n) {
...
template<typename T> void fill_assign_indices_cb(const void *value, void *dst, const IndexMask &mask) {
	const T &value_ = *static_cast<const T *>(value);
	T *dst_ = static_cast<T *>(dst);

	mask.foreach_index_optimized<size_t>([&](size_t i) { dst_[i] = value_; });
}

template<typename T> void fill_construct_cb(const void *value, void *dst, size_t n) {
	const T &value_ = *static_cast<const T *>(value);
	T *dst_ = static_cast<T *>(dst);
...
template<typename T> void fill_construct_indices_cb(const void *value, void *dst, const IndexMask &mask) {
	const T &value_ = *static_cast<const T *>(value);
	T *dst_ = static_cast<T *>(dst);

	mask.foreach_index_optimized<size_t>([&](size_t i) { new (dst_ + i) T(value_); });
}
...
template<typename T, CPPTypeFlags Flags> CPPType::CPPType(TypeTag<T> /*type*/, TypeForValue<CPPTypeFlags, Flags> /*flags*/, const StringRef debug_name) {
	using namespace cpp_type_util;

	debug_name_ = debug_name;
	size_ = size_t(sizeof(T));
	alignment_ = size_t(alignof(T));
	...
	has_special_member_functions_ = (default_construct_ && copy_construct_ && copy_assign_ && move_construct_ && move_assign_ && destruct_);
}

}  // namespace rose

/** Create a new #CPPType that can be accessed through `CPPType::get<T>()`. */
#define ROSE_CPP_TYPE_MAKE(TYPE_NAME, FLAGS)                                                                        \
	template<> const rose::CPPType &rose::CPPType::get_impl<TYPE_NAME>() {                                          \
		static CPPType type{rose::TypeTag<TYPE_NAME>(), TypeForValue<CPPTypeFlags, FLAGS>(), STRINGIFY(TYPE_NAME)}; \
		return type;                                                                                                \
	}

/** Register a #CPPType created with #ROSE_CPP_TYPE_MAKE. */
#define ROSE_CPP_TYPE_REGISTER(TYPE_NAME) rose::CPPType::get<TYPE_NAME>()

#endif	// LIB_CPP_TYPES_MAKE_HH
