...
public:
	...
	static constexpr size_t not_found = LIB_NPOS;
	...
	constexpr StringRef substr(size_t start, size_t size) const;
 ...
 */
constexpr StringRef StringRefBase::substr(const size_t start, const size_t max_size = SIZE_MAX) const {
	ROSE_assert(max_size >= 0);
	ROSE_assert(start >= 0);
	const size_t substr_size = std::min(max_size, size_ - start);
...
constexpr size_t index_or_npos_to_size_type(size_t index) {
...
constexpr size_t StringRefBase::find(char c, size_t pos) const {
	ROSE_assert(pos >= 0);
	return index_or_npos_to_size_type(std::string_view(*this).find(c, size_t(pos)));
}

constexpr size_t StringRefBase::find(StringRef str, size_t pos) const {
	ROSE_assert(pos >= 0);
	return index_or_npos_to_size_type(std::string_view(*this).find(str, size_t(pos)));
}

constexpr size_t StringRefBase::rfind(char c, size_t pos) const {
	ROSE_assert(pos >= 0);
	return index_or_npos_to_size_type(std::string_view(*this).rfind(c, size_t(pos)));
}

constexpr size_t StringRefBase::rfind(StringRef str, size_t pos) const {
	ROSE_assert(pos >= 0);
	return index_or_npos_to_size_type(std::string_view(*this).rfind(str, size_t(pos)));
}

constexpr size_t StringRefBase::find_first_of(StringRef chars, size_t pos) const {
	ROSE_assert(pos >= 0);
	return index_or_npos_to_size_type(std::string_view(*this).find_first_of(chars, size_t(pos)));
}

constexpr size_t StringRefBase::find_first_of(char c, size_t pos) const {
	ROSE_assert(pos >= 0);
	return index_or_npos_to_size_type(std::string_view(*this).find_first_of(c, size_t(pos)));
}

constexpr size_t StringRefBase::find_last_of(StringRef chars, size_t pos) const {
	ROSE_assert(pos >= 0);
	return index_or_npos_to_size_type(std::string_view(*this).find_last_of(chars, size_t(pos)));
}

constexpr size_t StringRefBase::find_last_of(char c, size_t pos) const {
	ROSE_assert(pos >= 0);
	return index_or_npos_to_size_type(std::string_view(*this).find_last_of(c, size_t(pos)));
}

constexpr size_t StringRefBase::find_first_not_of(StringRef chars, size_t pos) const {
	ROSE_assert(pos >= 0);
	return index_or_npos_to_size_type(std::string_view(*this).find_first_not_of(chars, size_t(pos)));
}

constexpr size_t StringRefBase::find_first_not_of(char c, size_t pos) const {
	ROSE_assert(pos >= 0);
	return index_or_npos_to_size_type(std::string_view(*this).find_first_not_of(c, size_t(pos)));
}

constexpr size_t StringRefBase::find_last_not_of(StringRef chars, size_t pos) const {
	ROSE_assert(pos >= 0);
	return index_or_npos_to_size_type(std::string_view(*this).find_last_not_of(chars, size_t(pos)));
}

constexpr size_t StringRefBase::find_last_not_of(char c, size_t pos) const {
	ROSE_assert(pos >= 0);
	return index_or_npos_to_size_type(std::string_view(*this).find_last_not_of(c, size_t(pos)));
}
...
constexpr StringRefNull::StringRefNull(const char *str, const size_t size) : StringRefBase(str, size) {
	ROSE_assert(size_t(LIB_strlen(str)) == size);
}
 ...
 * NULL.
 */
inline StringRefNull::StringRefNull(const char *str) : StringRefBase(str, size_t(LIB_strlen(str))) {
 ...
 */
constexpr StringRef::StringRef(const char *str) : StringRefBase(str, str ? size_t(std::char_traits<char>::length(str)) : 0) {
}
...
constexpr StringRef StringRef::drop_suffix(const size_t n) const {
	ROSE_assert(n >= 0);
	const size_t new_size = std::max<size_t>(0, size_ - n);
 ...
 */
constexpr StringRef::StringRef(const char *begin, const char *one_after_end) : StringRefBase(begin, size_t(one_after_end - begin)) {
 ...
 * will point to uninitialized memory.
 */
inline StringRef::StringRef(const std::string &str) : StringRefBase(str.data(), size_t(str.size())) {
}

constexpr StringRef::StringRef(std::string_view view) : StringRefBase(view.data(), size_t(view.size())) {
}
