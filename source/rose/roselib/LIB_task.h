...
void LIB_task_scheduler_init(void);
void LIB_task_scheduler_exit(void);
int LIB_task_scheduler_num_threads(void);
...
typedef struct TaskParallelSettings {
	 ...
	 * LIB_task_parallel_range() by itself will always use threading when range
...
ROSE_INLINE void LIB_parallel_range_settings_defaults(TaskParallelSettings *settings);

void LIB_task_parallel_range(int start, int stop, void *userdata, TaskParallelRangeFunc func, const TaskParallelSettings *settings);
 ...
 */
void LIB_task_parallel_iterator(void *userdata, TaskParallelIteratorIterFunc iter_func, void *init_item, int init_index, int items_num, TaskParallelIteratorFunc func, const TaskParallelSettings *settings);
 ...
 */
void LIB_task_parallel_listbase(struct ListBase *listbase, void *userdata, TaskParallelIteratorFunc func, const TaskParallelSettings *settings);
 ...
 *
 * \param mempool: The iterable MemPool to loop over.
 * \param userdata: Common userdata passed to all instances of \a func.
 * \param func: Callback function.
 * \param settings: See public API doc of TaskParallelSettings for description of all settings.
 *
 * \note There is no static scheduling here.
 */
void LIB_task_parallel_mempool(struct MemPool *mempool, void *userdata, TaskParallelMempoolFunc func, const TaskParallelSettings *settings);
...
ROSE_INLINE void LIB_parallel_mempool_settings_defaults(TaskParallelSettings *settings) {
	memset(settings, 0, sizeof(*settings));
	settings->use_threading = true;
}
 ...
 *
 * \code{.c}
 * TaskGraph *task_graph = LIB_task_graph_create();
 * TaskNode *root = LIB_task_graph_node_create(task_graph, root_exec, NULL, NULL);
 * TaskNode *node_1 = LIB_task_graph_node_create(task_graph, node_exec, NULL, NULL);
 * TaskNode *node_2 = LIB_task_graph_node_create(task_graph, node_exec, NULL, NULL);
 * TaskNode *node_3 = LIB_task_graph_node_create(task_graph, node_exec, NULL, NULL);
 * TaskNode *node_4 = LIB_task_graph_node_create(task_graph, node_exec, NULL, NULL);
 *
 * LIB_task_graph_edge_create(root, node_1);
 * LIB_task_graph_edge_create(root, node_2);
 * LIB_task_graph_edge_create(node_2, node_3);
 * LIB_task_graph_edge_create(node_2, node_4);
 * \endcode
 *
 ...
 *
 *    `LIB_task_graph_node_push_work(root);`
 *
 ...
 *
 *    `LIB_task_graph_free(task_graph);`
 *
 ...
 * Task data is freed when calling #LIB_task_graph_free.
 *
 * \code{.c}
 * MyData *task_data = MEM_callocN(sizeof(MyData), __func__);
 * TaskNode *root = LIB_task_graph_node_create(task_graph, root_exec, task_data, MEM_freeN);
 * TaskNode *node_1 = LIB_task_graph_node_create(task_graph, node_exec, task_data, NULL);
 * TaskNode *node_2 = LIB_task_graph_node_create(task_graph, node_exec, task_data, NULL);
 * TaskNode *node_3 = LIB_task_graph_node_create(task_graph, node_exec, task_data, NULL);
 * TaskNode *node_4 = LIB_task_graph_node_create(task_graph, node_exec, task_data, NULL);
 * \endcode
 * \{ */
...
struct TaskGraph *LIB_task_graph_create(void);
void LIB_task_graph_work_and_wait(struct TaskGraph *task_graph);
void LIB_task_graph_free(struct TaskGraph *task_graph);
