...
using IndexMaskSegment = OffsetSpan<size_t, int16_t>;
...
public:
	 ...
	 */
	IndexMask slice_and_offset(IndexRange range, size_t offset, IndexMaskMemory &memory) const;
	IndexMask slice_and_offset(size_t start, size_t size, size_t offset, IndexMaskMemory &memory) const;
...
#endif
inline void
optimized_foreach_index_with_pos(const IndexMaskSegment segment, const size_t segment_pos, const Fn fn) {
	ROSE_assert(segment.last() < std::numeric_limits<T>::max());
...
template<typename Fn> inline IndexMask IndexMask::from_predicate(const IndexMask &universe, const GrainSize grain_size, IndexMaskMemory &memory, Fn &&predicate) {
	return detail::from_predicate_impl(universe, grain_size, memory, [&](const IndexMaskSegment indices, int16_t *__restrict r_true_indices) {
		...
		for (const int16_t *in_current = indices.base_span().data(); in_current < in_end; in_current++) {
			const size_t local_index = *in_current;
		...
		const size_t true_indices_num = size_t(r_current - r_true_indices);
