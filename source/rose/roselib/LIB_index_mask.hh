#ifndef LIB_INDEX_MASK_HH
#define LIB_INDEX_MASK_HH
...
#include "LIB_index_mask_fwd.hh"
#include "LIB_index_ranges_builder_fwd.hh"
#include "LIB_linear_allocator.hh"
#include "LIB_offset_span.hh"
#include "LIB_task.hh"
#include "LIB_unique_sorted_indices.hh"
#include "LIB_vector.hh"
#include "LIB_vector_set.hh"
#include "LIB_virtual_array_fwd.hh"
 ...
 */
static constexpr size_t max_segment_size_shift = 14;
static constexpr size_t max_segment_size = (1 << max_segment_size_shift); /* 16384 */
static constexpr size_t max_segment_size_mask_low = max_segment_size - 1;
static constexpr size_t max_segment_size_mask_high = ~max_segment_size_mask_low;
...
struct IndexMaskData {
	 ...
	 */
	const size_t *cumulative_segment_sizes_;
	/**
	 ...
	 */
	size_t begin_index_in_segment_;
	/**
 ...
 * has an `size_t` index offset that is added to each referenced `int16_t` index.
 */
class IndexMaskSegment : public OffsetSpan<size_t, int16_t> {
public:
	using OffsetSpan<size_t, int16_t>::OffsetSpan;

	explicit IndexMaskSegment(const OffsetSpan<size_t, int16_t> span);

	IndexMaskSegment slice(const IndexRange &range) const;
	IndexMaskSegment slice(const size_t start, const size_t size) const;
	 ...
	 */
	IndexMaskSegment shift(const size_t shift) const;
};

/**
 * An #IndexMask is a sequence of unique and sorted indices (`LIB_unique_sorted_indices.hh`).
 ...
 * `int16_t`, but each segment also has a `size_t` offset.
 *
...
public:
	...
	explicit IndexMask(size_t size);
	 ...
	 */
	static IndexMask from_repeating(const IndexMask &mask_to_repeat, size_t repetitions, size_t stride, size_t initial_offset, IndexMaskMemory &memory);
	/**
	 * Constructs a mask that contains every nth index the given number of times.
	 */
	static IndexMask from_every_nth(size_t n, size_t indices_num, const size_t initial_offset, IndexMaskMemory &memory);
	/**
	 ...
	 */
	using Initializer = std::variant<IndexRange, Span<size_t>, Span<int>, size_t>;
	...
	template<typename Fn> static IndexMask from_predicate(const IndexMask &universe, GrainSize grain_size, IndexMaskMemory &memory, Fn &&predicate);
	/**
	 ...
	 */
	static IndexMask from_batch_predicate(const IndexMask &universe, GrainSize grain_size, IndexMaskMemory &memory, FunctionRef<size_t(const IndexMaskSegment &universe_segment, IndexRangesBuilder<int16_t> &builder)> batch_predicate);
	/** Sorts all indices from #universe into the different output masks. */
	template<typename T, typename Fn> static void from_groups(const IndexMask &universe, IndexMaskMemory &memory, Fn &&get_group_index, MutableSpan<IndexMask> r_masks);
	 ...
	 */
	std::optional<RawMaskIterator> find(size_t query_index) const;
	std::optional<RawMaskIterator> find_larger_equal(size_t query_index) const;
	std::optional<RawMaskIterator> find_smaller_equal(size_t query_index) const;
	/**
	 * \return True when the #query_index is stored in the mask.
	 */
	bool contains(size_t query_index) const;

	/** \return The iterator for the given index such that `mask[iterator] == mask[index]`. */
	RawMaskIterator index_to_iterator(size_t index) const;
	/** \return The index for the given iterator such that `mask[iterator] == mask[index]`. */
	size_t iterator_to_index(const RawMaskIterator &it) const;
	 ...
	 * iteration.
	 */
	size_t operator[](const RawMaskIterator &it) const;
	...
	IndexMask slice(size_t start, size_t size) const;
	IndexMask slice(RawMaskIterator first_it, RawMaskIterator last_it, size_t size) const;
	/**
	...
	IndexMask slice_content(size_t start, size_t size) const;
	/**
	 ...
	 */
	IndexMask slice_and_shift(IndexRange range, size_t offset, IndexMaskMemory &memory) const;
	IndexMask slice_and_shift(size_t start, size_t size, size_t offset, IndexMaskMemory &memory) const;

	/**
	 * Adds an offset to every index in the mask.
	 */
	IndexMask shift(const size_t offset, IndexMaskMemory &memory) const;
	 ...
	 */
	size_t segments_num() const;
	/**
	 * \return Indices stored in the n-th segment.
	 */
	IndexMaskSegment segment(size_t segment_i) const;
	...
	template<typename IndexT, typename Fn> void foreach_index_optimized(GrainSize grain_size, Fn &&fn) const;
	 ...
	 *  - `(IndexMaskSegment segment, size_t segment_pos)`
	 *
	 ...
	 * - `(auto segment)`
	 * - `(auto segment, size_t segment_pos)`
	 *
	 ...
	 * - `(IndexRange segment)`
	 * - `(IndexRange segment, size_t segment_pos)`
	 */
	 ...
	 */
	void to_bits(MutableBitSpan r_bits, size_t offset = 0) const;
	/**
	 ...
	 * an #IndexRange.
	 */
	template<size_t N = 4> Vector<std::variant<IndexRange, IndexMaskSegment>, N> to_spans_and_ranges() const;
...
private:
	...
	std::array<size_t, 2> cumulative_segment_sizes_;
	IndexMask mask_;
...
inline IndexMaskFromSegment::IndexMaskFromSegment() : segment_offset_(0), segment_indices_(nullptr) {
...
inline const IndexMask &IndexMaskFromSegment::update(const IndexMaskSegment segment) {
	...
	ROSE_assert(std::is_sorted(indices.begin(), indices.end()));
	ROSE_assert(indices[0] >= 0);
	ROSE_assert(indices.last() < max_segment_size);
	const size_t indices_num = indices.size();
...
inline const std::array<int16_t, max_segment_size> &get_static_indices_array() {
	alignas(64) static const std::array<int16_t, max_segment_size> data = build_static_indices_array();
	return data;
}

template<typename T> inline void masked_fill(MutableSpan<T> data, const T &value, const IndexMask &mask) {
	mask.foreach_index_optimized<size_t>([&](const size_t i) { data[i] = value; });
}
...
inline IndexMaskSegment::IndexMaskSegment(const OffsetSpan<size_t, int16_t> span) : OffsetSpan<size_t, int16_t>(span) {
}

inline IndexMaskSegment IndexMaskSegment::slice(const IndexRange &range) const {
	return IndexMaskSegment(static_cast<const OffsetSpan<size_t, int16_t> *>(this)->slice(range));
}

inline IndexMaskSegment IndexMaskSegment::slice(const size_t start, const size_t size) const {
	return IndexMaskSegment(static_cast<const OffsetSpan<size_t, int16_t> *>(this)->slice(start, size));
}

inline IndexMaskSegment IndexMaskSegment::shift(const size_t shift) const {
	ROSE_assert(this->is_empty() || (*this)[0] + shift >= 0);
...
inline void init_empty_mask(IndexMaskData &data) {
	static constexpr size_t cumulative_sizes_for_empty_mask[1] = {0};
...
inline IndexMask::IndexMask(const size_t size) {
	if (size == 0) {
		init_empty_mask(*this);
		return;
	}
...
inline IndexMask::IndexMask(const IndexRange range) {
	...
	const size_t one_after_last = range.one_after_last();
	*this = get_static_index_mask_for_min_size(one_after_last);

	const size_t first_segment_i = range.first() >> max_segment_size_shift;
	const size_t last_segment_i = range.last() >> max_segment_size_shift;
...
inline size_t IndexMask::size() const {
	return indices_num_;
}
...
inline IndexRange IndexMask::bounds() const {
	...
	const size_t first = this->first();
	const size_t last = this->last();
	return IndexRange::from_begin_end_inclusive(first, last);
}

inline size_t IndexMask::first() const {
	ROSE_assert(indices_num_ > 0);
	return segment_offsets_[0] + indices_by_segment_[0][begin_index_in_segment_];
}

inline size_t IndexMask::last() const {
	ROSE_assert(indices_num_ > 0);
	const size_t last_segment_i = segments_num_ - 1;
	return segment_offsets_[last_segment_i] + indices_by_segment_[last_segment_i][end_index_in_segment_ - 1];
}

inline size_t IndexMask::min_array_size() const {
	if (indices_num_ == 0) {
		return 0;
	}
	return this->last() + 1;
...
inline RawMaskIterator IndexMask::index_to_iterator(const size_t index) const {
	ROSE_assert(index >= 0);
	ROSE_assert(index < indices_num_);
	RawMaskIterator it;
	const size_t full_index = index + cumulative_segment_sizes_[0] + begin_index_in_segment_;
	it.segment_i = -1 + binary_search::find_predicate_begin(cumulative_segment_sizes_, cumulative_segment_sizes_ + segments_num_ + 1, [&](const size_t cumulative_size) { return cumulative_size > full_index; });
	it.index_in_segment = full_index - cumulative_segment_sizes_[it.segment_i];
	return it;
}

inline size_t IndexMask::iterator_to_index(const RawMaskIterator &it) const {
	ROSE_assert(it.segment_i >= 0);
	ROSE_assert(it.segment_i < segments_num_);
	ROSE_assert(it.index_in_segment >= 0);
	ROSE_assert(it.index_in_segment < cumulative_segment_sizes_[it.segment_i + 1] - cumulative_segment_sizes_[it.segment_i]);
	return it.index_in_segment + cumulative_segment_sizes_[it.segment_i] - cumulative_segment_sizes_[0] - begin_index_in_segment_;
}

inline size_t IndexMask::operator[](const size_t i) const {
	const RawMaskIterator it = this->index_to_iterator(i);
	return (*this)[it];
}

inline size_t IndexMask::operator[](const RawMaskIterator &it) const {
	return segment_offsets_[it.segment_i] + indices_by_segment_[it.segment_i][it.index_in_segment];
}

inline size_t IndexMask::segments_num() const {
	return segments_num_;
}

inline IndexMaskSegment IndexMask::segment(const size_t segment_i) const {
	ROSE_assert(segment_i >= 0);
	ROSE_assert(segment_i < segments_num_);
	const size_t full_segment_size = cumulative_segment_sizes_[segment_i + 1] - cumulative_segment_sizes_[segment_i];
	const size_t begin_index = (segment_i == 0) ? begin_index_in_segment_ : 0;
	const size_t end_index = (segment_i == segments_num_ - 1) ? end_index_in_segment_ : full_segment_size;
	const size_t segment_size = end_index - begin_index;
	return IndexMaskSegment{segment_offsets_[segment_i], {indices_by_segment_[segment_i] + begin_index, segment_size}};
}
...
template<typename Fn> constexpr bool has_segment_and_start_parameter = std::is_invocable_r_v<void, Fn, IndexMaskSegment, size_t> || std::is_invocable_r_v<void, Fn, IndexRange, size_t>;

template<typename Fn> inline void IndexMask::foreach_index(Fn &&fn) const {
	this->foreach_segment([&](const IndexMaskSegment indices, [[maybe_unused]] const size_t start_segment_pos) {
		if constexpr (std::is_invocable_r_v<void, Fn, size_t, size_t>) {
			for (const size_t i : indices.index_range()) {
...
template<typename Fn> inline void IndexMask::foreach_index(const GrainSize grain_size, Fn &&fn) const {
	threading::parallel_for(this->index_range(), grain_size.value, [&](const IndexRange range) {
		const IndexMask sub_mask = this->slice(range);
		sub_mask.foreach_index([&](const size_t i, [[maybe_unused]] const size_t index_pos) {
			if constexpr (std::is_invocable_r_v<void, Fn, size_t, size_t>) {
...
inline void optimized_foreach_index(const IndexMaskSegment segment, const Fn fn) {
	ROSE_assert(segment.last() < std::numeric_limits<T>::max());
...
template<typename T, typename Fn> inline void optimized_foreach_index_with_pos(const IndexMaskSegment segment, const size_t segment_pos, const Fn fn) {
	ROSE_assert(segment.last() < std::numeric_limits<T>::max());
	ROSE_assert(segment.size() + segment_pos < std::numeric_limits<T>::max());
	if (unique_sorted_indices::non_empty_is_range(segment.base_span())) {
		const T start = T(segment[0]);
		const T last = T(segment.last());
...
template<typename IndexT, typename Fn> inline void IndexMask::foreach_index_optimized(Fn &&fn) const {
	this->foreach_segment([&](const IndexMaskSegment segment, [[maybe_unused]] const size_t segment_pos) {
...
template<typename IndexT, typename Fn> inline void IndexMask::foreach_index_optimized(const GrainSize grain_size, Fn &&fn) const {
	threading::parallel_for(this->index_range(), grain_size.value, [&](const IndexRange range) {
		const IndexMask sub_mask = this->slice(range);
		sub_mask.foreach_segment([&](const IndexMaskSegment segment, [[maybe_unused]] const size_t segment_pos) {
			if constexpr (std::is_invocable_r_v<void, Fn, IndexT, IndexT>) {
...
template<typename Fn> inline void IndexMask::foreach_segment_optimized(Fn &&fn) const {
	this->foreach_segment([&](const IndexMaskSegment segment, [[maybe_unused]] const size_t start_segment_pos) {
		if (unique_sorted_indices::non_empty_is_range(segment.base_span())) {
...
template<typename Fn> inline void IndexMask::foreach_segment_optimized(const GrainSize grain_size, Fn &&fn) const {
	threading::parallel_for(this->index_range(), grain_size.value, [&](const IndexRange range) {
		const IndexMask sub_mask = this->slice(range);
		sub_mask.foreach_segment_optimized([&fn, range_start = range.start()](const auto segment, [[maybe_unused]] const size_t start_segment_pos) {
			if constexpr (has_segment_and_start_parameter<Fn>) {
...
template<typename Fn> inline void IndexMask::foreach_segment(Fn &&fn) const {
	[[maybe_unused]] size_t segment_pos = 0;
	for (const size_t segment_i : IndexRange(segments_num_)) {
...
template<typename Fn> inline void IndexMask::foreach_segment(const GrainSize grain_size, Fn &&fn) const {
	threading::parallel_for(this->index_range(), grain_size.value, [&](const IndexRange range) {
		const IndexMask sub_mask = this->slice(range);
		sub_mask.foreach_segment([&fn, range_start = range.start()](const IndexMaskSegment mask_segment, [[maybe_unused]] const size_t segment_pos) {
			if constexpr (has_segment_and_start_parameter<Fn>) {
...
template<typename Fn> inline void IndexMask::foreach_range(Fn &&fn) const {
	this->foreach_segment([&](const IndexMaskSegment indices, [[maybe_unused]] size_t segment_pos) {
		Span<int16_t> base_indices = indices.base_span();
		while (!base_indices.is_empty()) {
			const size_t next_range_size = unique_sorted_indices::find_size_of_next_range(base_indices);
			const IndexRange range(size_t(base_indices[0]) + indices.offset(), next_range_size);
			if constexpr (has_segment_and_start_parameter<Fn>) {
...
namespace detail {
IndexMask from_predicate_impl(const IndexMask &universe, GrainSize grain_size, IndexMaskMemory &memory, FunctionRef<size_t(IndexMaskSegment indices, int16_t *r_true_indices)> filter_indices);
}

template<typename Fn> inline IndexMask IndexMask::from_predicate(const IndexMask &universe, const GrainSize grain_size, IndexMaskMemory &memory, Fn &&predicate) {
	return detail::from_predicate_impl(universe, grain_size, memory, [&](const IndexMaskSegment indices, int16_t *__restrict r_true_indices) {
		int16_t *r_current = r_true_indices;
		const int16_t *in_end = indices.base_span().end();
		const size_t offset = indices.offset();
		for (const int16_t *in_current = indices.base_span().data(); in_current < in_end; in_current++) {
			const int16_t local_index = *in_current;
			const size_t global_index = size_t(local_index) + offset;
			...
			*r_current = local_index;
			...
			ROSE_assert(ELEM(int8_t(condition), 0, 1));
...
template<typename T, typename Fn> void IndexMask::from_groups(const IndexMask &universe, IndexMaskMemory &memory, Fn &&get_group_index, MutableSpan<IndexMask> r_masks) {
	Vector<Vector<T>> indices_by_group(r_masks.size());
	universe.foreach_index([&](const size_t i) {
		const int group_index = get_group_index(i);
		indices_by_group[group_index].append(T(i));
	});
	for (const size_t i : r_masks.index_range()) {
...
std::optional<IndexRange> inline IndexMask::to_range() const {
	...
	const size_t first_index = this->first();
	const size_t last_index = this->last();
...
template<size_t N> inline Vector<std::variant<IndexRange, IndexMaskSegment>, N> IndexMask::to_spans_and_ranges() const {
