...
public:
	 ...
	 *  assert(stack.pop() == 5);
	 */
	Stack(const std::initializer_list<T> &values, Allocator allocator = {}) : Stack(Span<T>(values), allocator) {
	}
	...
	Stack(Stack &&other) noexcept(std::is_nothrow_move_constructible_v<T>) : Stack(NoExceptConstructor(), other.allocator_) {
		uninitialized_relocate_n<T>(other.inline_buffer_, std::min(other.size_, InlineBufferCapacity), inline_buffer_);
	...
	T &peek() {
		ROSE_assert(size_ > 0);
		ROSE_assert(top_ > top_chunk_->begin);
		return *(top_ - 1);
	}
	const T &peek() const {
		ROSE_assert(size_ > 0);
	...
	void push_multiple(Span<T> values) {
		...
		while (!remaining_values.is_empty()) {
			...
			const size_t remaining_capacity = top_chunk_->capacity_end - top_;
			const size_t amount = std::min(remaining_values.size(), remaining_capacity);
			try {
...
private:
	 ...
	 */
	void activate_next_chunk(const size_t size_hint) {
		ROSE_assert(top_ == top_chunk_->capacity_end);
		if (top_chunk_->above == nullptr) {
			const size_t new_capacity = std::max(size_hint, top_chunk_->capacity() * 2 + 10);

			/* Do a single memory allocation for the Chunk and the array it references. */
			void *buffer = allocator_.allocate(sizeof(Chunk) + sizeof(T) * new_capacity + alignof(T), alignof(Chunk), AT);
			void *chunk_buffer = buffer;
			void *data_buffer = reinterpret_cast<void *>((reinterpret_cast<uintptr_t>(buffer) + sizeof(Chunk) + alignof(T) - 1) & ~(alignof(T) - 1));
...
/**
 * Same as a normal Stack, but does not use Rose's guarded allocator. This is useful when
 * allocating memory with static storage duration.
 */
template<typename T, size_t InlineBufferCapacity = default_inline_buffer_capacity(sizeof(T))> using RawStack = Stack<T, InlineBufferCapacity, RawAllocator>;
