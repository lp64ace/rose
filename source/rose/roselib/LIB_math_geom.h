#ifndef LIB_MATH_GEOM_H
#define LIB_MATH_GEOM_H
...
typedef struct DistRayAABB_Precalc {
	float ray_origin[3];
...
typedef struct DistProjectedAABBPrecalc {
	float ray_origin[3];
	float ray_direction[3];
	float ray_inv_dir[3];
	float pmat[4][4];
 ...
 */
typedef struct IsectRayPrecalc {
...
typedef struct IsectRayAABB_Precalc {
	float ray_origin[3];
	float ray_inv_dir[3];
	int sign[3];
