...
#define _lib_array_totalsize_dynamic(arr) \
	(((arr) == NULL) ? 0 : MEM_allocN_len(arr) / sizeof(*(arr)))

#define _lib_array_totalsize_static(arr) \
	(sizeof(_##arr##_static) / sizeof(*(arr)))

#define _lib_array_totalsize(arr) \
	((size_t)(((void *)(arr) == (void *)_##arr##_static && (void *)(arr) != NULL) ? _lib_array_totalsize_static(arr) : _lib_array_totalsize_dynamic(arr)))
 ...
 */
void _lib_array_grow_func(void **arr_p, const void *arr_static, int sizeof_arr_p, int arr_len, int num, const char *alloc_str);
...
#define LIB_array_reserve(arr, num) \
	(void)((((void *)(arr) == NULL) && ((void *)(_##arr##_static) != NULL) &&		/* don't add _##arr##_len below because it must be zero */ \
			(_lib_array_totalsize_static(arr) >= (size_t)(_##arr##_len + (num)))) ? /* we have an empty array and a static var big enough */ \
				 (void)(*(void **)&arr = (void *)_##arr##_static) :					/* use existing static array or allocate */ \
				 (LIKELY(_lib_array_totalsize(arr) >= (size_t)(_##arr##_len + (num))) ? (void)0 /* do nothing */ : _lib_array_grow_func((void **)&(arr), _##arr##_static, sizeof(*(arr)), _##arr##_len, num, "LIB_array." #arr)))
...
#define LIB_array_len_set(arr, len) \
	{ \
		/** No reallocation just update the length. */ \
		_##arr##_len = (len); \
	} \
	((void)0)
