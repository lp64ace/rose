...
template<typename From, typename To> void uninitialized_convert_n(const From *src, int64_t n, To *dst) {
	std::uninitialized_copy_n(src, n, dst);
}
...
template<size_t Size, size_t Alignment> class AlignedBuffer {
	...
	ROSE_NO_UNIQUE_ADDRESS BufferType buffer_;
...
inline constexpr size_t default_inline_buffer_capacity(size_t element_size) {
	return (size_t(element_size) < 100) ? 4 : 0;
}
