#ifndef LIB_IMPLIT_SHARING_HH
#define LIB_IMPLIT_SHARING_HH
...
public:
	virtual ~ImplicitSharingInfo() {
		ROSE_assert(strong_users_ == 0);
		ROSE_assert(weak_users_ == 0);
	}
...
void *resize_trivial_array_impl(void *old_data, size_t old_size, size_t new_size, size_t alignment, const ImplicitSharingInfo **sharing_info);
void *make_trivial_data_mutable_impl(void *old_data, size_t size, size_t alignment, const ImplicitSharingInfo **sharing_info);
 ...
 */
template<typename T> void copy_shared_pointer(T *src_ptr, const ImplicitSharingInfo *src_sharing_info, T **r_dst_ptr, const ImplicitSharingInfo **r_dst_sharing_info) {
	*r_dst_ptr = src_ptr;
	*r_dst_sharing_info = src_sharing_info;
	if (*r_dst_ptr) {
		ROSE_assert(*r_dst_sharing_info != nullptr);
 ...
 */
template<typename T> void make_trivial_data_mutable(T **data, const ImplicitSharingInfo **sharing_info, const size_t size) {
	*data = static_cast<T *>(detail::make_trivial_data_mutable_impl(*data, sizeof(T) * size, alignof(T), sharing_info));
}
 ...
 * initialized.
 */
template<typename T> void resize_trivial_array(T **data, const ImplicitSharingInfo **sharing_info, size_t old_size, size_t new_size) {
	*data = static_cast<T *>(detail::resize_trivial_array_impl(*data, sizeof(T) * old_size, sizeof(T) * new_size, alignof(T), sharing_info));
}
