#ifndef LIB_MATH_EULER_TYPES_HH
#define LIB_MATH_EULER_TYPES_HH
 ...
 *
 * A `rose::math::EulerXYZ` represent an Euler triple with fixed axis order (XYZ).
 * A `rose::math::Euler3` represents an Euler triple with arbitrary axis order.
 *
...
public:
	...
	static EulerXYZBase identity() {
		return {AngleRadianBase<T>::identity(), AngleRadianBase<T>::identity(), AngleRadianBase<T>::identity()};
	}
	...
	ROSE_STRUCT_EQUALITY_OPERATORS_1(EulerXYZBase, xyz_)
...
public:
	 ...
	 */
	template<typename AngleU> Euler3Base(const VecBase<AngleU, 3> &angles_xyz, EulerOrder order) : EulerBase<T>(angles_xyz), order_(order){};

	Euler3Base(const AngleT &x, const AngleT &y, const AngleT &z, EulerOrder order) : EulerBase<T>(x, y, z), order_(order) {};
	...
	Euler3Base wrapped_around(const Euler3Base &reference) const {
		return {VecBase<AngleT, 3>(EulerXYZBase<T>(this->xyz_).wrapped_around(reference.xyz_)), order_};
	}
	...
	ROSE_STRUCT_EQUALITY_OPERATORS_2(Euler3Base, xyz_, order_)
