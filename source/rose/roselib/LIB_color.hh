 ...
 *     LIB_color_convert_to_scene_linear(theme_color).premultiply_alpha();
 * \endcode
 *
 * The API is structured to make most use of inlining. Most notable are space
 * conversions done via `LIB_color_convert_to*` functions.
 *
 * - Conversions between spaces (theme <=> scene linear) should always be done by
 *   invoking the `LIB_color_convert_to*` methods.
...
enum class eSpace {
	/** Rose theme color space (sRGB). */
	Theme,
	/** Rose internal scene linear color space (maps to scene_linear role in OCIO). */
	SceneLinear,
	/** Rose internal scene linear color space compressed to be stored in 4 uint8_t. */
...
public:
	...
	constexpr ColorRGBA(const ChannelStorageType rgba[4]) : r(rgba[0]), g(rgba[1]), b(rgba[2]), a(rgba[3]) {
	}

	constexpr ColorRGBA(const ChannelStorageType r, const ChannelStorageType g, const ChannelStorageType b, const ChannelStorageType a) : r(r), g(g), b(b), a(a) {
	}
...
ROSE_INLINE ColorTheme4<float> LIB_color_convert_to_theme4f(const ColorTheme4<uint8_t> &srgb4b);
ROSE_INLINE ColorTheme4<uint8_t> LIB_color_convert_to_theme4b(const ColorTheme4<float> &srgb4f);
...
template<eAlpha Alpha> class ColorSceneLinearByteEncoded4b final : public ColorRGBA<uint8_t, eSpace::SceneLinearByteEncoded, Alpha> {
public:
	constexpr ColorSceneLinearByteEncoded4b() = default;

	constexpr ColorSceneLinearByteEncoded4b(const uint8_t *rgba) : ColorRGBA<uint8_t, eSpace::SceneLinearByteEncoded, Alpha>(rgba) {
	}

	constexpr ColorSceneLinearByteEncoded4b(uint8_t r, uint8_t g, uint8_t b, uint8_t a) : ColorRGBA<uint8_t, eSpace::SceneLinearByteEncoded, Alpha>(r, g, b, a) {
	}
 ...
 * inlined.
 */
template<typename ChannelStorageType> class ColorTheme4 final : public ColorRGBA<ChannelStorageType, eSpace::Theme, eAlpha::Straight> {
public:
	constexpr ColorTheme4() = default;

	constexpr ColorTheme4(const ChannelStorageType *rgba) : ColorRGBA<ChannelStorageType, eSpace::Theme, eAlpha::Straight>(rgba) {
	}

	constexpr ColorTheme4(ChannelStorageType r, ChannelStorageType g, ChannelStorageType b, ChannelStorageType a) : ColorRGBA<ChannelStorageType, eSpace::Theme, eAlpha::Straight>(r, g, b, a) {
	}
	...
	ColorTheme4<float> to_4f() const {
		if constexpr ((std::is_same_v<ChannelStorageType, uint8_t>)) {
			return LIB_color_convert_to_theme4f(*this);
		}
		else {
			return *this;
		}
	...
	ColorTheme4<uint8_t> to_4b() const {
		if constexpr ((std::is_same_v<ChannelStorageType, float>)) {
			return LIB_color_convert_to_theme4b(*this);
		}
		else {
			return *this;
		}
...
ROSE_INLINE ColorTheme4b LIB_color_convert_to_theme4b(const ColorTheme4f &theme4f) {
	ColorTheme4b theme4b;
	rgba_float_to_uchar(theme4b, theme4f);
	return theme4b;
}

ROSE_INLINE ColorTheme4f LIB_color_convert_to_theme4f(const ColorTheme4b &theme4b) {
	ColorTheme4f theme4f;
	rgba_uchar_to_float(theme4f, theme4b);
	return theme4f;
}

ROSE_INLINE ColorSceneLinear4f<eAlpha::Straight> LIB_color_convert_to_scene_linear(const ColorTheme4f &theme4f) {
	ColorSceneLinear4f<eAlpha::Straight> scene_linear;
	srgb_to_linearrgb_v4(scene_linear, theme4f);
	return scene_linear;
}

ROSE_INLINE ColorSceneLinear4f<eAlpha::Straight> LIB_color_convert_to_scene_linear(const ColorTheme4b &theme4b) {
	ColorSceneLinear4f<eAlpha::Straight> scene_linear;
	srgb_to_linearrgb_uchar4(scene_linear, theme4b);
	return scene_linear;
}

ROSE_INLINE ColorTheme4f LIB_color_convert_to_theme4f(const ColorSceneLinear4f<eAlpha::Straight> &scene_linear) {
	ColorTheme4f theme4f;
	linearrgb_to_srgb_v4(theme4f, scene_linear);
	return theme4f;
}

ROSE_INLINE ColorTheme4b LIB_color_convert_to_theme4b(const ColorSceneLinear4f<eAlpha::Straight> &scene_linear) {
	ColorTheme4b theme4b;
