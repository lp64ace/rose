...
typedef struct RMHeader {
	...
	/**
	 * This is the index assigned to this specific element.
	 * - Use RM_elem_index_get/set macros to access.
	...
	/**
	 * Internal use only!
	 * \note We are very picky about not bloating this struct but in this case its padded up to 16
	 * bytes anyway, so adding a flag here gives no increase in size.
	 */
	char api_flag;
} RMHeader;
...
typedef struct RMVert {
	 ...
	 * which is a bit of an abuse of internal #RMesh data but also works OK for now.
	 * (use with care!).
	 */
	struct RMEdge *e;
} RMVert;
...
typedef struct RMLoop {
	 ...
	 *
	 * Vertices (#RMLoop.v & #RMLoop.next.b) always contain vertices from (#RMEdge.v1 & #RMEdge.v2).
 ...
 */
#ifdef __cplusplus
#	define RM_CHECK_TYPE_ELEM_ASSIGN(ele) *((void **)&ele)
#else
#	define RM_CHECK_TYPE_ELEM_ASSIGN(ele) ele
#endif
...
/* setting zero so we can catch bugs in RMesh */
#ifndef NDEBUG
#	define RM_OMP_LIMIT 0
#else
#	define RM_OMP_LIMIT 65535
#endif
