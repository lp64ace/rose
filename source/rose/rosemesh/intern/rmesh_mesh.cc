...
#include "RM_class.h"
#include "intern/rmesh_iterators.h"
...
static void rm_mempool_init_ex(const bool use_toolflags, LIB_mempool **r_vpool, LIB_mempool **r_epool, LIB_mempool **r_lpool, LIB_mempool **r_fpool) {
	...
	if (r_vpool) {
		*r_vpool = LIB_mempool_create(vert_size, rm_mesh_allocsize_default_verts, rm_mesh_chunksize_default_verts, ROSE_MEMPOOL_ALLOW_ITER);
	}
	if (r_epool) {
		*r_epool = LIB_mempool_create(edge_size, rm_mesh_allocsize_default_edges, rm_mesh_chunksize_default_edges, ROSE_MEMPOOL_ALLOW_ITER);
	}
	if (r_lpool) {
		*r_lpool = LIB_mempool_create(loop_size, rm_mesh_allocsize_default_loops, rm_mesh_chunksize_default_loops, ROSE_MEMPOOL_ALLOW_ITER);
	}
	if (r_fpool) {
		*r_fpool = LIB_mempool_create(face_size, rm_mesh_allocsize_default_faces, rm_mesh_chunksize_default_faces, ROSE_MEMPOOL_ALLOW_ITER);
	}
}
...
RMVert *RM_vert_at_index_find(RMesh *mesh, const int index) {
	return static_cast<RMVert *>(LIB_mempool_findelem(mesh->vpool, index));
}

RMEdge *RM_edge_at_index_find(RMesh *mesh, const int index) {
	return static_cast<RMEdge *>(LIB_mempool_findelem(mesh->epool, index));
}

RMFace *RM_face_at_index_find(RMesh *mesh, const int index) {
	return static_cast<RMFace *>(LIB_mempool_findelem(mesh->fpool, index));
}
