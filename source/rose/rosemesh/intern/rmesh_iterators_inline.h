...
ROSE_INLINE void *RM_iter_new(RMIter *iter, RMesh *mesh, const char itype, void *data) {
	if (LIKELY(RM_iter_init(iter, mesh, itype, data))) {
...
#ifdef __ROSE_TASK_H__

ROSE_INLINE void RM_iter_parallel(struct RMesh *mesh, const char itype, TaskParallelMempoolFunc func, void *userdata, const TaskParallelSettings *settings) {
	/* inlining optimizes out this switch when called with the defined type */
	switch ((RMIterType)itype) {
		case RM_VERTS_OF_MESH:
			...
			break;
		case RM_EDGES_OF_MESH:
			LIB_task_parallel_mempool(mesh->epool, userdata, func, settings);
			break;
		case RM_FACES_OF_MESH:
			LIB_task_parallel_mempool(mesh->fpool, userdata, func, settings);
			break;
		default:
			/* should never happen */
			ROSE_assert_unreachable();
