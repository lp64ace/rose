...
#include "RM_class.h"
#include "intern/rmesh_private.h"
#include "intern/rmesh_core.h"
...
static void rm_vert_attrs_copy(RMesh *m_src, RMesh *m_dst, const RMVert *v_src, RMVert *v_dst, eCustomDataMask mask_exclude) {
	...
	CustomData_mesh_copy_data_exclude_by_type(&m_src->vdata, &m_dst->vdata, v_src->head.data, &v_dst->head.data, mask_exclude);
}

static void rm_edge_attrs_copy(RMesh *m_src, RMesh *m_dst, const RMEdge *e_src, RMEdge *e_dst, eCustomDataMask mask_exclude) {
	...
	CustomData_mesh_copy_data_exclude_by_type(&m_src->edata, &m_dst->edata, e_src->head.data, &e_dst->head.data, mask_exclude);
}

static void rm_loop_attrs_copy(RMesh *m_src, RMesh *m_dst, const RMLoop *l_src, RMLoop *l_dst, eCustomDataMask mask_exclude) {
	...
	CustomData_mesh_copy_data_exclude_by_type(&m_src->ldata, &m_dst->ldata, l_src->head.data, &l_dst->head.data, mask_exclude);
}

static void rm_face_attrs_copy(RMesh *m_src, RMesh *m_dst, const RMFace *f_src, RMFace *f_dst, eCustomDataMask mask_exclude) {
	...
	CustomData_mesh_copy_data_exclude_by_type(&m_src->pdata, &m_dst->pdata, f_src->head.data, &f_dst->head.data, mask_exclude);
