...
uint rft_get_char_index(FontRFT *font, uint charcode) {
...
ROSE_INLINE GlyphRFT *rft_glyph_from_utf8_and_step(FontRFT *font, GlyphCacheRFT *gc, GlyphRFT *g_prev, const char *str, size_t str_len, size_t *i_p, int32_t *pen_x) {
	uint charcode = LIB_str_utf8_as_unicode_step_safe(str, str_len, i_p);
	...
	ROSE_assert(charcode != LIB_UTF8_ERR);
...
int rft_font_draw_mono(FontRFT *font, const char *str, const size_t str_len, int cwidth, int tab_columns) {
	...
	while ((i < str_len) && str[i]) {
		...
		const int col = UNLIKELY(g->c == '\t') ? (tab_columns - (columns % tab_columns)) : LIB_wcwidth_safe((char32_t)(g->c));
		columns += col;
			...
			for (int x = ((chx >= 0) ? 0 : -chx); x < width_clip; x++) {
				...
				if (a_byte) {
					...
					if (font_pixel[3] != 0) {
						...
						cbuf[0] = (uchar)divide_round_i(tmp[0], tmp[3]);
						cbuf[1] = (uchar)divide_round_i(tmp[1], tmp[3]);
						cbuf[2] = (uchar)divide_round_i(tmp[2], tmp[3]);
						cbuf[3] = (uchar)divide_round_i(tmp[3], 255);
					}
				}
			}
...
char *rft_display_name(FontRFT *font) {
	...
	return LIB_sprintf_allocN("%s %s", font->face->family_name, font->face->style_name);
}
 ...
 */
static FontRFT *rft_font_new_impl(const char *filepath, const char *mem_name, const uchar *mem, const size_t mem_size, void *ft_library) {
