...
static int rft_search_by_filepath(const char *filepath) {
	for (int i = 0; i < ROSE_MAX_FONT; i++) {
		...
		if (LIB_path_cmp(font->filepath, filepath)) {
			return i;
		}
	}
	return -1;
...
int RFT_load_unique(const char *filepath) {
	...
	/* This isn't essential, it will just cause confusing behavior to load a font that appears to succeed, then doesn't show up. */
...
void RFT_unload(const char *filepath) {
	for (int i = 0; i < ROSE_MAX_FONT; i++) {
		...
		if (LIB_path_cmp(font->filepath, filepath) == 0) {
...
char *RFT_display_name_from_file(const char *filepath) {
	...
	if (FT_Init_FreeType(&ft_library) == FT_Err_Ok) {
		...
		if (FT_New_Face(ft_library, filepath, 0, &face) == FT_Err_Ok) {
			if (face->family_name) {
				name = LIB_sprintf_allocN("%s %s", face->family_name, face->style_name);
			}
			FT_Done_Face(face);
		}
...
bool RFT_has_glyph(int fontid, uint unicode) {
	FontRFT *font = rft_get(fontid);
	if (font) {
...
void RFT_metrics_attach(int fontid, uchar *mem, int mem_size) {
	FontRFT *font = rft_get(fontid);
...
void RFT_color4ubv(int fontid, const uchar rgba[4]) {
	FontRFT *font = rft_get(fontid);
...
void RFT_color3ubv_alpha(int fontid, const uchar rgb[3], uchar alpha) {
	FontRFT *font = rft_get(fontid);
...
void RFT_color3ubv(int fontid, const uchar rgb[3]) {
	RFT_color3ubv_alpha(fontid, rgb, 255);
}

void RFT_color4ub(int fontid, uchar r, uchar g, uchar b, uchar alpha) {
	FontRFT *font = rft_get(fontid);
...
void RFT_color3ub(int fontid, uchar r, uchar g, uchar b) {
	FontRFT *font = rft_get(fontid);
...
void RFT_color4fv(int fontid, const float rgba[4]) {
	...
	if (font) {
		font->color[0] = (uchar)clampi((int)(rgba[0] * 255.0f), 0, 255);
		font->color[1] = (uchar)clampi((int)(rgba[1] * 255.0f), 0, 255);
		font->color[2] = (uchar)clampi((int)(rgba[2] * 255.0f), 0, 255);
		font->color[3] = (uchar)clampi((int)(rgba[3] * 255.0f), 0, 255);
	}
}
...
void RFT_draw_ex(int fontid, const char *str, const size_t str_len, ResultRFT *r_info) {
	FontRFT *font = rft_get(fontid);
...
void RFT_draw(int fontid, const char *str, const size_t str_len) {
...
int RFT_draw_mono(int fontid, const char *str, const size_t str_len, int cwidth, int tab_columns) {
	if (str_len == 0 || str[0] == '\0') {
		return 0;
	}
...
void RFT_shadow(int fontid, int level, const float rgba[4]) {
	...
	if (font) {
		...
		font->shadow_color[0] = (uchar)clampi(rgba[0] * 255.0f, 0, 255);
		font->shadow_color[1] = (uchar)clampi(rgba[1] * 255.0f, 0, 255);
		font->shadow_color[2] = (uchar)clampi(rgba[2] * 255.0f, 0, 255);
		font->shadow_color[3] = (uchar)clampi(rgba[3] * 255.0f, 0, 255);
	}
}
...
void RFT_buffer(int fontid, float *fbuf, uchar *cbuf, int w, int h, int nch) {
	FontRFT *font = rft_get(fontid);
...
void rft_draw_buffer__start(FontRFT *font) {
	...
	buf_info->col_char[0] = (uchar)clampi(buf_info->col_init[0] * 255.0f, 0, 255);
	buf_info->col_char[1] = (uchar)clampi(buf_info->col_init[1] * 255.0f, 0, 255);
	buf_info->col_char[2] = (uchar)clampi(buf_info->col_init[2] * 255.0f, 0, 255);
	buf_info->col_char[3] = (uchar)clampi(buf_info->col_init[3] * 255.0f, 0, 255);
