...
int RFT_draw_mono(int fontid, const char *str, size_t str_len, int cwidth);
 ...
 *
 * \note called from a thread, so it bypasses the normal RFT_* api (which isn't thread-safe).
 */
bool RFT_thumb_preview(const char *filename, unsigned char *buf, int w, int h, int channels);
...
int RFT_load_default(bool unique);
int RFT_load_mono_default(bool unique);
void RFT_load_font_stack(void);
