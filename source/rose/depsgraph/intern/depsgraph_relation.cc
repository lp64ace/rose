...
Relation::Relation(Node *from, Node *to, const char *description) : from(from), to(to), name(description), flag(RELATION_FLAG_NONE) {
...
Relation::~Relation() {
	/* Sanity check. */
	ROSE_assert(from != nullptr && to != nullptr);
}
