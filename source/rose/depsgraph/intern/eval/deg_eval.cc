...
void schedule_children(DepsgraphEvalState *state, OperationNode *node, ScheduleFunction *schedule_function, ScheduleFunctionArgs... schedule_function_args);
...
void evaluate_node(const DepsgraphEvalState *state, OperationNode *operation_node) {
	...
	/* Sanity checks. */
	ROSE_assert_msg(!operation_node->is_noop(), "NOOP nodes should not actually be scheduled");
	/* Perform operation. */
...
bool need_evaluate_operation_at_stage(DepsgraphEvalState *state, const OperationNode *operation_node) {
	...
	switch (state->stage) {
		...
		case EvaluationStage::THREADED_EVALUATION:
			 ...
			 * stage). */
			ROSE_assert(operation_node->scheduled || component_node->type != NodeType::COPY_ON_WRITE);
			return true;

		case EvaluationStage::SINGLE_THREADED_WORKAROUND:
			return true;
	}
	ROSE_assert_msg(0, "Unhandled evaluation stage, should never happen.");
	return false;
}
 ...
 */
template<typename ScheduleFunction, typename... ScheduleFunctionArgs> void schedule_node(DepsgraphEvalState *state, OperationNode *node, bool dec_parents, ScheduleFunction *schedule_function, ScheduleFunctionArgs... schedule_function_args) {
	...
	if (dec_parents) {
		ROSE_assert(node->num_links_pending > 0);
...
template<typename ScheduleFunction, typename... ScheduleFunctionArgs> void schedule_graph(DepsgraphEvalState *state, ScheduleFunction *schedule_function, ScheduleFunctionArgs... schedule_function_args) {
	for (OperationNode *node : state->graph->operations) {
		schedule_node(state, node, false, schedule_function, schedule_function_args...);
	}
}

template<typename ScheduleFunction, typename... ScheduleFunctionArgs> void schedule_children(DepsgraphEvalState *state, OperationNode *node, ScheduleFunction *schedule_function, ScheduleFunctionArgs... schedule_function_args) {
	for (Relation *rel : node->outlinks) {
		OperationNode *child = (OperationNode *)rel->to;
		ROSE_assert(child->type == NodeType::OPERATION);
		if (child->scheduled) {
			/* Happens when having cyclic dependencies. */
			continue;
		}
		schedule_node(state, child, (rel->flag & RELATION_FLAG_CYCLIC) == 0, schedule_function, schedule_function_args...);
	}
}

void schedule_node_to_queue(OperationNode *node, const int /*thread_id*/, GSQueue *evaluation_queue) {
	LIB_gsqueue_push(evaluation_queue, &node);
}

void evaluate_graph_single_threaded(DepsgraphEvalState *state) {
	GSQueue *evaluation_queue = LIB_gsqueue_new(sizeof(OperationNode *));
...
void depsgraph_ensure_view_layer(Depsgraph *graph) {
	...
	if (deg_copy_on_write_is_expanded(&scene_cow->id) && (scene_cow->id.recalc & ID_RECALC_COPY_ON_WRITE) == 0) {
		return;
	}
