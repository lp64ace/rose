#include "intern/builder/deg_builder_cycle.hh"
...
struct CyclesSolverState {
	CyclesSolverState(Depsgraph *graph) : graph(graph), traversal_stack(nullptr), num_cycles(0) {
		traversal_stack = LIB_stack_new(sizeof(StackEntry), "DEG detect cycles stack");
	}
	~CyclesSolverState() {
		LIB_stack_free(traversal_stack);
		if (num_cycles != 0) {
			fprintf(stdout, "[Depsgraph] Detected %d dependency cycles\n", num_cycles);
		}
	}
	Depsgraph *graph;
	RoseStack *traversal_stack;
	int num_cycles;
};
...
void solve_cycles(CyclesSolverState *state) {
	RoseStack *traversal_stack = state->traversal_stack;
			...
			if (rel->to->type == NodeType::OPERATION) {
				...
				if (to_state == NODE_IN_STACK) {
					...
					while (current->node != to) {
						...
						cycle_str += "  " + current->from->node->full_identifier() + " via '" + current->via_relation->name + "'\n";
						current = current->from;
					}
					fprintf(stdout, "[Depsgraph] Dependency cycle detected:\n%s", cycle_str.c_str());
