...
AnimatedPropertyID::AnimatedPropertyID(const PointerRNA *pointer_rna, const PropertyRNA *property_rna) : AnimatedPropertyID(*pointer_rna, property_rna) {
}

AnimatedPropertyID::AnimatedPropertyID(const PointerRNA &pointer_rna, const PropertyRNA *property_rna) : data(pointer_rna.data), property_rna(property_rna) {
}

AnimatedPropertyID::AnimatedPropertyID(ID *id, StructRNA *type, const char *property_name) : data(id) {
	property_rna = RNA_struct_type_find_property(type, property_name);
}

AnimatedPropertyID::AnimatedPropertyID(ID *id, StructRNA *type, void *data, const char *property_name) : data(data) {
	property_rna = RNA_struct_type_find_property(type, property_name);
}
...
void animated_property_cb(ID *id, FCurve *fcurve, void *data_v) {
	if (fcurve->path == nullptr || fcurve->path[0] == '\0') {
		return;
	}
...
void AnimatedPropertyStorage::initializeFromID(DepsgraphBuilderCache *builder_cache, ID *id) {
	...
	data.pointer_rna = RNA_id_pointer_create(id);
	data.animated_property_storage = this;
	data.builder_cache = builder_cache;
	KER_fcurves_id_cb(id, animated_property_cb, &data);
}
...
void AnimatedPropertyStorage::tagPropertyAsAnimated(const PointerRNA *pointer_rna, const PropertyRNA *property_rna) {
...
bool AnimatedPropertyStorage::isPropertyAnimated(const PointerRNA *pointer_rna, const PropertyRNA *property_rna) {
...
DepsgraphBuilderCache::~DepsgraphBuilderCache() {
	for (AnimatedPropertyStorage *animated_property_storage : animated_property_storage_map_.values()) {
		delete animated_property_storage;
	}
}

AnimatedPropertyStorage *DepsgraphBuilderCache::ensureAnimatedPropertyStorage(ID *id) {
	return animated_property_storage_map_.lookup_or_add_cb(id, []() {
		return new AnimatedPropertyStorage();
	});
}
