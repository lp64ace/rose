...
ComponentNode::OperationIDKey::OperationIDKey() : opcode(OperationCode::OPERATION), name(""), name_tag(-1) {
}

ComponentNode::OperationIDKey::OperationIDKey(OperationCode opcode) : opcode(opcode), name(""), name_tag(-1) {
}

ComponentNode::OperationIDKey::OperationIDKey(OperationCode opcode, const char *name, int name_tag) : opcode(opcode), name(name), name_tag(name_tag) {
}

std::string ComponentNode::OperationIDKey::identifier() const {
	const std::string codebuf = std::to_string(static_cast<int>(opcode));
...
uint64_t ComponentNode::OperationIDKey::hash() const {
	...
	return LIB_ghashutil_combine_hash(name_tag, LIB_ghashutil_combine_hash(LIB_ghashutil_uinthash(opcode_as_int), LIB_ghashutil_strhash_p(name)));
}

ComponentNode::ComponentNode() : entry_operation(nullptr), exit_operation(nullptr), affects_directly_visible(false) {
...
std::string ComponentNode::identifier() const {
	const std::string idname = this->owner->name;
	const std::string typebuf = "" + std::to_string(static_cast<int>(type)) + ")";
	return typebuf + name + " : " + idname + "{ affects_directly_visible: " + (affects_directly_visible ? "true" : "false") + "}";
}
...
OperationNode *ComponentNode::find_operation(OperationIDKey key) const {
	...
	if (operations_map != nullptr) {
		...
		for (OperationNode *op_node : operations) {
			if (op_node->opcode == key.opcode && op_node->name_tag == key.name_tag && STREQ(op_node->name.c_str(), key.name)) {
				node = op_node;
				break;
			}
		}
...
OperationNode *ComponentNode::find_operation(OperationCode opcode, const char *name, int name_tag) const {
...
OperationNode *ComponentNode::get_operation(OperationIDKey key) const {
	...
	if (node == nullptr) {
		fprintf(stderr, "[Depsgraph] %s: %s(%s) failed\n", this->identifier().c_str(), __func__, key.identifier().c_str());
		ROSE_assert_msg(0, "Request for non-existing operation, should not happen");
		return nullptr;
	}
	return node;
}

OperationNode *ComponentNode::get_operation(OperationCode opcode, const char *name, int name_tag) const {
	OperationIDKey key(opcode, name, name_tag);
...
OperationNode *ComponentNode::add_operation(const DepsEvalOperationCb &op, OperationCode opcode, const char *name, int name_tag) {
	...
	if (!op_node) {
		...
		fprintf(stderr, "[Depsgraph] %s: Operation already exists - %s has %s at %p\n", __func__, this->identifier().c_str(), op_node->identifier().c_str(), op_node);
...
void ComponentNode::set_entry_operation(OperationNode *op_node) {
	ROSE_assert(entry_operation == nullptr);
	entry_operation = op_node;
}

void ComponentNode::set_exit_operation(OperationNode *op_node) {
	ROSE_assert(exit_operation == nullptr);
...
void BoneComponentNode::init(const ID *id, const char *subdata) {
	...
	this->pchan = KER_pose_channel_find_name(object->pose, subdata);
}
