#ifndef DEG_NODE_COMPONENT_HH
#define DEG_NODE_COMPONENT_HH
...
#define DEG_COMPONENT_NODE_DEFINE(name, NAME, id_recalc_tag)                                                    \
	DEG_COMPONENT_NODE_DEFINE_TYPEINFO(name##ComponentNode, NodeType::NAME, #name " Component", id_recalc_tag); \
...
struct ParametersComponentNode : public ComponentNode {
	virtual bool need_tag_cow_before_update() override {
		if (ID_TYPE_SUPPORTS_PARAMS_WITHOUT_COW(owner->id_type)) {
			...
			// ROSE_assert(deg_copy_on_write_is_expanded(owner->id_cow));
			return false;
		}
		return true;
	}
