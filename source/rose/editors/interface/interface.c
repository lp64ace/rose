...
GUIBatchCache g_ui_batch_cache;
...
static void set_roundbox_vertex_data(GPUVertBufRaw *vflag_step, uint32_t d) {
	uint32_t *data = (uint32_t *)GPU_vertbuf_raw_step(vflag_step);
	*data = d;
}
...
GPUBatch *ui_batch_roundbox_widget_get() {
	if (g_ui_batch_cache.roundbox_widget == NULL) {
		...
		GPUIndexBuf *ibo = GPU_indexbuf_build(&ibuf);

		g_ui_batch_cache.roundbox_widget = GPU_batch_create_ex(GPU_PRIM_TRIS, vbo, ibo, GPU_BATCH_OWNS_INDEX | GPU_BATCH_OWNS_VBO);
