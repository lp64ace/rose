...
static CornerNormalSpace corner_fan_space_define(const float3 &lnor, const float3 &vec_ref, const float3 &vec_other, const Span<float3> edge_vectors) {
	...
	if (std::abs(dtp_ref) >= LNOR_SPACE_TRIGO_THRESHOLD || std::abs(dtp_other) >= LNOR_SPACE_TRIGO_THRESHOLD) {
	...
	if (!edge_vectors.is_empty()) {
		...
		for (const float3 &vec : edge_vectors) {
			alpha += math::safe_acos(math::dot(vec, lnor));
		}
		 ...
		 */
		ROSE_assert(edge_vectors.size() >= 2);
		lnor_space.ref_alpha = alpha / float(edge_vectors.size());
	}
	else {
		lnor_space.ref_alpha = (math::safe_acos(math::dot(vec_ref, lnor)) + math::safe_acos(math::dot(vec_other, lnor))) / 2.0f;
	}
	...
	if (dtp < LNOR_SPACE_TRIGO_THRESHOLD) {
		const float beta = math::safe_acos(dtp);
		lnor_space.ref_beta = (math::dot(lnor_space.vec_ortho, vec_other_proj) < 0.0f) ? pi2 - beta : beta;
	}
	else {
...
ROSE_INLINE float unit_short_to_float(const short val) {
	return float(val) / float(SHRT_MAX);
}

ROSE_INLINE short unit_float_to_short(const float val) {
	/* Rounding. */
...
short2 corner_space_custom_normal_to_data(const CornerNormalSpace &lnor_space, const float3 &custom_lnor) {
	...
	if (is_zero_v3(custom_lnor) || equals_v3_v3(lnor_space.vec_lnor, custom_lnor)) {
		return short2(0);
	}
	...
	const float alpha = math::safe_acos(cos_alpha);
	...
	if (cos_beta < LNOR_SPACE_TRIGO_THRESHOLD) {
		float beta = math::safe_acos(cos_beta);
...
ROSE_NOINLINE static void handle_fan_result_and_custom_normals(const Span<short2> custom_normals, const Span<VertCornerInfo> corner_infos, const Span<float3> edge_dirs, const Span<int> local_corners_in_fan, float3 &fan_normal, CornerNormalSpaceArray *r_fan_spaces, Vector<CornerSpaceGroup, 0> *r_local_space_groups) {
	...
	const CornerNormalSpace fan_space = corner_fan_space_define(fan_normal, edge_dirs[local_edge_first], edge_dirs[local_edge_last], fan_edge_dirs);
...
void normals_calc_corners(const Span<float3> vert_positions, const OffsetIndices<int> polys, const Span<int> corner_verts, const Span<int> corner_edges, const GroupedSpan<int> vert_to_face_map, const Span<float3> poly_normals, const Span<bool> sharp_edges, const Span<bool> sharp_faces, const Span<short2> custom_normals, CornerNormalSpaceArray *r_fan_spaces, MutableSpan<float3> corner_normals) {
	...
	threading::parallel_for(vert_positions.index_range(), 256, [&](const IndexRange range) {
		...
		Vector<CornerSpaceGroup, 0> *local_space_groups = r_fan_spaces ? &space_groups.local() : nullptr;
		...
		for (const int vert : range) {
			...
			while (true) {
				...
				if (!custom_normals.is_empty() || r_fan_spaces) {
					handle_fan_result_and_custom_normals(custom_normals, corner_infos, edge_dirs, corners_in_fan, fan_normal, r_fan_spaces, local_space_groups);
				}
	...
	const OffsetIndices<int> space_offsets = offset_indices::accumulate_counts_to_offsets(space_groups_count);
	...
	const int64_t mean_size = std::max<int64_t>(1, space_offsets.total_size() / space_offsets.size());
	...
	threading::parallel_for(all_space_groups.index_range(), grain_size, [&](const IndexRange range) {
		for (const int thread_i : range) {
			...
			for (const int group_i : local_space_groups.index_range()) {
				...
				r_fan_spaces->corner_space_indices.as_mutable_span().fill_indices(local_space_groups[group_i].fan_corners.as_span(), space_index);
			}
			if (!r_fan_spaces->create_corners_by_space) {
				continue;
			}
			for (const int group_i : local_space_groups.index_range()) {
				const int space_index = space_offsets[thread_i][group_i];
				r_fan_spaces->corners_by_space[space_index] = std::move(local_space_groups[group_i].fan_corners);
			}
		}
	});
}
...
static void mesh_normals_corner_custom_set(const Span<float3> positions, const OffsetIndices<int> faces, const Span<int> corner_verts, const Span<int> corner_edges, const GroupedSpan<int> vert_to_face_map, const Span<float3> vert_normals, const Span<float3> face_normals, const Span<bool> sharp_faces, const bool use_vertices, MutableSpan<float3> r_custom_corner_normals, MutableSpan<bool> sharp_edges, MutableSpan<short2> r_clnors_data) {
	/* We *may* make that poor #kernel::mesh::normals_calc_corners() even more complex by making it
	 ...
	 * better to keep some simplicity here, and just call #kernel::mesh::normals_calc_corners() twice!
	 */
	CornerNormalSpaceArray lnors_spacearr;
	lnors_spacearr.create_corners_by_space = true;

	rose::Array<float3> corner_normals(corner_verts.size());
	rose::Array<int> corner_to_face = build_corner_to_face_map(faces);

	/* Compute current lnor spacearr. */
	normals_calc_corners(positions, faces, corner_verts, corner_edges, vert_to_face_map, face_normals, sharp_edges, sharp_faces, r_clnors_data, &lnors_spacearr, corner_normals);
	...
	rose::BitVector<> done_corners(corner_verts.size(), false);
	...
	for (const int i : corner_verts.index_range()) {
		if (lnors_spacearr.corner_space_indices[i] == -1) {
			...
			fprintf(stderr, "[Kernel] Still getting invalid nullptr corner space in second for loop %d!\n", i);
			continue;
		}
		if (!done_corners[i]) {
			continue;
		...
		if (fan_corners.size() < 2) {
			...
			r_clnors_data[i] = corner_space_custom_normal_to_data(lnors_spacearr.spaces[space_index], r_custom_corner_normals[nidx]);
			done_corners[i].reset();
		}
		else {
			float3 avg_nor(0.0f);
			...
			short2 clnor_data_tmp = corner_space_custom_normal_to_data(lnors_spacearr.spaces[space_index], avg_nor);
...
ROSE_INLINE void mesh_set_custom_normals(Mesh *mesh, rose::MutableSpan<float3> r_custom_normals, const bool use_vertices) {
	short2 *custom_normals_ptr = NULL;

	do {
		/** If a layer already exists use that one, otherwise create a new one. */
		custom_normals_ptr = static_cast<short2 *>(CustomData_get_layer_named_for_write(&mesh->ldata, CD_PROP_INT16_2D, "custom_normal", mesh->totloop));
		if (custom_normals_ptr) {
			break;
		}

		custom_normals_ptr = static_cast<short2 *>(CustomData_add_layer_named(&mesh->ldata, CD_PROP_INT16_2D, CD_CONSTRUCT, mesh->totloop, "custom_normal"));
		if (custom_normals_ptr) {
			break;
		}
	} while (false);
	...
	rose::MutableSpan<short2> custom_normals(custom_normals_ptr, mesh->totloop);

	rose::MutableSpan<bool> sharp_edges = KER_mesh_edge_sharp_edge_for_write_span(mesh);
	rose::MutableSpan<bool> sharp_faces = KER_mesh_poly_sharp_face_for_write_span(mesh);

	const rose::Span<float3> vert_positions = KER_mesh_vert_positions_span(mesh);
	...
	const rose::GroupedSpan<int> vert_to_face_map = KER_mesh_vert_to_face_map_span(mesh);
	const rose::Span<float3> vert_normals = KER_mesh_vert_normals_span(mesh);
	const rose::Span<float3> poly_normals = KER_mesh_poly_normals_span(mesh);

	mesh_normals_corner_custom_set(vert_positions, polys, corner_verts, corner_edges, vert_to_face_map, vert_normals, poly_normals, sharp_faces, use_vertices, r_custom_normals, sharp_edges, custom_normals);
}
...
const float (*KER_mesh_corner_normals_ensure(const struct Mesh *mesh))[3] {
	...
	mesh->runtime->corner_normals_cache.ensure([&](rose::Vector<float3> &r_data) {
		...
		if (CustomData_get_layer_named(&mesh->ldata, CD_PROP_FLOAT3, "custom_normal")) {
			rose::Span<float3> custom_normals(static_cast<const float3 *>(CustomData_get_layer_named(&mesh->ldata, CD_PROP_FLOAT3, "custom_normal")), mesh->totloop);

			for (size_t i : r_data.index_range()) {
				r_data[i] = custom_normals[i];
			}

			return;
		}

		const rose::Span<bool> sharp_edges = KER_mesh_edge_sharp_edge_span(mesh);
		const rose::Span<bool> sharp_faces = KER_mesh_poly_sharp_face_span(mesh);
		...
		if (CustomData_get_layer_named(&mesh->ldata, CD_PROP_INT16_2D, "custom_normal")) {
			rose::Span<short2> custom_normals(
				static_cast<const short2 *>(CustomData_get_layer_named(&mesh->ldata, CD_PROP_INT16_2D, "custom_normal")),
				mesh->totloop
			);

			rose::kernel::mesh::normals_calc_corners(positions, polys, corner_verts, corner_edges, vert_to_face, poly_normals, sharp_edges, sharp_faces, custom_normals, NULL, r_data);
		}
		else {
			rose::kernel::mesh::normals_calc_corners(positions, polys, corner_verts, corner_edges, vert_to_face, poly_normals, sharp_edges, sharp_faces, rose::Span<short2>(), NULL, r_data);
		}
	});
...
static void normalize_vecs(rose::MutableSpan<float3> normals) {
	rose::threading::parallel_for(normals.index_range(), 4096, [&](const rose::IndexRange range) {
		for (const int i : range) {
			normals[i] = rose::math::normalize(normals[i]);
		}
	});
}

void KER_mesh_set_custom_normals(Mesh *mesh, float (*r_normals)[3]) {
	rose::MutableSpan<float3> corner_normals(reinterpret_cast<float3 *>(r_normals), mesh->totloop);

	normalize_vecs(corner_normals);

	rose::kernel::mesh::mesh_set_custom_normals(mesh, corner_normals, false);
}
