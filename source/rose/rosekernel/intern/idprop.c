...
void IDP_CopyProperty_ex(IDProperty *destination, const IDProperty *source, int flag) {
...
IDProperty *IDP_CopyString(const IDProperty *property, const int flag) {
	ROSE_assert(property->type == IDP_STRING);

	IDProperty *newp = IDP_DuplicateGeneric(property, flag);
	if (IDP_String(property)) {
		IDP_String(newp) = MEM_dupallocN(IDP_String(property));
	}
...
IDProperty *IDP_DuplicateIDPArray(const IDProperty *array, int flag) {
	...
	IDP_IDPArray(narray) = MEM_dupallocN(IDP_IDPArray(array));
...
void IDP_ResizeIDPArray(IDProperty *property, size_t length) {
	...
	size_t newsize = (length >> 3) + (length < 9 ? 3 : 6) + length;
	IDP_IDPArray(property) = MEM_recallocN(IDP_IDPArray(property), sizeof(IDProperty) * newsize);
