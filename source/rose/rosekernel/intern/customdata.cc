...
ROSE_STATIC_ASSERT(ARRAY_SIZE(((CustomData *)NULL)->typemap) == CD_NUMTYPES, "size mismatch");
...
static void layerInterp_propInt(const void **sources, const float *weights, const float * /*sub_weights*/, const int count, void *dest) {
	float result = 0.0f;
	for (const int64_t i : rose::IndexRange(count)) {
...
static bool layerEqual_propfloat2(const void *data1, const void *data2) {
	const rose::float2 &a = *static_cast<const rose::float2 *>(data1);
	const rose::float2 &b = *static_cast<const rose::float2 *>(data2);
	
	return len_squared_v2(a - b) < 0.00001f;
}

static void layerInitMinMax_propfloat2(void *vmin, void *vmax) {
	rose::float2 &min = *static_cast<rose::float2 *>(vmin);
	rose::float2 &max = *static_cast<rose::float2 *>(vmax);
	INIT_MINMAX2(min, max);
}

static void layerDoMinMax_propfloat2(const void *data, void *vmin, void *vmax) {
	const rose::float2 &value = *static_cast<const rose::float2 *>(data);
	rose::float2 &a = *static_cast<rose::float2 *>(vmin);
	rose::float2 &b = *static_cast<rose::float2 *>(vmax);
	DO_MINMAX2(value, a, b);
}

static void layerCopyValue_propfloat2(const void *source, void *dest, const int mixmode, const float mixfactor) {
	const rose::float2 &a = *static_cast<const rose::float2 *>(source);
	rose::float2 &b = *static_cast<rose::float2 *>(dest);
	...
	if (mixmode == CDT_MIX_NOMIX) {
		...
		interp_v2_v2v2(b, b, a, mixfactor);
	}
}
...
static CustomDataLayer *customData_add_layer__internal(CustomData *data, eCustomDataType type, std::optional<eCDAllocType> alloctype, void *layer_data_to_assign, const rose::ImplicitSharingInfo *sharing_info_to_assign, int totelem, const char *name);
...
static bool BM_attribute_stored_in_mesh_builtin(const char *name) {
#ifdef _DEBUG
...
CustomData CustomData_shallow_copy_remove_non_mesh_attributes(const CustomData *src, const eCustomDataMask mask) {
	rose::Vector<CustomDataLayer> dst_layers;
	for (const CustomDataLayer &layer : rose::Span<CustomDataLayer>(src->layers, src->totlayer)) {
		if (BM_attribute_stored_in_mesh_builtin(layer.name)) {
			continue;
		}
	...
	CustomData dst = *src;
	dst.layers = static_cast<CustomDataLayer *>(MEM_calloc_arrayN(dst_layers.size(), sizeof(CustomDataLayer), __func__));
...
static CustomDataLayer *customData_add_layer__internal(CustomData *data, const eCustomDataType type, const std::optional<eCDAllocType> alloctype, void *layer_data_to_assign, const rose::ImplicitSharingInfo *sharing_info_to_assign, const int totelem, const char *name) {
	const LayerTypeInfo &type_info = *layerType_getInfo(type);
	int flag = 0;
...
const void *CustomData_add_layer_with_data(CustomData *data, const eCustomDataType type, void *layer_data, const int totelem, const rose::ImplicitSharingInfo *sharing_info) {
	const LayerTypeInfo *typeInfo = layerType_getInfo(type);
...
void *CustomData_add_layer_named(CustomData *data, const eCustomDataType type, const eCDAllocType alloctype, const int totelem, const char *name) {
...
const void *CustomData_add_layer_named_with_data(CustomData *data, eCustomDataType type, void *layer_data, int totelem, const char *name, const rose::ImplicitSharingInfo *sharing_info) {
...
void *CustomData_add_layer_anonymous(CustomData *data, const eCustomDataType type, const eCDAllocType alloctype, const int totelem, const AnonymousAttributeIDHandle *anonymous_id) {
...
const void *CustomData_add_layer_anonymous_with_data(CustomData *data, const eCustomDataType type, const AnonymousAttributeIDHandle *anonymous_id, const int totelem, void *layer_data, const ImplicitSharingInfoHandle *sharing_info) {
	const char *name = anonymous_id->name().c_str();
	CustomDataLayer *layer = customData_add_layer__internal(data, type, std::nullopt, layer_data, sharing_info, totelem, name);
	CustomData_update_typemap(data);
...
static void CustomData_mesh_set_default_n(CustomData *data, void **block, const int n) {
...
void CustomData_mesh_set_default(CustomData *data, void **block) {
	if (*block == nullptr) {
		CustomData_mesh_alloc_block(data, block);
	}

	for (int i = 0; i < data->totlayer; i++) {
		CustomData_mesh_set_default_n(data, block, i);
	}
}
...
void CustomData_mesh_copy_data_exclude_by_type(const CustomData *source, CustomData *dest, void *src_block, void **dest_block, const eCustomDataMask mask_exclude) {
	...
	if (*dest_block == nullptr) {
		CustomData_mesh_alloc_block(dest, dest_block);
		if (*dest_block) {
	...
	for (int src_i = 0; src_i < source->totlayer; src_i++) {
		...
		while (dest_i < dest->totlayer && dest->layers[dest_i].type < source->layers[src_i].type) {
			CustomData_mesh_set_default_n(dest, dest_block, dest_i);
			dest_i++;
		}
...
void CustomData_mesh_copy_data(const CustomData *source, CustomData *dest, void *src_block, void **dest_block) {
	CustomData_mesh_copy_data_exclude_by_type(source, dest, src_block, dest_block, 0);
}
...
void CustomData_copy_data_layer(const CustomData *source, CustomData *dest, const int src_layer_index, const int dst_layer_index, const int src_index, const int dst_index, const int count) {
	...
	if (!count || !src_data || !dst_data) {
		if (count && !(src_data == nullptr && dst_data == nullptr)) {
			fprintf(stderr, "null data for %s type (%p --> %p), skipping", layerType_getName(eCustomDataType(source->layers[src_layer_index].type)), (void *)src_data, (void *)dst_data);
		}
		return;
	}
...
void CustomData_mesh_free_block(CustomData *data, void **block) {
	if (*block == nullptr) {
		return;
	}
...
void CustomData_mesh_free_block_data_exclude_by_type(CustomData *data, void *block, const eCustomDataMask mask_exclude) {
	if (block == nullptr) {
		return;
	}
	for (int i = 0; i < data->totlayer; i++) {
...
void CustomData_mesh_free_block_data(CustomData *data, void *block) {
	if (block == nullptr) {
		return;
	}
	for (int i = 0; i < data->totlayer; i++) {
...
void CustomData_mesh_alloc_block(CustomData *data, void **block) {
	if (*block) {
		CustomData_mesh_free_block(data, block);
	}
...
bool CustomData_mesh_has_free(const CustomData *data) {
	for (int i = 0; i < data->totlayer; i++) {
		const LayerTypeInfo *typeInfo = layerType_getInfo(eCustomDataType(data->layers[i].type));
		if (typeInfo->free) {
			return true;
...
void CustomData_set_layer_unique_name(CustomData *data, const int index) {
	...
	if (nlayer->name[0] == '\0') {
		LIB_strncpy(nlayer->name, typeInfo->defaultname, ARRAY_SIZE(nlayer->name));
	}
...
void CustomData_validate_layer_name(const CustomData *data, const eCustomDataType type, const char *name, char *outname) {
	...
	if (index == -1) {
		...
		LIB_strncpy(outname, data->layers[index].name, MAX_CUSTOMDATA_LAYER_NAME);
	}
	else {
		LIB_strncpy(outname, name, MAX_CUSTOMDATA_LAYER_NAME);
	}
}
...
static bool CustomData_layer_ensure_data_exists(CustomDataLayer *layer, size_t count) {
	...
	switch (layer->type) {
		...
		case CD_PROP_BOOL:
		case CD_PROP_FLOAT2:
			layer->data = MEM_calloc_arrayN(count, typeInfo->size, layerType_getName(eCustomDataType(layer->type)));
		...
		default:
			...
			fprintf(stderr, "CustomDataLayer->data is NULL for type %d.", layer->type);
			break;
	}
	return false;
}
...
bool CustomData_layer_validate(CustomDataLayer *layer, const uint totitems, const bool do_fixes) {
	...
	ROSE_assert(MEM_allocN_len(layer->data) >= totitems * typeInfo->size);
...
const rose::CPPType *custom_data_type_to_cpp_type(const eCustomDataType type) {
	switch (type) {
		case CD_PROP_FLOAT:
			return &rose::CPPType::get<float>();
		case CD_PROP_FLOAT2:
			return &rose::CPPType::get<float2>();
		case CD_PROP_FLOAT3:
			return &rose::CPPType::get<float3>();
		case CD_PROP_INT32:
			return &rose::CPPType::get<int>();
		case CD_PROP_INT32_2D:
			return &rose::CPPType::get<int2>();
		case CD_PROP_COLOR:
			return &rose::CPPType::get<ColorGeometry4f>();
		case CD_PROP_BOOL:
			return &rose::CPPType::get<bool>();
		case CD_PROP_INT8:
			return &rose::CPPType::get<int8_t>();
		case CD_PROP_BYTE_COLOR:
			return &rose::CPPType::get<ColorGeometry4b>();
		case CD_PROP_STRING:
			return &rose::CPPType::get<MStringProperty>();
		default:
			return nullptr;
	}
}

eCustomDataType cpp_type_to_custom_data_type(const rose::CPPType &type) {
	if (type.is<float>()) {
