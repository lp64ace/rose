...
public:
	...
	template<ParamCategory Category, typename T> void add(ParamTag<Category, T> /*tag*/, const char *name);
...
inline SignatureBuilder::SignatureBuilder(const char *function_name, Signature &signature_to_build) : signature_(signature_to_build) {
...
template<typename T> inline void SignatureBuilder::single_output(const char *name, const ParamFlag flag) {
	this->single_output(name, CPPType::get<T>(), flag);
}

inline void SignatureBuilder::single_output(const char *name, const CPPType &type, const ParamFlag flag) {
	this->output(name, DataType::ForSingle(type), flag);
}

template<typename T> inline void SignatureBuilder::vector_output(const char *name, const ParamFlag flag) {
	this->vector_output(name, CPPType::get<T>(), flag);
}

inline void SignatureBuilder::vector_output(const char *name, const CPPType &base_type, const ParamFlag flag) {
...
template<ParamCategory Category, typename T> inline void SignatureBuilder::add(ParamTag<Category, T> /*tag*/, const char *name) {
	switch (Category) {
