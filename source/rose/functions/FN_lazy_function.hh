...
struct Context {
	...
	Context(void *storage, UserData *user_data, LocalUserData *local_user_data) : storage(storage), user_data(user_data), local_user_data(local_user_data) {
	}
};
...
struct Input {
	...
	Input(const char *debug_name, const CPPType &type, const ValueUsage usage = ValueUsage::Used) : debug_name(debug_name), type(&type), usage(usage) {
	}
};
...
inline void LazyFunction::execute(Params &params, const Context &context) const {
	ROSE_assert(this->always_used_inputs_available(params));
...
inline Params::Params(const LazyFunction &fn, [[maybe_unused]] bool allow_multi_threading_initially)
	: fn_(fn)
#ifdef FN_LAZY_FUNCTION_DEBUG_THREADS
	  ,
...
inline void *Params::try_get_input_data_ptr(const int index) const {
	ROSE_assert(index >= 0 && index < fn_.inputs().size());
...
inline void *Params::get_output_data_ptr(const int index) {
	ROSE_assert(index >= 0 && index < fn_.outputs().size());
	this->assert_valid_thread();
