...
protected:
	...
	GFieldBase(NodePtr node, const int node_output_index) : node_(std::move(node)), node_output_index_(node_output_index) {
	}
...
public:
	...
	GField(std::shared_ptr<FieldNode> node, const int node_output_index = 0) : GFieldBase<std::shared_ptr<FieldNode>>(std::move(node), node_output_index) {
	}
};
...
public:
	...
	GFieldRef(const GField &field) : GFieldBase<const FieldNode *>(&field.node(), field.node_output_index()) {
	}

	GFieldRef(const FieldNode &node, const int node_output_index = 0) : GFieldBase<const FieldNode *>(&node, node_output_index) {
	}
};
...
public:
	...
	Field(GField field) : GField(std::move(field)) {
		ROSE_assert(this->cpp_type().template is<T>());
	}
	...
	Field(std::shared_ptr<FieldNode> node, const int node_output_index = 0) : Field(GField(std::move(node), node_output_index)) {
	}
};

/** True when T is any Field<...> type. */
template<typename T> static constexpr bool is_field_v = std::is_base_of_v<detail::TypedFieldBase, T> && !std::is_same_v<detail::TypedFieldBase, T>;
...
public:
	...
	rose::StringRef debug_name() const;
	const CPPType &cpp_type() const;
...
public:
	...
	FieldEvaluator(const FieldContext &context, const IndexMask *mask) : context_(context), mask_(*mask) {
	}

	/** Construct a field evaluator for all indices less than #size. */
	FieldEvaluator(const FieldContext &context, const size_t size) : context_(context), mask_(scope_.construct<IndexMask>(size)) {
	}
	...
	template<typename T> int add(Field<T> field, VArray<T> *varray_ptr) {
		...
		dst_varrays_.append({});
		output_pointer_infos_.append(OutputPointerInfo{varray_ptr, [](void *dst, const GVArray &varray, ResourceScope & /*scope*/) { *(VArray<T> *)dst = varray.typed<T>(); }});
		return field_index;
	}
