...
static void build_multi_function_procedure_for_fields(mf::Procedure &procedure, ResourceScope &scope, const FieldTreeInfo &field_tree_info, Span<GFieldRef> output_fields) {
	...
	for (const FieldInput &field_input : field_tree_info.deduplicated_field_inputs) {
		mf::Variable &variable = builder.add_input_parameter(mf::DataType::ForSingle(field_input.cpp_type()), field_input.debug_name());
		...
		while (!fields_to_check.is_empty()) {
			...
			switch (field_node.node_type()) {
					...
					if (field_with_index.current_input_index < operation_inputs.size()) {
						...
						for (const int param_index : multi_function.param_indices()) {
							...
							if (interface_type == mf::ParamType::Input) {
								...
								const bool output_is_ignored = field_tree_info.field_users.lookup(output_field).is_empty() && !output_fields.contains(output_field);
				...
				case FieldNodeType::Constant: {
					...
					const mf::MultiFunction &fn = procedure.construct_function<mf::CustomMF_GenericConstant>(constant_node.type(), constant_node.value().get(), false);
	...
	for (const GFieldRef &field : output_fields) {
		...
		if (!already_output_variables.add(variable)) {
			 ...
			 * a copy first. */
			const mf::MultiFunction &copy_fn = scope.construct<mf::CustomMF_GenericCopy>(variable->data_type());
	...
	ROSE_assert(procedure.validate());
}
...
Vector<GVArray> evaluate_fields(ResourceScope &scope, Span<GFieldRef> fields_to_evaluate, const IndexMask &mask, const FieldContext &context, Span<GVMutableArray> dst_varrays) {
	...
	auto get_dst_varray = [&](int index) -> GVMutableArray {
		...
		ROSE_assert(varray.size() >= array_size);
		return varray;
	};
	...
	Vector<GVArray> field_context_inputs = get_field_context_inputs(scope, mask, context, field_tree_info.deduplicated_field_inputs);
	...
	for (const int out_index : fields_to_evaluate.index_range()) {
		...
		switch (field_node.node_type()) {
			...
			case FieldNodeType::Constant: {
				...
				r_varrays[out_index] = GVArray::ForSingleRef(field_constant.type(), mask.min_array_size(), field_constant.value().get());
				break;
			}
			case FieldNodeType::Operation: {
				break;
	...
	if (!varying_fields_to_evaluate.is_empty()) {
		...
		build_multi_function_procedure_for_fields(procedure, scope, field_tree_info, varying_fields_to_evaluate);
		...
		for (const int i : varying_fields_to_evaluate.index_range()) {
			...
			if (!dst_varray || !dst_varray.is_span()) {
				...
				if (!type.is_trivially_destructible()) {
					...
					scope.add_destruct_call([buffer, mask, &type]() { type.destruct_indices(buffer, mask); });
				}

				r_varrays[out_index] = GVArray::ForSpan({type, buffer, (size_t)array_size});
			}
			else {
			...
			const GMutableSpan span{type, buffer, (size_t)array_size};
	...
	if (!constant_fields_to_evaluate.is_empty()) {
		/* Build the procedure for those fields. */
		mf::Procedure procedure;
		build_multi_function_procedure_for_fields(procedure, scope, field_tree_info, constant_fields_to_evaluate);
		mf::ProcedureExecutor procedure_executor{procedure};
	...
	if (!dst_varrays.is_empty()) {
		for (const int out_index : fields_to_evaluate.index_range()) {
			...
			ROSE_assert(computed_varray.type() == dst_varray.type());
			...
			if (dst_varray.is_span()) {
				array_utils::copy(computed_varray, mask, dst_varray.get_internal_span().take_front(mask.min_array_size()));
			}
			else {
...
Field<bool> invert_boolean_field(const Field<bool> &field) {
	static auto not_fn = mf::build::SI1_SO<bool, bool>("Not", [](bool a) { return !a; }, mf::build::exec_presets::AllSpanOrSingle());
...
GVArray FieldContext::get_varray_for_input(const FieldInput &field_input, const IndexMask &mask, ResourceScope &scope) const {
...
GVArray IndexFieldInput::get_varray_for_context(const fn::FieldContext & /*context*/, const IndexMask &mask, ResourceScope & /*scope*/) const {
...
FieldOperation::FieldOperation(std::shared_ptr<const mf::MultiFunction> function, Vector<GField> inputs) : FieldOperation(*function, std::move(inputs)) {
...
static std::shared_ptr<const FieldInputs> combine_field_inputs(Span<GField> fields) {
	...
	std::shared_ptr<FieldInputs> new_field_inputs = std::make_shared<FieldInputs>(**field_inputs_candidate);
...
FieldOperation::FieldOperation(const mf::MultiFunction &function, Vector<GField> inputs) : FieldNode(FieldNodeType::Operation), function_(&function), inputs_(std::move(inputs)) {
...
FieldInput::FieldInput(const CPPType &type, std::string debug_name) : FieldNode(FieldNodeType::Input), type_(&type), debug_name_(std::move(debug_name)) {
...
FieldConstant::FieldConstant(const CPPType &type, const void *value) : FieldNode(FieldNodeType::Constant), type_(type) {
...
int FieldEvaluator::add(GField field, GVArray *varray_ptr) {
	...
	output_pointer_infos_.append(OutputPointerInfo{varray_ptr, [](void *dst, const GVArray &varray, ResourceScope & /*scope*/) { *static_cast<GVArray *>(dst) = varray; }});
	return field_index;
}
...
static IndexMask evaluate_selection(const Field<bool> &selection_field, const FieldContext &context, IndexMask full_mask, ResourceScope &scope) {
	if (selection_field) {
		VArray<bool> selection = evaluate_fields(scope, {selection_field}, full_mask, context)[0].typed<bool>();
...
void FieldEvaluator::evaluate() {
	ROSE_assert_msg(!is_evaluated_, "Cannot evaluate fields twice.");
	...
	ROSE_assert(fields_to_evaluate_.size() == evaluated_varrays_.size());
	for (const int i : fields_to_evaluate_.index_range()) {
