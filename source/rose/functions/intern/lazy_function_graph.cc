...
void Graph::add_link(OutputSocket &from, InputSocket &to) {
	ROSE_assert(to.origin_ == nullptr);
	ROSE_assert(from.type_ == to.type_);
	to.origin_ = &from;
...
std::string Socket::detailed_name() const {
	std::stringstream ss;
	ss << node_->name() << ":" << (is_input_ ? "IN" : "OUT") << ":" << index_in_node_ << ":" << this->name();
	return ss.str();
}
...
void Graph::ToDotOptions::add_edge_attributes(const OutputSocket & /*from*/, const InputSocket & /*to*/, dot::DirectedEdge & /*dot_edge*/) const {
}
...
std::string Graph::to_dot(const ToDotOptions &options) const {
	...
	for (const Node *node : nodes_) {
		...
		for (const InputSocket *socket : node->inputs()) {
			dot::NodeWithSockets::Input &dot_input = dot_node_with_sockets.add_input(options.socket_name(*socket));
			dot_input.fontcolor = options.socket_font_color(*socket);
		}
		for (const OutputSocket *socket : node->outputs()) {
			dot::NodeWithSockets::Output &dot_output = dot_node_with_sockets.add_output(options.socket_name(*socket));
	...
	for (const Node *node : nodes_) {
		for (const InputSocket *socket : node->inputs()) {
			...
			if (const OutputSocket *origin = socket->origin()) {
				...
				dot::DirectedEdge &dot_edge = digraph.new_edge(from_dot_node.output(origin->index()), to_dot_port);
