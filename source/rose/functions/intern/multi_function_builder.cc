...
CustomMF_GenericConstant::CustomMF_GenericConstant(const CPPType &type, const void *value, bool make_value_copy) : type_(type), owns_value_(make_value_copy) {
	if (make_value_copy) {
...
void CustomMF_GenericConstant::call(const IndexMask &mask, Params params, Context /*context*/) const {
...
void CustomMF_GenericConstantArray::call(const IndexMask &mask, Params params, Context /*context*/) const {
	GVectorArray &vectors = params.vector_output(0);
	mask.foreach_index([&](const size_t i) { vectors.extend(i, array_); });
}

CustomMF_DefaultOutput::CustomMF_DefaultOutput(Span<DataType> input_types, Span<DataType> output_types) : output_amount_(output_types.size()) {
