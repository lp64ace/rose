...
public:
	...
	void split_into(ScheduledNodes &other) {
		...
		const size_t priority_split = priority_.size() / 2;
		const size_t normal_split = normal_.size() / 2;
...
public:
	Executor(const GraphExecutor &self) : self_(self) {
		...
		ROSE_assert(self_.graph_.node_indices_are_valid());
	}
	...
	void execute(Params &params, const Context &context) {
		...
		ROSE_SCOPED_DEFER(deferred_func);
		...
		if (is_first_execution_) {
			...
			char *buffer = static_cast<char *>(local_data.allocator->allocate(self_.init_buffer_info_.total_size, alignof(void *)));
			this->initialize_node_states(buffer);

			loaded_inputs_ = MutableSpan{reinterpret_cast<std::atomic<uint8_t> *>(buffer + self_.init_buffer_info_.loaded_inputs_array_offset), self_.graph_inputs_.size()};
			...
			if (self_.side_effect_provider_ != nullptr) {
				...
				for (const FunctionNode *node : side_effect_nodes) {
					ROSE_assert(self_.graph_.nodes().contains(node));
...
private:
	void initialize_node_states(char *buffer) {
		...
		node_states_ = MutableSpan{reinterpret_cast<NodeState **>(buffer + self_.init_buffer_info_.node_states_array_offset), nodes.size()};
	...
	void schedule_for_new_output_usages(CurrentTask &current_task, const LocalData &local_data) {
		for (const int graph_output_index : self_.graph_outputs_.index_range()) {
			...
			this->with_locked_node(node, node_state, current_task, local_data, [&](LockedNode &locked_node) {
	...
	void set_defaulted_graph_outputs(const LocalData &local_data) {
		for (const int graph_output_index : self_.graph_outputs_.index_range()) {
			...
			ROSE_assert(default_value != nullptr);
	...
	void schedule_side_effect_nodes(const Span<const FunctionNode *> side_effect_nodes, CurrentTask &current_task, const LocalData &local_data) {
		for (const FunctionNode *node : side_effect_nodes) {
			NodeState &node_state = *node_states_[node->index_in_graph()];
			this->with_locked_node(*node, node_state, current_task, local_data, [&](LockedNode &locked_node) { this->schedule_node(locked_node, current_task, false); });
		}
	}
	...
	void forward_newly_provided_input(CurrentTask &current_task, const LocalData &local_data, const int graph_input_index, void *input_data) {
	...
	void notify_output_required(const OutputSocket &socket, CurrentTask &current_task, const LocalData &local_data) {
		const Node &node = socket.node();
	...
	void notify_output_unused(const OutputSocket &socket, CurrentTask &current_task, const LocalData &local_data) {
		const Node &node = socket.node();
		const int index_in_node = socket.index();
		NodeState &node_state = *node_states_[node.index_in_graph()];
		OutputState &output_state = node_state.outputs[index_in_node];
		...
		this->with_locked_node(node, node_state, current_task, local_data, [&](LockedNode &locked_node) {
			...
			if (output_state.potential_target_sockets == 0) {
				ROSE_assert(output_state.usage != ValueUsage::Unused);
	...
	void schedule_node(LockedNode &locked_node, CurrentTask &current_task, const bool is_priority) {
		ROSE_assert(locked_node.node.is_function());
	...
	void with_locked_node(const Node &node, NodeState &node_state, CurrentTask &current_task, const LocalData &local_data, const FunctionRef<void(LockedNode &)> f) {
		ROSE_assert(&node_state == node_states_[node.index_in_graph()]);
		...
		this->send_output_required_notifications(locked_node.delayed_required_outputs, current_task, local_data);
		this->send_output_unused_notifications(locked_node.delayed_unused_outputs, current_task, local_data);
	}

	void send_output_required_notifications(const Span<const OutputSocket *> sockets, CurrentTask &current_task, const LocalData &local_data) {
		for (const OutputSocket *socket : sockets) {
			this->notify_output_required(*socket, current_task, local_data);
		}
	}

	void send_output_unused_notifications(const Span<const OutputSocket *> sockets, CurrentTask &current_task, const LocalData &local_data) {
		for (const OutputSocket *socket : sockets) {
	...
	void run_node_task(const FunctionNode &node, CurrentTask &current_task, const LocalData &local_data) {
		NodeState &node_state = *node_states_[node.index_in_graph()];
		...
		this->with_locked_node(node, node_state, current_task, local_data, [&](LockedNode &locked_node) {
			ROSE_assert(node_state.schedule_state == NodeScheduleState::Scheduled);
		...
		if (node_needs_execution) {
			if (!node_state.storage_and_defaults_initialized) {
				...
				for (const int input_index : node.inputs().index_range()) {
					...
					ROSE_assert(input_state.value == nullptr);
		...
		this->with_locked_node(node, node_state, current_task, local_data, [&](LockedNode &locked_node) {
			...
			const bool reschedule_requested = node_state.schedule_state == NodeScheduleState::RunningAndRescheduled;
	...
	void assert_expected_outputs_have_been_computed(LockedNode &locked_node, const LocalData &local_data) {
		const FunctionNode &node = static_cast<const FunctionNode &>(locked_node.node);
	...
	void execute_node(const FunctionNode &node, NodeState &node_state, CurrentTask &current_task, const LocalData &local_data);

	void set_input_unused_during_execution(const Node &node, NodeState &node_state, const int input_index, CurrentTask &current_task, const LocalData &local_data) {
		const InputSocket &input_socket = node.input(input_index);
		this->with_locked_node(node, node_state, current_task, local_data, [&](LockedNode &locked_node) { this->set_input_unused(locked_node, input_socket); });
	}
	...
	void set_input_unused(LockedNode &locked_node, const InputSocket &input_socket) {
		...
		ROSE_assert(input_state.usage != ValueUsage::Used);
	...
	void *set_input_required_during_execution(const Node &node, NodeState &node_state, const int input_index, CurrentTask &current_task, const LocalData &local_data) {
		const InputSocket &input_socket = node.input(input_index);
		void *result;
		this->with_locked_node(node, node_state, current_task, local_data, [&](LockedNode &locked_node) { result = this->set_input_required(locked_node, input_socket); });
		return result;
	}

	void *set_input_required(LockedNode &locked_node, const InputSocket &input_socket) {
		ROSE_assert(&locked_node.node == &input_socket.node());
		NodeState &node_state = locked_node.node_state;
		const int input_index = input_socket.index();
		InputState &input_state = node_state.inputs[input_index];

		ROSE_assert(input_state.usage != ValueUsage::Unused);
		...
		ROSE_assert(origin_socket != nullptr);
		locked_node.delayed_required_outputs.append(origin_socket);
		return nullptr;
	}

	void forward_value_to_linked_inputs(const OutputSocket &from_socket, GMutablePointer value_to_forward, CurrentTask &current_task, const LocalData &local_data) {
		ROSE_assert(value_to_forward.get() != nullptr);
		const CPPType &type = *value_to_forward.type();
		const Context local_context{context_->storage, context_->user_data, local_data.local_user_data};
		...
		for (const InputSocket *target_socket : targets) {
...
#endif
			ROSE_assert(!input_state.was_ready_for_execution);
			ROSE_assert(target_socket->type() == type);
			ROSE_assert(target_socket->origin() == &from_socket);
			...
			if (target_node.is_interface()) {
				...
				const int graph_output_index = self_.graph_output_index_by_socket_index_[target_socket->index()];
				if (graph_output_index != -1 && params_->get_output_usage(graph_output_index) != ValueUsage::Unused) {
			...
			this->with_locked_node(target_node, node_state, current_task, local_data, [&](LockedNode &locked_node) {
				if (input_state.usage == ValueUsage::Unused) {
					return;
				}
				if (is_last_target) {
	...
	void forward_value_to_input(LockedNode &locked_node, InputState &input_state, GMutablePointer value, CurrentTask &current_task) {
		NodeState &node_state = locked_node.node_state;
		...
		if (input_state.usage == ValueUsage::Used) {
			node_state.missing_required_inputs -= 1;
			if (node_state.missing_required_inputs == 0 || (locked_node.node.is_function() && static_cast<const FunctionNode &>(locked_node.node).function().allow_missing_requested_inputs())) {
				this->schedule_node(locked_node, current_task, false);
			}
		}
	}
	...
	bool try_enable_multi_threading() {
		...
		if (LIB_system_thread_count() <= 1) {
			return false;
		}
		this->ensure_thread_locals();
		task_pool_.store(LIB_task_pool_create(this, TASK_PRIORITY_HIGH));
		return true;
	}
	...
	void push_all_scheduled_nodes_to_task_pool(CurrentTask &current_task) {
		ROSE_assert(this->use_multi_threading());
	...
	void push_to_task_pool(std::unique_ptr<ScheduledNodes> scheduled_nodes) {
		...
		LIB_task_pool_push(
			...
			[](TaskPool *pool, void *data) {
				Executor &executor = *static_cast<Executor *>(LIB_task_pool_user_data(pool));
...
public:
	GraphExecutorLFParams(const LazyFunction &fn, Executor &executor, const Node &node, NodeState &node_state, CurrentTask &current_task, const Executor::LocalData &local_data) : Params(fn, node_state.enabled_multi_threading), executor_(executor), node_(node), node_state_(node_state), current_task_(current_task), caller_local_data_(local_data) {
	}
...
private:
	...
	void *try_get_input_data_ptr_or_request_impl(const int index) override {
		...
		return executor_.set_input_required_during_execution(node_, node_state_, index, current_task_, this->get_local_data());
	}

	void *get_output_data_ptr_impl(const int index) override {
		OutputState &output_state = node_state_.outputs[index];
		ROSE_assert(!output_state.has_been_computed);
	...
	void output_set_impl(const int index) override {
		OutputState &output_state = node_state_.outputs[index];
		ROSE_assert(!output_state.has_been_computed);
		ROSE_assert(output_state.value != nullptr);
		const OutputSocket &output_socket = node_.output(index);
		executor_.forward_value_to_linked_inputs(output_socket, {output_socket.type(), output_state.value}, current_task_, this->get_local_data());
	...
	void set_input_unused_impl(const int index) override {
		executor_.set_input_unused_during_execution(node_, node_state_, index, current_task_, this->get_local_data());
	}
 ...
 */
inline void Executor::execute_node(const FunctionNode &node, NodeState &node_state, CurrentTask &current_task, const LocalData &local_data) {
	const LazyFunction &fn = node.function();
...
GraphExecutor::GraphExecutor(const Graph &graph, Vector<const GraphInputSocket *> graph_inputs, Vector<const GraphOutputSocket *> graph_outputs, const Logger *logger, const SideEffectProvider *side_effect_provider, const NodeExecuteWrapper *node_execute_wrapper) : graph_(graph), graph_inputs_(std::move(graph_inputs)), graph_outputs_(std::move(graph_outputs)), graph_input_index_by_socket_index_(graph.graph_inputs().size(), -1), graph_output_index_by_socket_index_(graph.graph_outputs().size(), -1), logger_(logger), side_effect_provider_(side_effect_provider), node_execute_wrapper_(node_execute_wrapper) {
	...
	for (const int i : graph_inputs_.index_range()) {
		...
		ROSE_assert(socket.node().is_interface());
...
void GraphExecutorLogger::log_socket_value(const Socket &socket, const GPointer value, const Context &context) const {
	UNUSED_VARS(socket, value, context);
}

void GraphExecutorLogger::log_before_node_execute(const FunctionNode &node, const Params &params, const Context &context) const {
	UNUSED_VARS(node, params, context);
}

void GraphExecutorLogger::log_after_node_execute(const FunctionNode &node, const Params &params, const Context &context) const {
	UNUSED_VARS(node, params, context);
}

Vector<const FunctionNode *> GraphExecutorSideEffectProvider::get_nodes_with_side_effects(const Context &context) const {
	UNUSED_VARS(context);
	return {};
}

void GraphExecutorLogger::dump_when_outputs_are_missing(const FunctionNode &node, Span<const OutputSocket *> missing_sockets, const Context &context) const {
	UNUSED_VARS(node, missing_sockets, context);
}

void GraphExecutorLogger::dump_when_input_is_set_twice(const InputSocket &target_socket, const OutputSocket &from_socket, const Context &context) const {
