...
using IndicesSplitVectors = std::array<Vector<size_t>, 2>;
...
struct VariableValue_GVectorArray : public VariableValue {
	...
	VariableValue_GVectorArray(GVectorArray &data, bool owned) : VariableValue(static_type), data(data), owned(owned) {
	}
};
...
public:
	...
	VariableValue_Span *obtain_Span(const CPPType &type, int size) {
		...
		if (alignment > min_alignment) {
			...
			Stack<void *> *stack = type.can_exist_in_buffer(small_value_max_size, small_value_max_alignment) ? &small_span_buffers_free_list_ : span_buffers_free_lists_.lookup_ptr(element_size);
			if (stack == nullptr || stack->is_empty()) {
				buffer = linear_allocator_.allocate(std::max<size_t>(element_size, small_value_max_size) * size, min_alignment);
			}
			else {
	...
	VariableValue_OneSingle *obtain_OneSingle(const CPPType &type) {
		const bool is_small = type.can_exist_in_buffer(small_value_max_size, small_value_max_alignment);
		Stack<void *> &stack = is_small ? small_single_value_free_list_ : single_value_free_lists_.lookup_or_add_default(&type);
		void *buffer;
		if (stack.is_empty()) {
			buffer = linear_allocator_.allocate(std::max<int>(small_value_max_size, type.size()), std::max<int>(small_value_max_alignment, type.alignment()));
		}
		else {
			buffer = stack.pop();
		}
		...
		switch (value->type) {
			...
			case ValueType::Span: {
				...
				if (value_typed->owned) {
					...
					Stack<void *> &buffers = type.can_exist_in_buffer(small_value_max_size, small_value_max_alignment) ? small_span_buffers_free_list_ : span_buffers_free_lists_.lookup_or_add_default(type.size());
...
public:
	...
	void add_as_input(ParamsBuilder &params, const IndexMask &mask, const DataType &data_type) const {
		...
		ROSE_assert(mask.size() <= tot_initialized_);
		ROSE_assert(value_ != nullptr);
		...
		switch (value_->type) {
			...
			case ValueType::OneSingle: {
				...
				ROSE_assert(value_typed->is_initialized);
		...
		switch (data_type.category()) {
			case DataType::Single: {
				...
				if (value_ != nullptr) {
					if (value_->type == ValueType::GVArray) {
						...
						this->value_as<VariableValue_GVArray>()->data.materialize_to_uninitialized(full_mask, new_value->data);
					}
			...
			case DataType::Vector: {
				...
				if (caller_provided_storage_ == nullptr) {
					...
					new_value = value_allocator.obtain_GVectorArray_not_owned(*static_cast<GVectorArray *>(caller_provided_storage_));
				}
				if (value_ != nullptr) {
					if (value_->type == ValueType::GVVectorArray) {
						...
						new_value->data.extend(full_mask, GVVectorArray_For_SingleGSpan{vector, (size_t)array_size});
					}
					else {
						ROSE_assert_unreachable();
					}
	...
	void add_as_output(ParamsBuilder &params, const IndexMask &mask, const IndexMask &full_mask, const DataType &data_type, ValueAllocator &value_allocator) {
		...
		ROSE_assert(mask.size() <= full_mask.size() - tot_initialized_);
		this->ensure_is_mutable(full_mask, data_type, value_allocator);
		ROSE_assert(value_ != nullptr);
	...
	void ensure_is_mutable__one(const DataType &data_type, ValueAllocator &value_allocator) {
		...
		switch (data_type.category()) {
			case DataType::Single: {
				...
				if (value_ != nullptr) {
					if (value_->type == ValueType::GVArray) {
						this->value_as<VariableValue_GVArray>()->data.get_internal_single_to_uninitialized(new_value->data);
						new_value->is_initialized = true;
					}
					else if (value_->type == ValueType::Span) {
						ROSE_assert(tot_initialized_ == 0);
			...
			case DataType::Vector: {
				...
				if (value_ != nullptr) {
					if (value_->type == ValueType::GVVectorArray) {
						const GVVectorArray &old_vector_array = this->value_as<VariableValue_GVVectorArray>()->data;
	...
	void add_as_output__one(ParamsBuilder &params, const IndexMask &mask, const DataType &data_type, ValueAllocator &value_allocator) {
		...
		switch (value_->type) {
			case ValueType::OneSingle: {
				auto *value_typed = this->value_as<VariableValue_OneSingle>();
				ROSE_assert(!value_typed->is_initialized);
				params.add_uninitialized_single_output(GMutableSpan{data_type.single_type(), value_typed->data, 1});
			...
			case ValueType::OneVector: {
				...
				ROSE_assert(value_typed->data[0].is_empty());
	...
	bool destruct(const IndexMask &mask, const IndexMask &full_mask, const DataType &data_type, ValueAllocator &value_allocator) {
		...
		ROSE_assert(new_tot_initialized >= 0);
		...
		switch (value_->type) {
			case ValueType::GVArray: {
				if (mask.size() < full_mask.size()) {
					 ...
					 * create a copy first. */
					this->ensure_is_mutable(full_mask, data_type, value_allocator);
					ROSE_assert(value_->type == ValueType::Span);
					const CPPType &type = data_type.single_type();
			...
			case ValueType::GVVectorArray: {
				if (mask.size() < full_mask.size()) {
					 ...
					 * copy directly. */
					this->ensure_is_mutable(full_mask, data_type, value_allocator);
					ROSE_assert(value_->type == ValueType::GVectorArray);
		...
		const bool should_self_destruct = new_tot_initialized == 0 && caller_provided_storage_ == nullptr;
	...
	void indices_split(const IndexMask &mask, IndicesSplitVectors &r_indices) {
		...
		switch (value_->type) {
			case ValueType::GVArray: {
				...
				mask.foreach_index([&](const size_t i) { r_indices[varray[i]].append(i); });
				break;
			}
			case ValueType::Span: {
				const Span<bool> span(static_cast<const bool *>(this->value_as<VariableValue_Span>()->data), mask.min_array_size());
				mask.foreach_index([&](const size_t i) { r_indices[span[i]].append(i); });
				break;
			}
			case ValueType::OneSingle: {
				auto *value_typed = this->value_as<VariableValue_OneSingle>();
				...
				Vector<size_t> &indices = r_indices[condition];
				indices.reserve(indices.size() + mask.size());
				mask.foreach_index_optimized<size_t>([&](const size_t i) { indices.append(i); });
				break;
			}
			case ValueType::GVVectorArray:
			case ValueType::GVectorArray:
	...
	template<typename T> T *value_as() {
		ROSE_assert(value_ != nullptr);
		ROSE_assert(value_->type == T::static_type);
...
public:
	VariableStates(LinearAllocator<> &linear_allocator, const Procedure &procedure, const IndexMask &full_mask) : value_allocator_(linear_allocator), procedure_(procedure), variable_states_(procedure.variables().size()), full_mask_(full_mask) {
	}
	...
	void add_initial_variable_states(const ProcedureExecutor &fn, const Procedure &procedure, Params &params) {
		for (const int param_index : fn.param_indices()) {
			...
			auto add_state = [&](VariableValue *value, bool input_is_initialized, void *caller_provided_storage = nullptr) {
				...
				ROSE_assert(variable_state.value_ == nullptr);
...
public:
	...
	void add_owned_indices(const Instruction &instruction, Vector<size_t> indices) {
		if (indices.is_empty()) {
			return;
		}
		...
		new_indices.referenced_indices = IndexMask::from_indices<size_t>(indices, *new_indices.memory);
		next_instructions_.push({&instruction, std::move(new_indices)});
	}
...
void ProcedureExecutor::call(const IndexMask &full_mask, Params params, Context context) const {
	ROSE_assert(procedure_.validate());
	...
	while (!scheduler.is_done()) {
		...
		switch (instruction.type()) {
			case InstructionType::Call: {
				const CallInstruction &call_instruction = static_cast<const CallInstruction &>(instruction);
			...
			case InstructionType::Branch: {
				const BranchInstruction &branch_instruction = static_cast<const BranchInstruction &>(instruction);
			...
			case InstructionType::Destruct: {
				const DestructInstruction &destruct_instruction = static_cast<const DestructInstruction &>(instruction);
			...
			case InstructionType::Dummy: {
				const DummyInstruction &dummy_instruction = static_cast<const DummyInstruction &>(instruction);
	...
	for (const int param_index : this->param_indices()) {
		...
		switch (param_type.interface_type()) {
			case ParamType::Input: {
				...
				ROSE_assert(variable_state.is_fully_uninitialized(full_mask));
				break;
			}
			case ParamType::Mutable:
			case ParamType::Output: {
				/* Mutable and output variables must be initialized in the end. */
				ROSE_assert(variable_state.is_fully_initialized(full_mask));
				/* Make sure that the data is in the memory provided by the caller. */
				variable_state.ensure_is_mutable(full_mask, param_type.data_type(), variable_states.value_allocator());
				break;
			}
		}
	}
