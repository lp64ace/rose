...
static bool supports_threading_by_slicing_params(const MultiFunction &fn) {
	for (const int i : fn.param_indices()) {
		...
		if (ELEM(param_type.interface_type(), ParamType::InterfaceType::Mutable, ParamType::InterfaceType::Output)) {
...
static size_t compute_grain_size(const ExecutionHints &hints, const IndexMask &mask) {
	size_t grain_size = hints.min_grain_size;
	if (hints.uniform_execution_time) {
		const int thread_count = LIB_system_thread_count();
		/* Avoid using a small grain size even if it is not necessary. */
		const size_t thread_based_grain_size = mask.size() / thread_count / 4;
		grain_size = std::max(grain_size, thread_based_grain_size);
	}
	if (hints.allocates_array) {
		const size_t max_grain_size = 10000;
...
static size_t compute_alignment(const size_t grain_size) {
	if (grain_size <= 512) {
		...
		switch (param_type.category()) {
			...
			case ParamCategory::SingleOutput: {
				if (bool(signature.params[param_index].flag & ParamFlag::SupportsUnusedOutput)) {
					const GMutableSpan span = full_params.uninitialized_single_output_if_required(param_index);
					if (span.is_empty()) {
...
void MultiFunction::call_auto(const IndexMask &mask, Params params, Context context) const {
	...
	const size_t grain_size = compute_grain_size(hints, mask);
	...
	const size_t alignment = compute_alignment(grain_size);
	threading::parallel_for_aligned(mask.index_range(), grain_size, alignment, [&](const IndexRange sub_range) {
		...
		const size_t input_slice_start = sliced_mask[0];
		const size_t input_slice_size = sliced_mask.last() - input_slice_start + 1;
		const IndexRange input_slice_range{input_slice_start, input_slice_size};

		IndexMaskMemory memory;
		const size_t offset = -input_slice_start;
