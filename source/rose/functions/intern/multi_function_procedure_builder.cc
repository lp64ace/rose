...
CallInstruction &ProcedureBuilder::add_call_with_all_variables(const MultiFunction &fn, Span<Variable *> param_variables) {
	CallInstruction &instruction = this->add_call_with_no_variables(fn);
	instruction.set_params(param_variables);
	return instruction;
}

Vector<Variable *> ProcedureBuilder::add_call(const MultiFunction &fn, Span<Variable *> input_and_mutable_variables) {
	...
	for (const int param_index : fn.param_indices()) {
		...
		switch (param_type.interface_type()) {
			...
			case ParamType::Output: {
				Variable &variable = procedure_->new_variable(param_type.data_type(), fn.param_name(param_index));
				instruction.set_param_variable(param_index, &variable);
	...
	ROSE_assert(input_and_mutable_variables.is_empty());
