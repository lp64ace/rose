...
void CallInstruction::set_param_variable(int param_index, Variable *variable) {
	...
	if (variable != nullptr) {
		...
		ROSE_assert(param_type.data_type() == variable->data_type());
#endif
...
void CallInstruction::set_params(Span<Variable *> variables) {
	ROSE_assert(variables.size() == params_.size());
...
bool Procedure::validate_initialization() const {
	...
	for (const DestructInstruction *instruction : destruct_instructions_) {
		...
		const InitState state = this->find_initialization_state_before_instruction(*instruction, variable);
	...
	for (const ReturnInstruction *instruction : return_instructions_) {
		for (const Variable *variable : variables_) {
			const InitState init_state = this->find_initialization_state_before_instruction(*instruction, *variable);
...
Procedure::InitState Procedure::find_initialization_state_before_instruction(const Instruction &target_instruction, const Variable &target_variable) const {
	InitState state;
...
public:
	...
	void create_nodes() {
		...
		auto add_instructions = [&](auto instructions) { all_instructions.extend(instructions.begin(), instructions.end()); };
		...
		for (const Instruction *representative : all_instructions) {
			...
			Vector<const Instruction *> block_instructions = this->get_instructions_in_block(*representative);
			std::stringstream ss;
			ss << "<";
	...
	void create_edges() {
		auto create_edge = [&](dot::Node &from_node, const Instruction *to_instruction) -> dot::DirectedEdge & {
		...
		for (auto item : dot_nodes_by_end_.items()) {
			...
			switch (from_instruction.type()) {
				case InstructionType::Call: {
					const Instruction *to_instruction = static_cast<const CallInstruction &>(from_instruction).next();
					create_edge(from_node, to_instruction);
					break;
				}
				case InstructionType::Destruct: {
					const Instruction *to_instruction = static_cast<const DestructInstruction &>(from_instruction).next();
					create_edge(from_node, to_instruction);
					break;
				}
				case InstructionType::Dummy: {
					const Instruction *to_instruction = static_cast<const DummyInstruction &>(from_instruction).next();
					create_edge(from_node, to_instruction);
					break;
				}
				case InstructionType::Return: {
				...
				case InstructionType::Branch: {
					const BranchInstruction &branch_instruction = static_cast<const BranchInstruction &>(from_instruction);
	...
	bool has_to_be_block_begin(const Instruction &instruction) {
		...
		if (ELEM(instruction.prev()[0].type(), InstructionCursor::Type::Branch, InstructionCursor::Type::Entry)) {
			return true;
		}
		return false;
	}
	...
	Vector<const Instruction *> get_instructions_in_block(const Instruction &representative) {
		...
		for (const Instruction *current = &begin; current != nullptr; current = this->get_next_instruction_in_block(*current, begin)) {
