...
#include "LIB_generic_pointer.hh"
...
public:
	...
	template<typename T> MutableSpan<T> uninitialized_single_output(int param_index, StringRef name = "");
	 ...
	 * required.
	 */
	template<typename T> MutableSpan<T> uninitialized_single_output_if_required(int param_index, StringRef name = "");
	GMutableSpan uninitialized_single_output_if_required(int param_index, StringRef name = "");

	template<typename T> const VVectorArray<T> &readonly_vector_input(int param_index, StringRef name = "");
	const GVVectorArray &readonly_vector_input(int param_index, StringRef name = "");

	template<typename T> GVectorArray_TypedMutableRef<T> vector_output(int param_index, StringRef name = "");
	...
	template<typename T> GVectorArray_TypedMutableRef<T> vector_mutable(int param_index, StringRef name = "");
...
inline ParamsBuilder::ParamsBuilder(const Signature &signature, const IndexMask &mask) : signature_(&signature), mask_(mask), min_array_size_(mask.min_array_size()) {
	actual_params_.reserve(signature.params.size());
}

template<typename T> inline void ParamsBuilder::add_readonly_single_input_value(T value, StringRef expected_name) {
	this->assert_current_param_type(ParamType::ForSingleInput(CPPType::get<T>()), expected_name);
	actual_params_.append_unchecked_as(std::in_place_type<GVArray>, varray_tag::single{}, CPPType::get<T>(), min_array_size_, &value);
}

template<typename T> inline void ParamsBuilder::add_readonly_single_input(const T *value, StringRef expected_name) {
	this->assert_current_param_type(ParamType::ForSingleInput(CPPType::get<T>()), expected_name);
	actual_params_.append_unchecked_as(std::in_place_type<GVArray>, varray_tag::single_ref{}, CPPType::get<T>(), min_array_size_, value);
}
...
inline void ParamsBuilder::add_readonly_single_input(const GSpan span, StringRef expected_name) {
	...
	ROSE_assert(span.size() >= min_array_size_);
...
inline void ParamsBuilder::add_readonly_single_input(GPointer value, StringRef expected_name) {
	...
	actual_params_.append_unchecked_as(std::in_place_type<GVArray>, varray_tag::single_ref{}, *value.type(), min_array_size_, value.get());
}
...
inline void ParamsBuilder::add_readonly_single_input(GVArray varray, StringRef expected_name) {
	...
	ROSE_assert(varray.size() >= min_array_size_);
	actual_params_.append_unchecked_as(std::in_place_type<GVArray>, std::move(varray));
}

inline void ParamsBuilder::add_readonly_vector_input(const GVectorArray &vector_array, StringRef expected_name) {
	this->add_readonly_vector_input(this->resource_scope().construct<GVVectorArray_For_GVectorArray>(vector_array), expected_name);
}

inline void ParamsBuilder::add_readonly_vector_input(const GSpan single_vector, StringRef expected_name) {
	this->add_readonly_vector_input(this->resource_scope().construct<GVVectorArray_For_SingleGSpan>(single_vector, min_array_size_), expected_name);
}

inline void ParamsBuilder::add_readonly_vector_input(const GVVectorArray &ref, StringRef expected_name) {
	this->assert_current_param_type(ParamType::ForVectorInput(ref.type()), expected_name);
	ROSE_assert(ref.size() >= min_array_size_);
	actual_params_.append_unchecked_as(std::in_place_type<const GVVectorArray *>, &ref);
}

template<typename T> inline void ParamsBuilder::add_uninitialized_single_output(T *value, StringRef expected_name) {
	this->add_uninitialized_single_output(GMutableSpan(CPPType::get<T>(), value, 1), expected_name);
}

inline void ParamsBuilder::add_uninitialized_single_output(GMutableSpan ref, StringRef expected_name) {
...
inline void ParamsBuilder::add_ignored_single_output(StringRef expected_name) {
	...
	ROSE_assert(param_type.category() == ParamCategory::SingleOutput);
...
inline void ParamsBuilder::add_vector_output(GVectorArray &vector_array, StringRef expected_name) {
	...
	ROSE_assert(vector_array.size() >= min_array_size_);
...
inline GMutableSpan ParamsBuilder::computed_array(int param_index) {
	ROSE_assert(ELEM(signature_->params[param_index].type.category(), ParamCategory::SingleOutput, ParamCategory::SingleMutable));
...
inline GVectorArray &ParamsBuilder::computed_vector_array(int param_index) {
	ROSE_assert(ELEM(signature_->params[param_index].type.category(), ParamCategory::VectorOutput, ParamCategory::VectorMutable));
...
inline void ParamsBuilder::assert_current_param_type(ParamType param_type, StringRef expected_name) {
	...
	if (expected_name != "") {
		...
		ROSE_assert(actual_name == expected_name);
	}

	ParamType expected_type = signature_->params[param_index].type;
	ROSE_assert(expected_type == param_type);
#endif
}
...
template<typename T> inline VArray<T> Params::readonly_single_input(int param_index, StringRef name) {
...
template<typename T> inline MutableSpan<T> Params::uninitialized_single_output(int param_index, StringRef name) {
...
inline GMutableSpan Params::uninitialized_single_output(int param_index, StringRef name) {
	this->assert_correct_param(param_index, name, ParamCategory::SingleOutput);
	ROSE_assert(!bool(builder_->signature_->params[param_index].flag & ParamFlag::SupportsUnusedOutput));
	GMutableSpan span = std::get<GMutableSpan>(builder_->actual_params_[param_index]);
	ROSE_assert(span.size() >= builder_->min_array_size_);
	return span;
}

template<typename T> inline MutableSpan<T> Params::uninitialized_single_output_if_required(int param_index, StringRef name) {
	return this->uninitialized_single_output_if_required(param_index, name).typed<T>();
}

inline GMutableSpan Params::uninitialized_single_output_if_required(int param_index, StringRef name) {
	this->assert_correct_param(param_index, name, ParamCategory::SingleOutput);
	ROSE_assert(bool(builder_->signature_->params[param_index].flag & ParamFlag::SupportsUnusedOutput));
	return std::get<GMutableSpan>(builder_->actual_params_[param_index]);
}

template<typename T> inline const VVectorArray<T> &Params::readonly_vector_input(int param_index, StringRef name) {
...
template<typename T> inline GVectorArray_TypedMutableRef<T> Params::vector_output(int param_index, StringRef name) {
...
template<typename T> inline GVectorArray_TypedMutableRef<T> Params::vector_mutable(int param_index, StringRef name) {
...
#ifndef NDEBUG
	ROSE_assert(builder_->signature_->params[param_index].type == param_type);
	if (name.size() > 0) {
		ROSE_assert(builder_->signature_->params[param_index].name == name);
	}
#endif
}
...
#ifndef NDEBUG
	ROSE_assert(builder_->signature_->params[param_index].type.category() == category);
	if (name.size() > 0) {
		ROSE_assert(builder_->signature_->params[param_index].name == name);
	}
#endif
